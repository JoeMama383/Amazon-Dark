...
assert 'Version: 7.493~orders-endtext-ci-repair' in C
assert '#define AD_VERSION "v7.493-orders-endtext-ci-repair"' in T
assert 'VER=7.493' in UI
assert 'AD_PROBE_VERSION=7.493' in SK
assert len(T.encode()) < 856000, len(T.encode())
for h in ('## FULL — v7.493','## VIEWPORT — v7.493','## TRANSITION — v7.493'):
    assert h in CMD, h
assert ' status' not in CMD
assert '### Arm' in CMD and '### Export' in CMD
assert 'tests/test_v7490_your_orders_theme.py' in (R/'scripts/validate.sh').read_text()
...
print('PASS: v7.493 exact-sync CI guard and probe-backed end-of-orders text fix retain the v7.490/v7.491 Orders and transition work')
