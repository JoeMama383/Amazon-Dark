...
assert 'VER=7.452' in UI and 'AD_PROBE_VERSION=7.452' in SK
...
assert len((R/'src/ADUniversalUIProbe7362.inc').read_bytes()) < 80000  # v7.452 adds PDP-only streaming control; production Tweak size gate remains unchanged
assert len(SB.encode()) < 19200
print('PASS: v7.452 preserves performance-consolidation recurring-work invariants, PDP frame caching, linear probe queues, and production source-size gates')
