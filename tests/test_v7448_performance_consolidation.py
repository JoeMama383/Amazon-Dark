...
assert 'VER=7.455' in UI and 'AD_PROBE_VERSION=7.455' in SK
...
assert len((R/'src/ADUniversalUIProbe7362.inc').read_bytes()) < 95000  # v7.455 adds PDP-only manual diagnostic control; production Tweak size gate remains unchanged
assert len(SB.encode()) < 19200
print('PASS: v7.455 preserves performance-consolidation recurring-work invariants, PDP frame caching, linear probe queues, and production source-size gates')
