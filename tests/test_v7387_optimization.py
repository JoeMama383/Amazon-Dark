...
S_golden=S
# v7.421 broadens only the BYG renderer ownership. Normalize those exact approved
...
for name,h in golden['probe_sha256'].items():
    ...
    if name=='ADUniversalUIProbe7362.js.inc':data=data.replace(b"version:'7.421'",b"version:'7.386'")  # capture-version metadata only
