...
assert 'Version: 7.491~orders-followup' in C
assert '#define AD_VERSION "v7.491-orders-followup"' in T
assert 'VER=7.491' in UI
assert 'AD_PROBE_VERSION=7.491' in SK
assert len(T.encode()) < 856000, len(T.encode())
for h in ('## FULL — v7.491','## VIEWPORT — v7.491','## TRANSITION — v7.491'):
    assert h in CMD, h
...
print('PASS: v7.491 follow-up themes the blue info tiles, search-bar seam, and quantity bubble while retaining the v7.490 Orders/transition ownership')
