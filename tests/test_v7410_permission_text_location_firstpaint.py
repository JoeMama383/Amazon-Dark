...
assert 'VER=7.410' in UI and 'AD_PROBE_VERSION=7.410' in SK and 'AD_PROBE_NAME=AmazonDark-v7.410' in SK
...
assert 'if(ADLocationInsetScrollerWitness7409(v))return YES;' in loc
outer=loc[loc.index('static BOOL ADLocationOuterWhiteShell7408'):loc.index('static BOOL ADLocationTransitionTopRail7410')]
assert outer.index('if(ADLocationInsetScrollerWitness7409(v))return YES;') < outer.index('UIView *root=ADLocationRootAny7202(v)')
