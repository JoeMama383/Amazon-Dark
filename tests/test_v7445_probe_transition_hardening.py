...
with tempfile.TemporaryDirectory(prefix='ad-v7447-ui-') as t:
    ...
    now=int(time.time())
    full='AmazonDark-v7.448-ui-full-probe-20260921-000000-000-r1.txt'; view='AmazonDark-v7.448-ui-viewport-probe-20260921-000001-000-r1.txt'
    (docs/full).write_text('FULL\n================ END RUN ================\n'); (docs/view).write_text('VIEWPORT\n================ END RUN ================\n')
    (docs/'AmazonDark-v7.448-ui-full.state').write_text(f'completed {now} {full}\n'); (docs/'AmazonDark-v7.448-ui-viewport.state').write_text(f'completed {now} {view}\n')
    ...
    (docs/'AmazonDark-v7.448-ui-full.state').write_text(f'partial {now} {full}\n')
    before=set(shared.glob('*.tar'))
    assert 'exactly one partial' in run('export','full')
    assert set(shared.glob('*.tar'))==before
    run('export',ok=False)
    (docs/'AmazonDark-v7.448-ui-full.state').write_text(f'started {now} {full}\n')
    assert 'still running or incomplete' in run('export','full',ok=False)

print('PASS: v7.448 freezes armed VIEWPORT at background, uses TAR for all probe exports, preserves bounded FULL, and retains the scale-normalized transition fix')
