...
with tempfile.TemporaryDirectory(prefix='ad-v7445-ui-') as t:
    ...
    now=int(time.time())
    full='AmazonDark-v7.446-ui-full-probe-20260921-000000-000-r1.txt'; view='AmazonDark-v7.446-ui-viewport-probe-20260921-000001-000-r1.txt'
    (docs/full).write_text('FULL\n================ END RUN ================\n'); (docs/view).write_text('VIEWPORT\n================ END RUN ================\n')
    (docs/'AmazonDark-v7.446-ui-full.state').write_text(f'completed {now} {full}\n'); (docs/'AmazonDark-v7.446-ui-viewport.state').write_text(f'completed {now} {view}\n')
    b=root/'bin'; b.mkdir(); (b/'plutil').write_text(PLUTIL); (b/'plutil').chmod(0o755); (b/'zip').write_text(ZIP_SHIM); (b/'zip').chmod(0o755); (b/'dpkg-query').write_text("#!/bin/sh\nprintf '7.446~probe-responsiveness'\n"); (b/'dpkg-query').chmod(0o755)
    ...
    # Partial evidence remains exportable; repeated export does not multiply ZIPs.
    (docs/'AmazonDark-v7.446-ui-full.state').write_text(f'partial {now} {full}\n')
    before=set(shared.glob('*.zip'))
    assert 'exactly one partial' in run('export','full')
    assert set(shared.glob('*.zip'))==before
    run('export',ok=False)
    (docs/'AmazonDark-v7.446-ui-full.state').write_text(f'started {now} {full}\n')
    assert 'still running or incomplete' in run('export','full',ok=False)

print('PASS: v7.446 isolates all three probes, hardens PDP FULL completion, and scale-normalizes the proven transition skeleton')
