...
for token in ['WEB_TIMEOUT label=%@ after=4.0s','WEB_CONTENT_GROWTH','WEB_BOTTOM_HOLD','bottomStable>=4','WEB_DOCUMENT_SWEEP v7.455 root-first']:
...
with tempfile.TemporaryDirectory(prefix='ad-v7447-ui-') as t:
    ...
    now=int(time.time())
    full='AmazonDark-v7.455-ui-full-probe-20260921-000000-000-r1.txt'; view='AmazonDark-v7.455-ui-viewport-probe-20260921-000001-000-r1.txt'
    (docs/full).write_text('FULL\n================ END RUN ================\n'); (docs/view).write_text('VIEWPORT\n================ END RUN ================\n')
    (docs/'AmazonDark-v7.455-ui-full.state').write_text(f'completed {now} {full}\n'); (docs/'AmazonDark-v7.455-ui-viewport.state').write_text(f'completed {now} {view}\n')
    ...
    (docs/'AmazonDark-v7.455-ui-full.state').write_text(f'partial {now} {full}\n')
    before=set(shared.glob('*.tar'))
    assert 'exactly one partial' in run('export','full')
    assert set(shared.glob('*.tar'))==before
    run('export',ok=False)
    (docs/'AmazonDark-v7.455-ui-full.state').write_text(f'started {now} {full}\n')
