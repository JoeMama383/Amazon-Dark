...
with tempfile.TemporaryDirectory(prefix='ad-v7421-parent-') as td:
    ...
    (bin/'dpkg-query').write_text('#!/bin/sh\nprintf "7.443~user-style-ad-ownership-validation-fix"\n'); (bin/'dpkg-query').chmod(0o755)
    receipt=amazon/'AmazonDark-v7.420-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','version':'v7.420-cart-topnav-payment-divider-fix'}))
    ...
    arm=amazon/'AmazonDark-v7.443-probe.arm'; launch=root/'AmazonDark-launch-probe.arm'
    ...
    assert r.returncode==0,(r.stdout,r.stderr)
    assert not arm.exists() and not launch.exists()
print('PASS: v7.437 transition helper accepts the direct-parent v7.420 receipt and arms current-version probe paths')
