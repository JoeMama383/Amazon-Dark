...
def main():
    ...
    assert 'version=7.380~cold-artwork-no-generic-xib base=v7.338' in source
    assert "Version: 7.380~amznkiller-features-optimization-audit\n" in (ROOT / "layout/DEBIAN/control").read_text()
    ...
    print("PASS: v7.380 preserves accepted snapshot cold-artwork policy; generic XIB is passive probe-only")
