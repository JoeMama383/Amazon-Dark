...
def main():
    ...
    assert 'version=7.409~cold-artwork-no-generic-xib base=v7.338' in source
    assert "Version: 7.409~permission-controls-location-rails-fix\n" in (ROOT / "layout/DEBIAN/control").read_text()
    ...
    for name,digest in expected.items():
        assert hashlib.sha256(static_block(name).replace("if(ADLaunchProbeArmed7351()) ADLaunchLog7337(","ADLaunchLog7337(").encode()).hexdigest()==digest,name
    assert 'if(!ADLaunchProbeArmed7351())return;' in source
    print("PASS: v7.388 preserves accepted snapshot cold-artwork policy; generic XIB is passive probe-only")
