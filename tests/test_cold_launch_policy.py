...
def main():
    ...
    assert 'version=7.382~cold-artwork-no-generic-xib base=v7.338' in source
    assert "Version: 7.382~sponsored-carousel-precision\n" in (ROOT / "layout/DEBIAN/control").read_text()
    ...
    print("PASS: v7.382 preserves accepted snapshot cold-artwork policy; generic XIB is passive probe-only")
