...
def main():
    ...
    assert 'version=7.384~cold-artwork-no-generic-xib base=v7.338' in source
    assert "Version: 7.384~sponsored-logos-build-fix\n" in (ROOT / "layout/DEBIAN/control").read_text()
    ...
    print("PASS: v7.384 preserves accepted snapshot cold-artwork policy; generic XIB is passive probe-only")
