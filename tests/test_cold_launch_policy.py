...
def main():
    ...
    assert 'version=7.381~cold-artwork-no-generic-xib base=v7.338' in source
    assert "Version: 7.381~sponsored-slot-collapse-ad-floor\n" in (ROOT / "layout/DEBIAN/control").read_text()
    ...
    print("PASS: v7.381 preserves accepted snapshot cold-artwork policy; generic XIB is passive probe-only")
