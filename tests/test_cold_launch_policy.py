...
def main():
    ...
    assert 'version=7.383~cold-artwork-no-generic-xib base=v7.338' in source
    assert "Version: 7.383~sponsored-selector-rules\n" in (ROOT / "layout/DEBIAN/control").read_text()
    ...
    print("PASS: v7.383 preserves accepted snapshot cold-artwork policy; generic XIB is passive probe-only")
