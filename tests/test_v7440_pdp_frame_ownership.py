...
for tok in ['NSSelectorFromString(@"_frames:")','evaluateJavaScript:inFrame:inContentWorld:completionHandler:','NSClassFromString(@"WKContentWorld")','NSSelectorFromString(@"pageWorld")','ADInjectFrameNode7441','ADForceChildFrameTheme7441']:
    assert tok in S,tok
# Event-driven reinjection catches initial, lazy, and navigated iframe loads with no recurring machinery.
bridge=S[S.index('static NSString *ADFrameOwnerTriggerJS7440'):S.index('static id ADPageWorld7441')]
for tok in ['adFrameOwner7441',"addEventListener('load'","tagName||''","'IFRAME'","DOMContentLoaded","pageshow","document-start"]: assert tok in bridge,tok
...
f=S[S.index('static NSString *ADForcedPDPFrameThemeJS7441'):S.index('static NSString *ADFrameOwnerTriggerJS7440')]
for tok in [
 'ad7441-forced-frame-theme','data-ad7441-frame-owner',':has([data-testid=product-card])',':has(.swiper-wrapper)',
...
for tok in ['#product-image-gallery .a-truncate-cut','img.p13n-product-image',".s-widget-container[class*='widgetId=container-search-results_sponsored']>.s-container-results",'[data-component-type=s-tiles-carousel-component-brand_logo]']:
    assert tok in S,tok
print('PASS: v7.441 owns PDP ad child frames natively and closes current main-document residuals without recurring scans')
