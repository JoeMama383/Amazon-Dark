...
# Core integration retains the existing programs and v7.439 completion while v7.443 adds main-residual + frame-owner triggers with no recurring machinery.
core=S.split('static NSString *ADCoreWebJS7271(void){',1)[1].split('static WKUserScript *ADSharedUserScript7387',1)[0]
assert '@"%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@"' in core  # v7.443 retains the v7.440 main-residual program but retires the frame-owner trigger, for 15 total
assert 'ADProductScrollVideoBorderJS7405()' in core and 'ADPDPCompletionJS7405()' in core and 'ADPDPSafeFrameJS7432()' in core and 'ADPDPCompletionTWBJS7405()' in core and 'ADPDPUICompletionJS7439()' in core and 'ADPDPMainResidualJS7440()' in core
...
assert 'AMAZONDARK v7.443 UNIVERSAL' in INC and 'AmazonDark-v7.443-ui-viewport.arm' in INC
assert "version:'7.443'" in JS
