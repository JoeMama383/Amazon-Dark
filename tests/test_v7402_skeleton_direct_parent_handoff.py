...
with tempfile.TemporaryDirectory(prefix='ad7402-parent-handoff-') as td:
    ...
    (bin/'dpkg-query').write_text('#!/bin/sh\nprintf "7.408~permission-location-switcher-hardening"\n'); (bin/'dpkg-query').chmod(0o755)
    receipt=amazon/'AmazonDark-v7.401-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','version':'v7.401-native-payment-sheets-completion'}))
    ...
    arm=amazon/'AmazonDark-v7.408-probe.arm'; launch=root/'AmazonDark-launch-probe.arm'
...
print('PASS: v7.402 skeleton helper discovers Amazon from exact direct-parent v7.401 receipt with metadata/plutil unavailable')
