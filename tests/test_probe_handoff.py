"""Exercise the v7.450 transition helper boundary with real plain-TAR output.
...
VERSION='7.450~pdp-readonly-full'
...
with tempfile.TemporaryDirectory(prefix='ad-v7447-transition-') as temp:
    ...
    docs=amazon/'Documents'; arm=docs/'AmazonDark-v7.450-probe.arm'; state=docs/'AmazonDark-v7.450-probe-export.state'
    ...
    old=docs/f'AmazonDark-v7.450-skeleton-{(armed-10)*1000}-111-transition.jsonl'; old.write_text('{"event":"SESSION_START","label":"transition"}\nOLD\n')
    ...
    fresh=docs/f'AmazonDark-v7.450-skeleton-{(armed+2)*1000}-333-transition.jsonl'; fresh.write_text('{"event":"SESSION_START","label":"transition"}\nFRESH\n')
    (docs/'AmazonDark-v7.450-probe-status.json').write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','version':'v7.450-pdp-readonly-full','reason':'capture-started'}))
    (mobile/'AmazonDark-v7.450-launch-sb-probe.txt').write_text('switcher evidence\n')
    ...
    text=run('export')
    assert 'Exported exactly one current v7.450 transition capture' in text, text
    ...
    receipt=docs/'AmazonDark-v7.450-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','version':'v7.450-pdp-readonly-full','reason':'arm-missing-or-unreadable'}))
...
print('PASS: v7.450 transition helper exports one current armed session as plain TAR and never bundles historical captures')
