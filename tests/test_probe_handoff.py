...
VERSION='7.387~runtime-css-optimization'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.387~runtime-css-optimization')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.387-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.387-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.387-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.387-runtime-css-optimization'}))
    old=mobile/'AmazonDark-v7.340-skeleton-1-55-both.jsonl';old.write_text('old capture\n')
    sb=mobile/'AmazonDark-v7.387-launch-sb-probe.txt';sb.write_text('snapshot.dark\n')
    ...
    assert not (other/'Documents/AmazonDark-v7.387-probe.arm').exists()
