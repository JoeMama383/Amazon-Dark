...
VERSION='7.358~search-row-shop-style-leaf-fix'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.358~search-row-shop-style-leaf-fix')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.358-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.358-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.358-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.358-search-row-shop-style-leaf-fix'}))
    ...
    assert not (other/'Documents/AmazonDark-v7.358-probe.arm').exists()
    ...
    # During upgrade, the immediately previous v7.357 receipt must locate Amazon before v7.358 runs.
    receipt.unlink()
    previous357=amazon/'Documents/AmazonDark-v7.357-probe-status.json'
    previous357.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon',
        'version':'v7.357-search-footer-shop-style-fix','reason':'arm-missing-or-unreadable'}))
    run('arm','transition',AD_PLUTIL_STYLE='unavailable')
    assert arm.read_text().split()[1]=='transition'
    assert launch_arm.exists()
    run('disarm'); assert not launch_arm.exists(); previous357.unlink()
    ...
    assert not launch_arm.exists()
    print('PASS: v7.357/v7.346/v7.344 upgrade receipts discover Amazon; transition arms v7.358')
