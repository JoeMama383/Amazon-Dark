...
VERSION='7.400~delivery-instructions-completion'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.400~delivery-instructions-completion')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.400-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.400-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.400-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.400-delivery-instructions-completion'}))
    old=mobile/'AmazonDark-v7.340-skeleton-1-55-both.jsonl';old.write_text('old capture\n')
    sb=mobile/'AmazonDark-v7.400-launch-sb-probe.txt';sb.write_text('snapshot.dark\n')
    ...
    assert not (other/'Documents/AmazonDark-v7.400-probe.arm').exists()
    ...
    receipt.unlink()
    previous399=amazon/'Documents/AmazonDark-v7.399-probe-status.json'
    previous399.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon',
        'version':'v7.399-add-address-form-completion','reason':'arm-missing-or-unreadable'}))
    run('arm','transition',AD_PLUTIL_STYLE='unavailable')
    assert arm.read_text().split()[1]=='transition'
    assert launch_arm.exists()
    run('disarm'); assert not launch_arm.exists(); previous399.unlink()
    ...
    assert not launch_arm.exists()
    print('PASS: current v7.400 plus direct-parent v7.399 and older proven upgrade receipts discover Amazon with plutil unavailable')
