...
VERSION='7.361~probed-renderer-paint-fix'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.361~probed-renderer-paint-fix')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.361-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.361-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.361-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.361-probed-renderer-paint-fix'}))
    ...
    assert not (other/'Documents/AmazonDark-v7.361-probe.arm').exists()
    ...
    # During upgrade, v7.360 must locate Amazon before v7.361 has run.
    receipt.unlink()
    ...
    assert not launch_arm.exists()
    print('PASS: v7.360/v7.359/v7.346/v7.344 upgrade receipts discover Amazon; transition arms v7.361')
