...
VERSION='7.352~search-product-ui-repair'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.352~search-product-ui-repair')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.352-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.352-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.352-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.352-search-product-ui-repair'}))
    ...
    assert not (other/'Documents/AmazonDark-v7.352-probe.arm').exists()
    ...
    assert not launch_arm.exists()
    print('PASS: v7.346/v7.344 upgrade receipts discover Amazon; transition arms v7.352')
