...
VERSION='7.360~search-tiles-cart-coupon-fix'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.360~search-tiles-cart-coupon-fix')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.360-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.360-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.360-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.360-search-tiles-cart-coupon-fix'}))
    ...
    assert not (other/'Documents/AmazonDark-v7.360-probe.arm').exists()
    ...
    # During upgrade, the immediately previous v7.359 receipt must locate Amazon before v7.360 runs.
    receipt.unlink()
    ...
    assert not launch_arm.exists()
    print('PASS: v7.359/v7.346/v7.344 upgrade receipts discover Amazon; transition arms v7.360')
