...
VERSION='7.402~payment-first-paint-switcher-fix'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.402~payment-first-paint-switcher-fix')
...
''');dpkg.chmod(0o755)
    ...
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.402-payment-first-paint-switcher-fix'}))
    ...
    assert not launch_arm.exists()
    print('PASS: current v7.402 plus prior proven upgrade receipts discover Amazon with plutil unavailable')
