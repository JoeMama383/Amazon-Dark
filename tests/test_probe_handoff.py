...
VERSION='7.423~cart-message-banner-fix'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.423~cart-message-banner-fix')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.423-probe.arm'
    ...
    sb=mobile/'AmazonDark-v7.423-launch-sb-probe.txt';sb.write_text('snapshot.dark\n')
    ...
    assert not (other/'Documents/AmazonDark-v7.423-probe.arm').exists()
    ...
    assert not launch_arm.exists()
    print('PASS: current v7.423 plus prior proven upgrade receipts discover Amazon with plutil unavailable')
