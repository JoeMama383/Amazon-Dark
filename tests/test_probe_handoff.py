...
VERSION='7.357~search-footer-shop-style-fix'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.357~search-footer-shop-style-fix')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.357-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.357-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.357-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.357-search-footer-shop-style-fix'}))
    ...
    assert not (other/'Documents/AmazonDark-v7.357-probe.arm').exists()
    ...
    # During upgrade, the immediately previous v7.356 receipt must locate Amazon before v7.357 runs.
    receipt.unlink()
    previous356=amazon/'Documents/AmazonDark-v7.356-probe-status.json'
    previous356.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon',
        'version':'v7.356-product-inline-ad-controls-fix','reason':'arm-missing-or-unreadable'}))
    run('arm','transition',AD_PLUTIL_STYLE='unavailable')
    assert arm.read_text().split()[1]=='transition'
    assert launch_arm.exists()
    run('disarm'); assert not launch_arm.exists(); previous356.unlink()
    ...
    assert not launch_arm.exists()
    print('PASS: v7.356/v7.346/v7.344 upgrade receipts discover Amazon; transition arms v7.357')
