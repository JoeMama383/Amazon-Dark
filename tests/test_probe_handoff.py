...
VERSION='7.408~permission-location-switcher-hardening'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.408~permission-location-switcher-hardening')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.408-probe.arm'
    ...
    sb=mobile/'AmazonDark-v7.408-launch-sb-probe.txt';sb.write_text('snapshot.dark\n')
    ...
    assert not (other/'Documents/AmazonDark-v7.408-probe.arm').exists()
    ...
    assert not launch_arm.exists()
    print('PASS: current v7.408 plus prior proven upgrade receipts discover Amazon with plutil unavailable')
