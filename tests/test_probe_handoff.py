...
VERSION='7.379~teal-transition-forensics-claude-audit'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.379~teal-transition-forensics-claude-audit')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.379-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.379-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.379-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.379-teal-transition-forensics-claude-audit'}))
    old=mobile/'AmazonDark-v7.340-skeleton-1-55-both.jsonl';old.write_text('old capture\n')
    sb=mobile/'AmazonDark-v7.379-launch-sb-probe.txt';sb.write_text('snapshot.dark\n')
    ...
    assert not (other/'Documents/AmazonDark-v7.379-probe.arm').exists()
