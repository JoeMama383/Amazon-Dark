...
VERSION='7.406~video-sponsored-footer-restore'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.406~video-sponsored-footer-restore')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.406-probe.arm'
    ...
    sb=mobile/'AmazonDark-v7.406-launch-sb-probe.txt';sb.write_text('snapshot.dark\n')
    ...
    assert not (other/'Documents/AmazonDark-v7.406-probe.arm').exists()
    ...
    assert not launch_arm.exists()
    print('PASS: current v7.406 plus prior proven upgrade receipts discover Amazon with plutil unavailable')
