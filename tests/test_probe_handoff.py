...
VERSION='7.347~cart-strip-owner-forensics'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.347~cart-strip-owner-forensics')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.347-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.347-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.347-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.347-cart-strip-owner-forensics'}))
    ...
    assert arm.read_text().split()[1]=='launch'
    assert not (other/'Documents/AmazonDark-v7.347-probe.arm').exists()
    ...
    with tarfile.open(archive) as t:
        assert t.extractfile('./AMAZON/'+log.name).read()==log.read_bytes()
    # During upgrade, the immediately previous v7.346 receipt must locate Amazon before v7.347 runs.
    receipt.unlink()
    ...
    print('PASS: v7.346/v7.344 upgrade receipts discover Amazon; transition arms v7.347')
