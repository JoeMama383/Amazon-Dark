...
VERSION='7.395~ui-coverage-audit-fix'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.395~ui-coverage-audit-fix')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.395-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.395-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.395-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.395-ui-coverage-audit-fix'}))
    old=mobile/'AmazonDark-v7.340-skeleton-1-55-both.jsonl';old.write_text('old capture\n')
    sb=mobile/'AmazonDark-v7.395-launch-sb-probe.txt';sb.write_text('snapshot.dark\n')
    ...
    assert not (other/'Documents/AmazonDark-v7.395-probe.arm').exists()
    ...
    assert not launch_arm.exists()
    print('PASS: current v7.395 plus v7.393/ v7.392/v7.391/v7.390 and older proven upgrade receipts discover Amazon with plutil unavailable')
