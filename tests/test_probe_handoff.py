...
VERSION='7.370~checkout-script-reinstall-theme'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.370~checkout-script-reinstall-theme')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.370-probe.arm'
    ...
    log=amazon/'Documents/AmazonDark-v7.370-skeleton-1-77-launch.jsonl'
    log.write_text('{"event":"SESSION_START","label":"launch"}\n')
    receipt=amazon/'Documents/AmazonDark-v7.370-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.370-checkout-script-reinstall-theme'}))
    ...
    assert not (other/'Documents/AmazonDark-v7.370-probe.arm').exists()
    ...
    # During upgrade, the immediate v7.369 receipt and older proven receipts must locate Amazon before v7.370 has run.
    receipt.unlink()
    ...
    assert not launch_arm.exists()
    print('PASS: v7.369/v7.366/v7.365/v7.364/v7.361/v7.360/v7.359/v7.346/v7.344 upgrade receipts discover Amazon; transition arms v7.370')
