...
VERSION='7.409~permission-controls-location-rails-fix'
...
import os,sys
v=os.environ.get('AD_INSTALLED','7.409~permission-controls-location-rails-fix')
...
''');dpkg.chmod(0o755)
    ...
    arm=amazon/'Documents/AmazonDark-v7.409-probe.arm'
    ...
    sb=mobile/'AmazonDark-v7.409-launch-sb-probe.txt';sb.write_text('snapshot.dark\n')
    ...
    assert not (other/'Documents/AmazonDark-v7.409-probe.arm').exists()
    ...
    receipt.unlink()
    previous402=amazon/'Documents/AmazonDark-v7.402-probe-status.json'
    previous402.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','reason':'capture-started','version':'v7.402-payment-first-paint-switcher-fix'}))
    ...
    assert not launch_arm.exists()
    print('PASS: current v7.409 plus prior proven upgrade receipts discover Amazon with plutil unavailable')
