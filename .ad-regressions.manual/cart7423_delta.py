...
def strip_cart7423(source):
    ...
    assert hashlib.sha256(source[a:b].encode()).hexdigest()=='fdabfba3502c43e29d6fb5682ea73d2436982eb2e007ed0a3685332880ba998a', 'v7.431 Cart CSS delta changed'
...
def strip_byg7423(source):
    ...
    assert source.count(start)==1
    a=source.index(start); b=source.index(end,a)
    assert hashlib.sha256(source[a:b].encode()).hexdigest()=='93f5533da14c68328e8cde4b73ec7e8eaf17fc8864717397244929b3f9d10a16', 'v7.431 BYG CSS delta changed'
    return source[:a]+source[b:]
