"""Exercise the v7.470 transition helper boundary with real plain-TAR output.
...
VERSION='7.470~pdp-isolated-frame-ownership'
...
with tempfile.TemporaryDirectory(prefix='ad-v7447-transition-') as temp:
    ...
    docs=amazon/'Documents'; arm=docs/'AmazonDark-v7.470-probe.arm'; state=docs/'AmazonDark-v7.470-probe-export.state'
    ...
    old=docs/f'AmazonDark-v7.470-skeleton-{(armed-10)*1000}-111-transition.jsonl'; old.write_text('{"event":"SESSION_START","label":"transition"}\nOLD\n')
    ...
    fresh=docs/f'AmazonDark-v7.470-skeleton-{(armed+2)*1000}-333-transition.jsonl'; fresh.write_text('{"event":"SESSION_START","label":"transition"}\nFRESH\n')
    (docs/'AmazonDark-v7.470-probe-status.json').write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','version':'v7.470-pdp-isolated-frame-ownership','reason':'capture-started'}))
    (mobile/'AmazonDark-v7.470-launch-sb-probe.txt').write_text('switcher evidence\n')
    ...
    text=run('export')
    assert 'Exported exactly one current v7.470 transition capture' in text, text
    ...
    receipt=docs/'AmazonDark-v7.470-probe-status.json'
    receipt.write_text(json.dumps({'event':'PROBE_BOOTSTRAP','bundle':'com.amazon.Amazon','version':'v7.470-pdp-isolated-frame-ownership','reason':'arm-missing-or-unreadable'}))
...
print('PASS: v7.470 transition helper exports one current armed session as plain TAR and never bundles historical captures')
