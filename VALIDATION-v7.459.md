...
- Probe/package identities are regenerated as v7.459 for FULL, VIEWPORT, and TRANSITION.

## Regression / performance checks

- `scripts/lint-logos.sh`: PASS.
- 137 Python regression scripts are accounted for as PASS against the current production source: the 133 unaffected tests in the full parallel batch passed; the two historical invariants that intentionally covered the changed VIEWPORT/flush behavior were updated to preserve their FULL guarantees and then passed; the v7.458 PDP guard and new v7.459 ownership/reliability test both passed separately. Documentation-dependent regressions were rerun after the v7.459 handoff metadata update and passed.
