# AmazonDark v7.336 — v7.330 cold + v6.185 warm/app-switcher on preserved v7.334 theming

- **Theming baseline is v7.334 and is preserved outside the explicit launch/lifecycle region.** No UI selector, color owner, border owner, image/TWB owner, Cart rule, Person rule, Menu rule, Alexa rule, or web/native theming mechanism is intentionally changed.
- Cold launches use the v7.330 process-scoped native SpringBoard scene-overlay architecture that was reported on-device with no white cold-transition flashes.
- Warm launches follow v6.185 behavior: once Amazon is the surviving/settled process, AmazonDark does not manufacture or replay a launch screen.
- App switcher follows v6.185 behavior: **no custom app-switcher/snapshot cover exists at all.** UIKit snapshots the already-themed live app naturally.
- The v7.331-v7.334 `WillResignActive`/background black-logo switcher cover, capture timer, PID switcher-release channel, and warm preflight release behavior are removed.
- AXU/Tez splash hooks only own the splash floor dark if Amazon actually creates those controllers; they are not hidden/shown as a warm-resume mechanism.
- No new MutationObserver, polling loop, RAF loop, scroll listener, recurring hierarchy scan, independent UIWindow, or SpringBoard switcher hook is added.
