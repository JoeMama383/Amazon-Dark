...
static void ADLaunchLog7337(NSString *event,NSString *detail){
    ...
    @try {
        ...
        dispatch_async(queue,^{@autoreleasepool{@try{
            NSString *path=@"/var/mobile/AmazonDark-v7.392-launch-sb-probe.txt";
...
%ctor {
    ...
    if(ADLaunchProbeArmed7351()) ADLaunchLog7337(@"ctor",[NSString stringWithFormat:@"version=7.392~cold-artwork-no-generic-xib base=v7.338 snapshotClass=%d factory=%d wrapper=%d placeholderProbe=%d logo=deferred",
