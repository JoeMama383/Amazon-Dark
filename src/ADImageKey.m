// Bounded image-key analysis and pixel adjustment.
...
static const int    AD_KEY_TOL      = 40;

static const int    AD_KEY_MIN_LUMA = 200;
...
UIImage *ADKeyWhiteBackground(UIImage *img, const char *bgHex){
    ...
    if (W < 24 || H < 24) return nil;
    if (W * H > 2200*2200) return nil;
...
BOOL ADIsDarkGlyph(UIImage *img){
    ...
    for (size_t y = 0; y < H; y += stepY){
        for (size_t x = 0; x < W; x += stepX){
            ...
            total++;
            if (p[3] < 128) { transparent++; continue; }
