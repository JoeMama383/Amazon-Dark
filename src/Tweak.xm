...
static NSString *ADDarkReaderBootstrap(void){
    ...
    gADBootstrap613 = [NSString stringWithFormat:
        @"(function(){try{"
         ...
         // v6.0.66: keep Amazon's native video glyphs, but make the two compact
         // right-edge video controls use one shared shell tint.  Pair the side
         // play/pause control with the nearby mute control by geometry so the large
         // center play overlay is never mistaken for the compact side button.
         // No wrapper/focus-box repair is attempted; Amazon owns that stock behavior.
         "try{var _v64=document.getElementById('advidwrap6064');if(_v64&&_v64.parentNode)_v64.parentNode.removeChild(_v64);var _v62=document.getElementById('advidctl6062');if(_v62&&_v62.parentNode)_v62.parentNode.removeChild(_v62);var _v66=document.getElementById('advidwrap6066');if(!_v66){_v66=document.createElement('style');_v66.id='advidwrap6066';_v66.textContent='[data-ad-videowrap6066],[data-ad-videowrap6066]::before,[data-ad-videowrap6066]::after{background:transparent!important;background-image:none!important;box-shadow:none!important;outline:none!important;border-color:transparent!important;}';(document.head||document.documentElement).appendChild(_v66);}"
           "function adVSem6066(e,lim){try{var s='',p=e,d=0;while(p&&d++<(lim||3)){var c=p.className;c=String(c&&c.baseVal!==undefined?c.baseVal:(c||''));s+=' '+c+' '+String((p.getAttribute&&p.getAttribute('aria-label'))||(p.getAttribute&&p.getAttribute('title'))||'');p=p.parentElement;}return s;}catch(x){return '';}}"
           "function adVCtls6066(host,re,ban){try{var A=host&&host.querySelectorAll?host.querySelectorAll('button,[role=button],[aria-label],[title],svg,i,[class*=play],[class*=pause],[class*=mute],[class*=volume]'):[],O=[];for(var i=0;i<A.length&&i<180;i++){var e=A[i],sem=adVSem6066(e,3);if(ban&&ban.test(sem))continue;if(!re.test(sem))continue;var q=(e.closest&&e.closest('button,[role=button]'))||e,r=q.getBoundingClientRect();if(r.width<20||r.width>96||r.height<20||r.height>96)continue;if(O.indexOf(q)<0)O.push(q);}return O;}catch(x){return [];}}"
           "function adVAlpha6066(c){try{c=String(c||'').replace(/\s+/g,'').toLowerCase();if(!c||c==='transparent'||c==='rgba(0,0,0,0)')return 0;var m=c.match(/^rgba?\((\d+),(\d+),(\d+)(?:,([0-9.]+))?\)$/);return m?(m[4]===undefined?1:parseFloat(m[4])):1;}catch(x){return 0;}}"
           "function adVShell6066(q){try{if(!q)return null;var Q=[q],D=q.querySelectorAll?q.querySelectorAll('*'):[];for(var i=0;i<D.length&&i<24;i++)Q.push(D[i]);var best=null,score=-1;for(var j=0;j<Q.length;j++){var e=Q[j],r=e.getBoundingClientRect();if(r.width<24||r.width>86||r.height<24||r.height>86)continue;if(Math.abs(r.width-r.height)>12)continue;var cs=getComputedStyle(e),a=adVAlpha6066(cs.backgroundColor),rad=parseFloat(cs.borderTopLeftRadius)||0,sc=(rad>=Math.min(r.width,r.height)*.28?5:0)+(a>.02?2:0)-(Math.abs(r.width-r.height)/20)-(r.width/240);if(sc>score){score=sc;best=e;}}return best;}catch(x){return null;}}"
           "function adVPair6066(host,vr){try{var M=adVCtls6066(host,/mute|unmute|volume|sound/i,/play|pause|caption|fullscreen/i),P=adVCtls6066(host,/play|pause/i,/mute|volume|sound|caption|fullscreen/i);if(!M.length||!P.length)return null;var mute=null,mScore=1e15;for(var i=0;i<M.length;i++){var mr=M[i].getBoundingClientRect(),mx=mr.left+mr.width/2,my=mr.top+mr.height/2;if(mx<vr.left-24||mx>vr.right+24||my<vr.top-24||my>vr.bottom+24)continue;var rightPenalty=Math.max(0,(vr.left+vr.width*.58)-mx);var sc=rightPenalty*rightPenalty+Math.abs((vr.right)-mx);if(sc<mScore){mScore=sc;mute=M[i];}}if(!mute)return null;var mr=mute.getBoundingClientRect(),mx=mr.left+mr.width/2,my=mr.top+mr.height/2,play=null,pScore=1e15;for(var j=0;j<P.length;j++){var pr=P[j].getBoundingClientRect(),px=pr.left+pr.width/2,py=pr.top+pr.height/2,dx=Math.abs(px-mx),dy=Math.abs(py-my);if(px<vr.left-24||px>vr.right+24||py<vr.top-24||py>vr.bottom+24)continue;if(dx>110||dy>180)continue;var sc2=dx*dx+dy*dy+(py>my?2500:0);if(sc2<pScore){pScore=sc2;play=P[j];}}return play?{play:play,mute:mute}:null;}catch(x){return null;}}"
           "function adVScrub6066(q){try{if(!q)return; q.setAttribute('data-ad-native615','1');if(window.__AD_STRIP_DR615__)window.__AD_STRIP_DR615__(q);var E=[q],D=q.querySelectorAll?q.querySelectorAll('*'):[];for(var i=0;i<D.length&&i<36;i++)E.push(D[i]);for(var j=0;j<E.length;j++){var e=E[j],st=e.style;if(!st)continue;var f=String(st.getPropertyValue('filter')||'').replace(/\s+/g,'').toLowerCase();if(st.getPropertyPriority('filter')==='important'&&f==='brightness(0)invert(1)')st.removeProperty('filter');if(e.__adGlyph){delete e.__adGlyph;}var by=String(e.__adBy||'');if(/^gfix/.test(by)||by==='videoCtl6062')delete e.__adBy;if(e.removeAttribute){e.removeAttribute('data-ad-videoctl6062');e.removeAttribute('data-ad-videowrap6064');e.removeAttribute('data-ad-videostock6064');e.removeAttribute('data-ad-videoshell6065');e.removeAttribute('data-ad-videostock6065');e.removeAttribute('data-ad-videowrap6065');}}}catch(x){}}"
           "function adVClearHost6066(q,shell){try{if(!q)return;var e=shell&&shell!==q?shell.parentElement:null,n=0;function clr(x){if(!x||x===shell)return;x.setAttribute('data-ad-videowrap6066','1');var st=x.style;if(!st)return;st.setProperty('background-color','transparent','important');st.setProperty('background-image','none','important');st.setProperty('box-shadow','none','important');st.setProperty('outline','none','important');st.setProperty('border-color','transparent','important');}clr(q);while(e&&e!==q&&n++<5){clr(e);e=e.parentElement;}if(e===q)clr(e);}catch(x){}}"
           "function adVMatchShells6066(play,mute){try{var ps=adVShell6066(play),ms=adVShell6066(mute);if(!ps||!ms)return 0;adVClearHost6066(play,ps);adVClearHost6066(mute,ms);var bg='rgba(255,255,255,0.38)';ps.style.setProperty('background-color',bg,'important');ms.style.setProperty('background-color',bg,'important');ps.style.setProperty('background-image','none','important');ms.style.setProperty('background-image','none','important');ps.style.setProperty('border-radius','50%','important');ms.style.setProperty('border-radius','50%','important');ps.style.setProperty('box-shadow','none','important');ms.style.setProperty('box-shadow','none','important');ps.setAttribute('data-ad-videoshell6066','1');ms.setAttribute('data-ad-videoshell6066','1');return 1;}catch(x){return 0;}}"
           "window.__AD_VIDEOSTOCK_ONE6066__=function(v){try{if(!v||String(v.tagName||'').toUpperCase()!=='VIDEO')return 0;var vr=v.getBoundingClientRect();if(vr.width<100||vr.height<70)return 0;var host=v,pd=0;while(host.parentElement&&pd++<4){var hp=host.parentElement,hr=hp.getBoundingClientRect();if(hr.width>=vr.width*.70&&hr.width<=vr.width*1.70&&hr.height>=vr.height*.70&&hr.height<=vr.height*1.75)host=hp;else break;}var pair=adVPair6066(host,vr);if(!pair)return 0;adVScrub6066(pair.play);adVScrub6066(pair.mute);pair.play.setAttribute('data-ad-videostock6066','1');pair.mute.setAttribute('data-ad-videostock6066','1');return adVMatchShells6066(pair.play,pair.mute);}catch(e){return 0;}};"
           "window.__AD_VIDEOSTOCK6066__=function(root){try{var V=[],n=0;function addFrom(b){if(!b)return;try{if(String(b.tagName||'').toUpperCase()==='VIDEO'&&V.indexOf(b)<0)V.push(b);var q=b.querySelectorAll?b.querySelectorAll('video'):[];for(var i=0;i<q.length&&V.length<36;i++)if(V.indexOf(q[i])<0)V.push(q[i]);}catch(x){}}addFrom(root);if(!V.length&&root&&root.nodeType===1){var p=root.parentElement,d=0;while(p&&d++<4&&!V.length){addFrom(p);p=p.parentElement;}}if(!V.length&&root===document)addFrom(document);for(var j=0;j<V.length&&j<36;j++)n+=window.__AD_VIDEOSTOCK_ONE6066__(V[j]);return n;}catch(e){return 0;}};"
           "var vcs66=function(ev){try{var t=ev&&ev.target;if(t&&String(t.tagName||'').toUpperCase()==='VIDEO'){window.__AD_VIDEOSTOCK_ONE6066__(t);setTimeout(function(){window.__AD_VIDEOSTOCK_ONE6066__(t);},90);}}catch(e){}};document.addEventListener('loadedmetadata',vcs66,true);document.addEventListener('loadeddata',vcs66,true);document.addEventListener('canplay',vcs66,true);document.addEventListener('play',vcs66,true);document.addEventListener('playing',vcs66,true);document.addEventListener('pause',vcs66,true);"
           "document.addEventListener('click',function(ev){try{var p=ev&&ev.target,d=0,sem='';while(p&&d++<4){var c=p.className;c=String(c&&c.baseVal!==undefined?c.baseVal:(c||''));sem+=' '+c+' '+String((p.getAttribute&&p.getAttribute('aria-label'))||(p.getAttribute&&p.getAttribute('title'))||'');p=p.parentElement;}if(!/play|pause|mute|unmute|volume|sound/i.test(sem)||/caption|fullscreen/i.test(sem))return;setTimeout(function(){window.__AD_VIDEOSTOCK6066__(document);},0);setTimeout(function(){window.__AD_VIDEOSTOCK6066__(document);},120);}catch(e){}},true);"
           "var vis66=function(){try{window.__AD_VIDEOSTOCK6066__(document);}catch(e){}};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',vis66,{once:true});else vis66();window.addEventListener('pageshow',vis66,{passive:true});}catch(e){}"
