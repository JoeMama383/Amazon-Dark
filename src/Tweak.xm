...
"+(window.__AD_DISC420SEEN__?(' D420SEEN['+window.__AD_DISC420SEEN__+']'):'')"
    ...
    "try{"
      "var key467='';try{var LS=document.querySelectorAll('link[rel=stylesheet]');"
        "var ks=[];for(var li=0;li<LS.length&&li<24;li++){var h=String(LS[li].href||'');"
          "ks.push(h.slice(h.lastIndexOf('/')+1));}"
        "ks.sort();key467='adcss467:'+ks.join('|').slice(0,220);}catch(e){}"
      "var hit467=null;try{if(key467)hit467=localStorage.getItem(key467);}catch(e){}"
      "if(hit467&&hit467.length>200){"
        "var st467=document.createElement('style');st467.className='darkreader darkreader--cached';"
        "st467.textContent=hit467;(document.head||document.documentElement).appendChild(st467);"
        "window.__AD_CACHE467__='hit len='+hit467.length;"
      "}else{"
        "if(!document.querySelector('style.darkreader'))DarkReader.enable(TH467,FX467);"
        "window.__AD_CACHE467__='miss';"
        "try{setTimeout(function(){try{"
          "if(!DarkReader.exportGeneratedCSS)return;"
          "var css=DarkReader.exportGeneratedCSS();"
          "if(css&&css.length>200&&css.length<600000&&key467){"
            "localStorage.setItem(key467,css);"
            "window.__AD_CACHE467__='stored len='+css.length;}"
        "}catch(e){}},2500);}catch(e){}"
      "}"
    "}catch(e){try{if(!document.querySelector('style.darkreader'))DarkReader.enable(TH467,FX467);}catch(e2){}}"
