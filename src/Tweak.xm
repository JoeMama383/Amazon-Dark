...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
         ...
         "if(window.top!==window){try{"
           ...
           // v5.369: product/search ad frames do not get the standalone box/chrome
           // painter. Keep only a contrast-safe text lane; actual White Background
           // Taming remains media-only below.
           "window.__AD_PRODUCTTEXT369__=function(){try{if(!document.body)return 0;var E=document.querySelectorAll('*'),n=0;for(var i=0;i<E.length&&i<900;i++){var e=E[i],tg=e.tagName;if(tg==='IMG'||tg==='PICTURE'||tg==='VIDEO'||tg==='CANVAS'||tg==='SVG'||tg==='svg'||tg==='USE'||tg==='PATH')continue;var own='';try{for(var q=0;q<e.childNodes.length&&q<10;q++){var nd=e.childNodes[q];if(nd.nodeType===3)own+=String(nd.nodeValue||'');}}catch(et){}var txt=own.trim();try{if(!txt&&e.childElementCount<=2&&!e.querySelector('img,svg,picture,video,canvas')){var tx=String(e.textContent||'').replace(/\\s+/g,' ').trim();if(tx.length>0&&tx.length<140)txt=tx;}}catch(ex){}if(!txt)continue;var cs=getComputedStyle(e),fg=AF.p(cs.color);if(!fg||AF.l(fg)>=0.60||AF.s(fg)>=0.12)continue;var p=e,bg=null,d=0;while(p&&d++<5){var pc=AF.p(getComputedStyle(p).backgroundColor);if(pc&&pc.a>0.25){bg=pc;break;}p=p.parentElement;}if(bg&&AF.l(bg)>=0.42)continue;e.style.setProperty('color','#e8e6e3','important');e.style.setProperty('-webkit-text-fill-color','#e8e6e3','important');n++;}window.__AD_PRODUCTTEXT369_N__=n;return n;}catch(e){return 0;}};"
           ...
           "window.__AMZDARK_ADTHEME__=function(){try{"
             ...
             "if(mode==='productad'){var oldp=document.getElementById('adfrstand362');if(oldp&&oldp.parentNode)oldp.parentNode.removeChild(oldp);var oldm=document.getElementById('adfrmin');if(oldm&&oldm.parentNode)oldm.parentNode.removeChild(oldm);if(!document.getElementById('adfrproduct369')){var pst=document.createElement('style');pst.id='adfrproduct369';pst.textContent='html,body{background-color:transparent !important;}';(document.head||document.documentElement).appendChild(pst);}try{var pe=document.querySelectorAll('*');for(var pi=0;pi<pe.length&&pi<900;pi++){var px=pe[pi];if(!px.__adStand362)continue;var pv=String(px.style.getPropertyValue('background-color')||'').replace(/\\s+/g,'').toLowerCase();if(pv==='#181a1b'||pv==='rgb(24,26,27)')px.style.removeProperty('background-color');var bs=['top','right','bottom','left'];for(var pbi=0;pbi<4;pbi++){var bp='border-'+bs[pbi]+'-color',bv=String(px.style.getPropertyValue(bp)||'').replace(/\\s+/g,'').toLowerCase();if(bv==='#3b3c3e'||bv==='rgb(59,60,62)')px.style.removeProperty(bp);}}}catch(pc369){}var ptx=window.__AD_PRODUCTTEXT369__?window.__AD_PRODUCTTEXT369__():0;var pt=window.__AMZDARK_ADTAME__?window.__AMZDARK_ADTAME__():0;var ps=window.__AD_SPON365__?window.__AD_SPON365__():0;window.__AD_STANDALONE__='product-stock369';window.__AD_ADTHEME__='mode=productad stock=1 text='+ptx+' '+String(window.__AD_ADTAME__||('tame='+pt))+' spon='+ps+' pref='+(window.__AD_PRODUCTREF369__?1:0);return ptx+pt+ps;}"
             "if(mode==='standalone'){var old=document.getElementById('adfrmin');if(old&&old.parentNode)old.parentNode.removeChild(old);var prodst=document.getElementById('adfrproduct369');if(prodst&&prodst.parentNode)prodst.parentNode.removeChild(prodst);"
               ...
               "window.__AD_ADTHEME__='mode=standalone '+String(window.__AD_STANDALONE__||('theme='+sn))+' '+String(window.__AD_ADTAME__||('tame='+stn))+' spon='+sp367;return sn+stn+sp367;}"
             "var old2=document.getElementById('adfrstand362');if(old2&&old2.parentNode)old2.parentNode.removeChild(old2);"
           ...
           "try{var _n=0,_iv=setInterval(function(){if(++_n>3){clearInterval(_iv);return;}"
             "try{window.__AMZDARK_ADTHEME__();window.__ADFPOST__();}catch(e){}},2500);}catch(e){}"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         // v5.369: exact remaining npack/sc card family. Do not wait for a painted
         ...
         "function _adExactBorder369(root){try{if(!root||root.nodeType!==1)return 0;function pc369(v){var m=/rgba?\\(([0-9.]+),\\s*([0-9.]+),\\s*([0-9.]+)/.exec(String(v||''));return m?[+m[1],+m[2],+m[3]]:null;}function br369(v){var c=pc369(v);if(!c)return false;var r=c[0],g=c[1],b=c[2],mx=Math.max(r,g,b),mn=Math.min(r,g,b);return r>=55&&r<=125&&g>=48&&g<=115&&b>=38&&b<=105&&(mx-mn)<=32&&r>=g-4&&g>=b-5&&(r-b)>=4;}var A=[];try{if(root.matches&&root.matches('[class*=\"_npack-asin-card_style_asin-cont\"],[class*=\"sc-card-style\"]'))A.push(root);var q=root.querySelectorAll?root.querySelectorAll('[class*=\"_npack-asin-card_style_asin-cont\"],[class*=\"sc-card-style\"]'):[];for(var i=0;i<q.length&&i<720;i++)A.push(q[i]);}catch(eq){}var n=0,seen=[];for(var j=0;j<A.length&&j<740;j++){var e=A[j];if(seen.indexOf(e)>=0)continue;seen.push(e);var lk369=(e.getAttribute&&e.getAttribute('data-ad-border-section')==='exact369'&&e.style.getPropertyValue('border-top-color')==='#3b4043'&&e.style.getPropertyValue('border-right-color')==='#3b4043'&&e.style.getPropertyValue('border-bottom-color')==='#3b4043'&&e.style.getPropertyValue('border-left-color')==='#3b4043'&&e.style.getPropertyPriority('border-top-color')==='important');if(lk369)continue;e.setAttribute('data-ad-cardborder','1');e.setAttribute('data-ad-border-section','exact369');e.style.setProperty('border-top-color','#3b4043','important');e.style.setProperty('border-right-color','#3b4043','important');e.style.setProperty('border-bottom-color','#3b4043','important');e.style.setProperty('border-left-color','#3b4043','important');e.style.setProperty('outline-color','#3b4043','important');try{var cs=getComputedStyle(e),bs=String(cs.boxShadow||'');if(bs&&bs!=='none'){var nb=bs.replace(/rgba?\\([^)]*\\)/g,function(z){return br369(z)?'rgb(59, 64, 67)':z;});if(nb!==bs)e.style.setProperty('box-shadow',nb,'important');}}catch(eb){}e.__adBy='cardborder369';n++;}window.__AD_BORDEREXACT369__=n;return n;}catch(e){return 0;}}"
         "try{window._adProductAds367=_adProductAds367;window._adBorderFast367=_adBorderFast367;window._adExactBorder369=_adExactBorder369;_adProductAds367(document.documentElement);_adBorderFast367(document.documentElement);_adExactBorder369(document.documentElement);setTimeout(function(){try{_adExactBorder369(document.documentElement);}catch(e){}},120);setTimeout(function(){try{_adExactBorder369(document.documentElement);}catch(e){}},650);setTimeout(function(){try{_adExactBorder369(document.documentElement);}catch(e){}},1800);}catch(e){}"
         ...
         "try{var _t=null;new MutationObserver(function(muts){var structural=0;"
           "try{for(var m2=0;m2<muts.length&&m2<40;m2++){var mm2=muts[m2];if(mm2.type==='attributes'){if(mm2.target){_adTameFast362(mm2.target);_adBorderFast367(mm2.target);_adExactBorder369(mm2.target);if(mm2.target.hasAttribute&&mm2.target.hasAttribute('data-ad-sponsored-light363'))_adSponsorPin366(mm2.target);if(mm2.target.hasAttribute&&mm2.target.hasAttribute('data-ad-reviewink367')){mm2.target.style.setProperty('color','#e8e6e3','important');mm2.target.style.setProperty('-webkit-text-fill-color','#e8e6e3','important');}}continue;}structural=1;var ad=mm2.addedNodes;for(var a2=0;a2<ad.length&&a2<20;a2++){_adPin(ad[a2]);_adTameFast362(ad[a2]);_adTextPins363(ad[a2]);if(ad[a2]&&ad[a2].nodeType===1){var tx367=String(ad[a2].textContent||'');if(tx367.length<7000&&/sponsored/i.test(tx367))_adProductAds367(ad[a2]);_adBorderFast367(ad[a2]);_adExactBorder369(ad[a2]);}}try{_adVideoCtlMain362();}catch(e){}}}catch(e){}"
...
"/*V5317FIX*/"
       ...
       "try{var pa31=document.querySelectorAll('[data-ad-productad367]').length,ri31=document.querySelectorAll('[data-ad-reviewink367]').length,bf31=document.querySelectorAll('[data-ad-border-section=\"fast367\"],[data-ad-border-section=\"fast368\"]').length,be31=document.querySelectorAll('[data-ad-border-section=\"exact369\"]').length,pf31=document.querySelectorAll('iframe[data-ad-frame-mode362=\"productad\"]').length;out.push('P31FIX[productAd='+pa31+' productFrame='+pf31+' reviewInk='+ri31+' borderFast='+bf31+' borderExact='+be31+']');}catch(e31){out.push('P31FIX[err '+(e31&&e31.message||e31)+']');}"
       "try{var P32=document.querySelectorAll('iframe[data-ad-frame-mode362=\"productad\"]'),R32=[];for(var i32=0;i32<P32.length&&i32<8;i32++){var f32=P32[i32],r32=f32.getBoundingClientRect();R32.push(String(f32.getAttribute('data-ad-frame-why369')||'-')+'@'+Math.round(r32.left)+','+Math.round(r32.top)+'/'+Math.round(r32.width)+'x'+Math.round(r32.height));}out.push('P32V369[productDoc='+String(window.__AD_PRODUCTDOC369__||0)+' productFrame='+P32.length+(R32.length?' '+R32.join(' ~~ '):'')+']');}catch(e32){out.push('P32V369[err '+(e32&&e32.message||e32)+']');}"
