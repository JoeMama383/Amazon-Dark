...
static NSString *ADFixesLiteral(void){
    ...
    return [NSString stringWithFormat:
            @"{css:'"
             ...
             // v5.392: Search/PDP Heart hydration can expose Amazon's 1px/lazy shim or
             // stock white button shell for a frame before the real Heart painter arrives.
             // Hide only known shim assets and keep the pre-marker shell transparent;
             // __AD_PRODUCTCTRL391RUN__ then paints the same stock square host in place.
             "[class*=puis-heart-position] img[src*=grey-pixel],"
             "[class*=puis-heart-position] img[src*=gray-pixel],"
             "[class*=puis-heart-position] img[src*=transparent-pixel],"
             "[class*=puis-heart-position] img[src*=placeholder],"
             "[class*=puis-heart-position] img[src*=spacer],"
             "[class*=puis-heart-position] img[src*=blank]"
             "{display:none !important;filter:none !important;opacity:0 !important;}"
             "[class*=puis-heart-position] button:not([data-ad-product391]),"
             "[class*=puis-heart-position] [role=button]:not([data-ad-product391])"
             ...
             // v5.392 Home-only cleanup markers. They are applied only by the bounded
             // main-document classifiers above; media/video elements are never marked.
             "[data-ad-standwrap392]{background-color:transparent !important;background-image:none !important;"
             "box-shadow:none !important;outline:none !important;}"
             "[data-ad-standwrapbefore392]::before,[data-ad-standwrapafter392]::after"
             "{background-color:transparent !important;background-image:none !important;box-shadow:none !important;}"
             "[data-ad-homebleed392]{background-color:#181a1b !important;background-blend-mode:normal !important;}"
             "[data-ad-homebleedbefore392]::before,[data-ad-homebleedafter392]::after"
             "{background-color:#181a1b !important;background-image:none !important;box-shadow:none !important;}"
...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         "}catch(e){}"
         // v5.392 Home standalone parent cleanup. The child probe can report black382=0
         // while the MAIN-document wrapper around a 430x130 standalone iframe is still
         // pure black. Clear only neutral-dark, geometry-matched wrapper paint; never
         // enter video/player frames and never touch the iframe contents.
         "function _adStandaloneWrap392(root){try{if(window.__ADFRAME_MODE__||!document.body)return 0;if(document.querySelector('#search,.s-search-results,[data-component-type=\"s-search-result\"],#productTitle,#dp-container,#ppd')){window.__AD_STANDWRAP392__='home=0 product=1';return 0;}var HS=document.querySelectorAll('[class*=\"_hp-mosaic-container_style_widgetContainer\"],[class*=\"_mosaic-container_style_widgetContainer\"],[class*=\"gwm-dashboard-container\"],[class*=\"gwm-window-layout\"],[class*=\"gwm-asin-tile\"]');if(HS.length<2){window.__AD_STANDWRAP392__='home=0';return 0;}function pc(v){var m=/rgba?\\((\\d+),\\s*(\\d+),\\s*(\\d+)(?:,\\s*([0-9.]+))?/i.exec(String(v||''));return m?[+m[1],+m[2],+m[3],m[4]===undefined?1:+m[4]]:null;}function neutralDark(q){if(!q||q[3]<.35)return false;var mx=Math.max(q[0],q[1],q[2]),mn=Math.min(q[0],q[1],q[2]),lum=(.2126*q[0]+.7152*q[1]+.0722*q[2])/255,sat=(mx-mn)/255;return lum<.24&&sat<.12;}function pseudoBad(e,w){try{var z=getComputedStyle(e,w),bi=String(z.backgroundImage||'none');if(bi.indexOf('url(')>=0)return false;return neutralDark(pc(z.backgroundColor));}catch(x){return false;}}var F=document.querySelectorAll('iframe[data-ad-frame-mode362=\"standalone\"]'),n=0,left=0,frames=0,bef=0,aft=0;for(var i=0;i<F.length&&i<100;i++){var f=F[i],r=f.getBoundingClientRect();if(r.width<280||r.width>540||r.height<50||r.height>230)continue;var sig=(String(f.src||'')+' '+String(f.title||'')+' '+String(f.name||'')+' '+String(f.className||'')).toLowerCase();if(/video|player|youtube|vimeo|captcha|challenge|map|payment/.test(sig))continue;frames++;f.style.setProperty('background','transparent','important');f.style.setProperty('background-color','transparent','important');f.style.setProperty('box-shadow','none','important');var p=f.parentElement,d=0;while(p&&d++<5){var pr=p.getBoundingClientRect();if(pr.width<r.width*.78||pr.width>r.width*1.30||pr.height<r.height*.60||pr.height>Math.max(460,r.height*3.7))break;if(p.querySelector&&p.querySelector('video'))break;var cs=getComputedStyle(p),bi=String(cs.backgroundImage||'none');if(bi.indexOf('url(')>=0)break;var bad=neutralDark(pc(cs.backgroundColor)),pb=pseudoBad(p,'::before'),pa=pseudoBad(p,'::after');if(bad||pb||pa){p.setAttribute('data-ad-standwrap392','1');if(bad){p.style.setProperty('background-color','transparent','important');if(bi!=='none')p.style.setProperty('background-image','none','important');p.style.setProperty('box-shadow','none','important');p.style.setProperty('outline','none','important');}if(pb){p.setAttribute('data-ad-standwrapbefore392','1');bef++;}if(pa){p.setAttribute('data-ad-standwrapafter392','1');aft++;}n++;}p=p.parentElement;}}var Q=document.querySelectorAll('[data-ad-standwrap392]');for(var j=0;j<Q.length;j++){var q=Q[j],qcs=getComputedStyle(q),qbi=String(qcs.backgroundImage||'none');if((qbi.indexOf('url(')<0&&neutralDark(pc(qcs.backgroundColor)))||pseudoBad(q,'::before')||pseudoBad(q,'::after'))left++;}window.__AD_STANDWRAP392__='frames='+frames+' fixed='+n+' blackleft='+left+' before='+bef+' after='+aft;return n;}catch(e){window.__AD_STANDWRAP392__='err '+(e&&e.message||e);return 0;}}"
         // v5.392 Home carousel bleed cleanup. The old homeAmbient386 gate only looks at
         // large page ancestors and intentionally skips carousel-ish classes; the supplied
         // video shows a saturated strip behind a row of several product/ad cards. Require
         // >=2 Home card seeds inside one structural row and refuse large-media/video rows,
         // then neutralize only non-URL background/pseudo paint on that row.
         "function _adHomeBleed392(root){try{if(window.__ADFRAME_MODE__||!document.body)return 0;if(document.querySelector('#search,.s-search-results,[data-component-type=\"s-search-result\"],#productTitle,#dp-container,#ppd')){window.__AD_HOMEBLEED392__='home=0 product=1';return 0;}var SEL='[class*=\"_hp-mosaic-container_style_widgetContainer\"],[class*=\"_mosaic-container_style_widgetContainer\"],[class*=\"gwm-asin-tile\"],[class*=\"npack-asin-card\"],[class*=\"canvas-card\"]',S=document.querySelectorAll(SEL);if(S.length<2){window.__AD_HOMEBLEED392__='home=0';return 0;}function pc(v){var m=/rgba?\\((\\d+),\\s*(\\d+),\\s*(\\d+)(?:,\\s*([0-9.]+))?/i.exec(String(v||''));return m?[+m[1],+m[2],+m[3],m[4]===undefined?1:+m[4]]:null;}function hot(q){if(!q||q[3]<.25)return false;var mx=Math.max(q[0],q[1],q[2]),mn=Math.min(q[0],q[1],q[2]);return (mx-mn)/255>.10;}function hotStyle(cs){var bg=pc(cs.backgroundColor),bi=String(cs.backgroundImage||'none');if(bi.indexOf('url(')>=0)return false;return hot(bg)||/gradient\\(/i.test(bi);}var W=innerWidth||430,H=innerHeight||900,A=[],seen=[];for(var si=0;si<S.length&&si<120;si++){var p=S[si].parentElement,u=0;while(p&&u++<6){if(A.indexOf(p)<0)A.push(p);if(p===document.body)break;p=p.parentElement;}}var n=0,left=0,before=0,after=0;for(var i=0;i<A.length&&i<500;i++){var e=A[i];if(!e||seen.indexOf(e)>=0)continue;seen.push(e);var r=e.getBoundingClientRect();if(r.width<W*.78||r.height<70||r.height>H*1.05)continue;var kids=e.querySelectorAll?e.querySelectorAll(SEL):[];if(kids.length<2)continue;var cs=getComputedStyle(e),bi=String(cs.backgroundImage||'none');if(bi.indexOf('url(')>=0)continue;var media=e.querySelectorAll?e.querySelectorAll('video,img,canvas'):[],hero=false;for(var m=0;m<media.length&&m<18;m++){var mr=media[m].getBoundingClientRect();if(mr.width*mr.height>r.width*r.height*.42){hero=true;break;}}if(hero)continue;var b=null,a=null;try{b=getComputedStyle(e,'::before');a=getComputedStyle(e,'::after');}catch(px){}var self=hotStyle(cs),hb=b&&hotStyle(b),ha=a&&hotStyle(a);if(!self&&!hb&&!ha)continue;e.setAttribute('data-ad-homebleed392','1');if(self){e.style.setProperty('background-color','#181a1b','important');if(/gradient\\(/i.test(bi)&&bi.indexOf('url(')<0)e.style.setProperty('background-image','none','important');e.style.setProperty('background-blend-mode','normal','important');}if(hb){e.setAttribute('data-ad-homebleedbefore392','1');before++;}if(ha){e.setAttribute('data-ad-homebleedafter392','1');after++;}n++;}var Q=document.querySelectorAll('[data-ad-homebleed392]');for(var j=0;j<Q.length;j++){var qcs=getComputedStyle(Q[j]),qb=null,qa=null;try{qb=getComputedStyle(Q[j],'::before');qa=getComputedStyle(Q[j],'::after');}catch(px2){}if(hotStyle(qcs)||(qb&&hotStyle(qb))||(qa&&hotStyle(qa)))left++;}window.__AD_HOMEBLEED392__='fixed='+n+' left='+left+' before='+before+' after='+after+' seeds='+S.length;return n;}catch(e){window.__AD_HOMEBLEED392__='err '+(e&&e.message||e);return 0;}}"
         "try{window._adStandaloneWrap392=_adStandaloneWrap392;window._adHomeBleed392=_adHomeBleed392;var ad392run=function(){try{_adStandaloneWrap392(document.documentElement);_adHomeBleed392(document.documentElement);}catch(e){}};ad392run();setTimeout(ad392run,240);setTimeout(ad392run,900);setTimeout(ad392run,2200);if(!window.__ADHOME392INIT__){window.__ADHOME392INIT__=1;var ad392t=0,ad392q=function(){clearTimeout(ad392t);ad392t=setTimeout(ad392run,90);};addEventListener('scroll',ad392q,{passive:true,capture:true});addEventListener('pageshow',ad392q,{passive:true});new MutationObserver(ad392q).observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','style','src']});}}catch(e){}"
         ...
         "window.__AD_PRODUCTCTRL391RUN__=function(){try{if(window.__ADFRAME_MODE__||!document.body)return 0;"
           ...
           "function structural(e){if(!e||e.nodeType!==1)return false;return !/^(IMG|INPUT|SVG|PATH|USE|POLYGON)$/.test(String(e.tagName||'').toUpperCase());}"
           "function badleaf(e){var z=String((e&&e.currentSrc)||e&&e.src||'')+' '+cn(e),t=String(e&&e.tagName||'').toUpperCase(),nw=e&&e.naturalWidth||0,nh=e&&e.naturalHeight||0;return /placehold|spacer|grey[-_]?pixel|gray[-_]?pixel|transparent[-_]?pixel|(?:^|[\\/_-])blank(?:[._-]|$)/i.test(z)||(t==='IMG'&&e.complete&&nw>0&&nh>0&&nw<=2&&nh<=2);}"
           ...
           // v5.392 Heart: keep Amazon's stock coordinates/host, but do not let a
           // placeholder or nested white shell win the first render. Skin a real square
           // host even while the glyph is hydrating; only paint a verified real glyph.
           "function heartGlyph(root){var Q=root.querySelectorAll('[class*=\"lists-treatment-hear\"] .a-icon,[class*=\"lists-treatment-hear\"] img,[class*=\"heart\"] .a-icon,.a-icon,img,svg,i'),best=null,bs=-1;for(var qi=0;qi<Q.length&&qi<80;qi++){var x=Q[qi],xr=rr(x);if(!xr||xr.width<5||xr.height<5||xr.width>36||xr.height>36||badleaf(x))continue;var cs=getComputedStyle(x),bi=String(cs.backgroundImage||'none'),mi=String(cs.maskImage||cs.webkitMaskImage||'none'),tg=String(x.tagName||'').toUpperCase(),sc=0,c=cn(x);if(/lists-treatment-hear|heart/i.test(c))sc+=20;if(bi!=='none'||mi!=='none')sc+=18;if(/^(IMG|I|SVG)$/.test(tg))sc+=5;if(sc>bs){bs=sc;best=x;}}return best;}"
           "function heartHost(root,g){if(g){var h=around(root,g,24,48);if(h)return h;}var Q=root.querySelectorAll('button,[role=button],span,div'),best=null,bd=999;for(var i=0;i<Q.length&&i<80;i++){var e=Q[i];if(!structural(e)||!square(e,24,48))continue;var r=rr(e),d=Math.abs(Math.max(r.width,r.height)-34);if(d<bd){bd=d;best=e;}}if(!best&&square(root,24,48))best=root;return best;}"
           "function clearHeartShell(h,g){if(!h||!h.querySelectorAll)return;var hr=rr(h);if(!hr)return;var cx=hr.left+hr.width/2,cy=hr.top+hr.height/2,Q=h.querySelectorAll('button,[role=button],span,div,img,[class*=\"placehold\"]');for(var i=0;i<Q.length&&i<60;i++){var x=Q[i];if(x===h||x===g)continue;if(badleaf(x)){x.style.setProperty('display','none','important');x.style.setProperty('opacity','0','important');x.style.setProperty('filter','none','important');continue;}if(!structural(x))continue;var r=rr(x);if(!r||r.width<14||r.height<14||r.width>50||r.height>50)continue;if(Math.abs((r.left+r.width/2)-cx)>5||Math.abs((r.top+r.height/2)-cy)>5)continue;x.style.setProperty('background-color','transparent','important');x.style.setProperty('border','0','important');x.style.setProperty('box-shadow','none','important');}}"
           "var H=document.querySelectorAll('[class*=\"puis-heart-position\"]');for(var hi=0;hi<H.length&&hi<180;hi++){var hr=H[hi],hg=heartGlyph(hr),hh=heartHost(hr,hg);if(!hh){miss++;continue;}if(skin(hh,'heart')){clearHeartShell(hh,hg);if(hg){glyphs(hh);paint(hg,hh);}else miss++;heart++;}}"
...
static NSString *ADProbeWebJS(void){
    return
       ...
       "})();}catch(e){}"
       "/*V5314FIX*//*V5315FIX*/"
"/*V5317FIX*//*V5376FIX*//*V5377FIX*//*V5378FIX*//*V5379FIX*//*V5380FIX*//*V5381FIX*//*V5382FIX*//*V5383FIX*//*V5384FIX*//*V5385FIX*//*V5386FIX*//*V5387FIX*//*V5388FIX*//*V5391FIX*/"
       ...
       // P56HOME392: main-document standalone-wrapper + carousel-bleed repair state.
       "try{var W56=document.querySelectorAll('[data-ad-standwrap392]'),B56=document.querySelectorAll('[data-ad-homebleed392]'),Q56=[];for(var i56=0;i56<W56.length&&Q56.length<5;i56++){var r56=W56[i56].getBoundingClientRect();Q56.push('W@'+Math.round(r56.left)+','+Math.round(r56.top)+'/'+Math.round(r56.width)+'x'+Math.round(r56.height));}for(var b56=0;b56<B56.length&&Q56.length<10;b56++){var br56=B56[b56].getBoundingClientRect();Q56.push('B@'+Math.round(br56.left)+','+Math.round(br56.top)+'/'+Math.round(br56.width)+'x'+Math.round(br56.height));}out.push('P56HOME392[wrap='+W56.length+' bleed='+B56.length+' wstate='+String(window.__AD_STANDWRAP392__||'-')+' bstate='+String(window.__AD_HOMEBLEED392__||'-')+(Q56.length?' '+Q56.join(' ~~ '):'')+']');}catch(e56){out.push('P56HOME392[err '+(e56&&e56.message||e56)+']');}"
       // P57HEART392: verify Heart hosts never expose a white shell and real glyphs hydrate.
       "try{var H57=document.querySelectorAll('[data-ad-product391=\"heart\"]'),white57=0,glyph57=0,Q57=[];for(var h57=0;h57<H57.length&&h57<80;h57++){var x57=H57[h57],s57=getComputedStyle(x57),r57=x57.getBoundingClientRect(),m57=/rgba?\\((\\d+),\\s*(\\d+),\\s*(\\d+)/.exec(String(s57.backgroundColor||''));if(m57&&(+m57[1])>220&&(+m57[2])>220&&(+m57[3])>220)white57++;var g57=x57.querySelector('[data-ad-productglyph391=\"1\"]');if(g57)glyph57++;if(Q57.length<6)Q57.push(Math.round(r57.left)+','+Math.round(r57.top)+'/'+Math.round(r57.width)+'x'+Math.round(r57.height)+'|g='+(g57?1:0));}out.push('P57HEART392[n='+H57.length+' glyph='+glyph57+' white='+white57+(Q57.length?' '+Q57.join(' ~~ '):'')+']');}catch(e57){out.push('P57HEART392[err '+(e57&&e57.message||e57)+']');}"
