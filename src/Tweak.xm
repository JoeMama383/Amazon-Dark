    ...
    return
       ...
       "try{var DM=[],VH=window.innerHeight||800;"
         "for(var z=0;z<N;z++){var e5=all[z];var r5=e5.getBoundingClientRect();"
           "if(r5.width<250||r5.height<28)continue;if(r5.bottom<0||r5.top>VH*2)continue;"
           "var s5=getComputedStyle(e5);"
           "var bw=parseFloat(s5.borderTopWidth)||0;var bL=bw>0?lum(s5.borderTopColor):-1;"
           "var ow=parseFloat(s5.outlineWidth)||0;var oL=(ow>0&&s5.outlineStyle!=='none')?lum(s5.outlineColor):-1;"
           "var bs5=String(s5.boxShadow||'');var sL=(bs5&&bs5!=='none')?lum(bs5):-1;"
           "var bgc=s5.backgroundColor;var gL=lum(bgc);"
           "var mx=Math.max(bL,oL,sL,gL);if(mx<0.45)continue;"
           "DM.push({a:r5.width*r5.height,d:Math.round(r5.width)+'x'+Math.round(r5.height)+'|'+e5.tagName+'|'+cls(e5)+'|b'+bw+'='+bL.toFixed(2)+'|o'+ow+'='+oL.toFixed(2)+'|sh='+(sL<0?'-':sL.toFixed(2))+'|bg='+gL.toFixed(2)+' '+bgc});}"
         "DM.sort(function(a,b){return b.a-a.a;});var dd=[];"
         "for(var z2=0;z2<DM.length&&z2<6;z2++)dd.push(DM[z2].d);"
         "out.push('P9DUMP['+(dd.length?dd.join(' ~ '):'none-light')+']');}catch(ed){out.push('P9DUMP[err]');}"
