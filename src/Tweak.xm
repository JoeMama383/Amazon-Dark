...
static int ADWTNativeContext(UIView *v){
    @try {
        ...
        // Shop by category is stock illustrated menu artwork all the way down the
        // section. It is not product photography and must never be tamed/glyphified.
        if (b.shopcat>=0 && y>b.shopcat) return 1;
        if (local==2) return 2;
...
// v5.380: if either canonical More-menu heading exists in the active view tree,
// this is the hamburger/Menu surface. Nothing in this surface receives White
// Background Taming. Small menu artwork is also kept in AlwaysOriginal form.
static BOOL ADIsHamburgerSurface380(UIView *v){
    @try {
        UIWindow *w=v.window; if(!w) return NO;
        ADWTBands362 b=ADWTBandsForWindow362(w);
        return (b.explore>=0 || b.shopcat>=0);
    } @catch(...) {}
    return NO;
}

// v5.380: Previously Watched is horizontally virtualized like Highlights, but its
...
static BOOL ADIsCategoryArtwork379(UIView *v){
    @try {
        ...
        UIWindow *w=v.window; if(!w) return NO;
        if (ADIsHamburgerSurface380(v)) return YES;
        CGRect r=[v convertRect:v.bounds toView:w]; CGFloat y=CGRectGetMidY(r);
        ADWTBands362 b=ADWTBandsForWindow362(w);
        if (b.explore>=0 && y>b.explore && (b.shopcat>b.explore ? y<b.shopcat : y<b.explore+720)) return YES;
        if (b.shopcat>=0 && y>b.shopcat) return YES;
...
static void ADRestoreCategoryArtwork379(UIImageView *iv){
    @try {
        ...
        if(cat379log<18){ cat379log++; ADLog(@"P44MENU380[%s %.0fx%.0f changed=%d mode=%ld]",object_getClassName(iv),iv.bounds.size.width,iv.bounds.size.height,changed?1:0,(long)iv.image.renderingMode); }
    } @catch(...) {}
}
...
static void ADApplyNativeWhiteTameView(UIView *v){
    @try {
        ...
        if (!ov) {
            ...
            if (ctx==2 && ADWTInWatchedCarousel380(v)) { static int watched380log=0; if(watched380log++<12) ADLog(@"P45WATCH380[%s %.0fx%.0f overlay=1]",object_getClassName(v),v.bounds.size.width,v.bounds.size.height); }
        } else {
...
static UIImage *ADGlyphifyForView(UIImage *img, UIView *v){
    @try {
        if (v && ADIsHamburgerSurface380(v)) return nil;
...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{(function(){"
         ...
         "function stockActionFix380(){try{var G=document.querySelectorAll('[class*=lists-framework-unfill],[class*=lists-framework-fill],[class*=lists-framework-action-button]'),hosts=[],leaves=0,skip=0,heart='fallback';function sq(e){if(!e||!e.getBoundingClientRect)return false;var r=e.getBoundingClientRect();return r.width>=24&&r.width<=62&&r.height>=24&&r.height<=62&&Math.abs(r.width-r.height)<=10;}function bad(h){return !!(h&&h.closest&&h.closest('[class*=heart],[class*=wish],[class*=mlt-icon-container],[role=checkbox]'));}function heartChrome(){try{var H=document.querySelectorAll('[class*=heart],[class*=wish]');for(var hi=0;hi<H.length&&hi<180;hi++){var p=H[hi],u=0;while(p&&u++<4){if(sq(p)){var c=getComputedStyle(p),bg=String(c.backgroundColor||''),br=String(c.border||''),sh=String(c.boxShadow||''),rad=String(c.borderRadius||'');if((bg&&bg!=='rgba(0, 0, 0, 0)'&&bg!=='transparent')||sh!=='none'||parseFloat(c.borderWidth||'0')>0){heart='heart';return {bg:bg,br:br,sh:sh,rad:rad};}}p=p.parentElement;}}}catch(e){}return {bg:'rgb(24, 26, 27)',br:'1.5px solid rgb(108, 112, 115)',sh:'inset 0 0 0 1px rgba(255,255,255,.10)',rad:'50%%'};}var hc=heartChrome();for(var i=0;i<G.length&&i<320;i++){var g=G[i],h=null;var cl=String(g.className&&g.className.baseVal!==undefined?g.className.baseVal:(g.className||''));if(/lists-framework-action-button/i.test(cl)&&sq(g))h=g;else{var p=g,u=0,fallback=null;while(p&&u++<6){if(sq(p)){if(!fallback)fallback=p;var tg=String(p.tagName||'').toUpperCase(),pc=String(p.className&&p.className.baseVal!==undefined?p.className.baseVal:(p.className||''));if(tg==='BUTTON'||tg==='A'||String(p.getAttribute&&p.getAttribute('role')||'').toLowerCase()==='button'||/lists-framework-action-button/i.test(pc)){h=p;break;}}p=p.parentElement;}if(!h)h=fallback;}if(!h||bad(h)||hosts.indexOf(h)>=0){skip++;continue;}hosts.push(h);h.removeAttribute('data-ad-stockaction378');h.removeAttribute('data-ad-stockaction379');h.setAttribute('data-ad-stockaction380','1');h.style.setProperty('background-color',hc.bg&&hc.bg!=='rgba(0, 0, 0, 0)'?hc.bg:'#181a1b','important');h.style.setProperty('border-radius',parseFloat(hc.rad)>=12?hc.rad:'50%%','important');if(hc.br&&hc.br!=='0px none rgb(0, 0, 0)')h.style.setProperty('border',hc.br,'important');else h.style.setProperty('border','1.5px solid #6c7073','important');h.style.setProperty('box-shadow',hc.sh&&hc.sh!=='none'?hc.sh:'inset 0 0 0 1px rgba(255,255,255,.10)','important');h.style.setProperty('overflow','hidden','important');h.style.setProperty('box-sizing','border-box','important');var A=h.querySelectorAll('[class*=lists-framework-unfill],[class*=lists-framework-fill],img,i,svg,path');for(var j=0;j<A.length&&j<48;j++){var a=A[j],ar=a.getBoundingClientRect();if(ar.width>50||ar.height>50)continue;var ac=String(a.className&&a.className.baseVal!==undefined?a.className.baseVal:(a.className||''));if(!/lists-framework-(?:unfill|fill)/i.test(ac)&&j>12)continue;a.setAttribute('data-ad-stockglyph380','1');var tg=String(a.tagName||'').toUpperCase();if(tg==='IMG'||tg==='I'||(!['SVG','PATH'].includes(tg)&&String(getComputedStyle(a).backgroundImage||'none')!=='none'))a.setAttribute('data-ad-stockraster380','1');a.style.setProperty('visibility','visible','important');a.style.setProperty('opacity','1','important');a.__adGlyph=1;a.__adBy='stockAction380';leaves++;}}window.__AD_STOCKACTION380__='hosts='+hosts.length+' leaves='+leaves+' skip='+skip+' chrome='+heart;}catch(e){window.__AD_STOCKACTION380__='err '+e;}}"
         ...
         "function cartChrome380(){try{var E=document.querySelectorAll('button,[role=button],a,span,div,input'),n=0,direct=0;function pc(v){var m=/rgba?\\((\\d+),\\s*(\\d+),\\s*(\\d+)/.exec(String(v||''));return m?[+m[1],+m[2],+m[3]]:null;}function light(e){try{var C=[getComputedStyle(e),getComputedStyle(e,'::before'),getComputedStyle(e,'::after')];for(var z=0;z<C.length;z++){var q=pc(C[z].backgroundColor);if(!q)continue;var mx=Math.max(q[0],q[1],q[2]),mn=Math.min(q[0],q[1],q[2]),lum=(.2126*q[0]+.7152*q[1]+.0722*q[2])/255,sat=(mx-mn)/255;if(lum>.76&&sat<.18)return true;}}catch(x){}return false;}function paint(e){if(!e||!e.style)return;e.setAttribute('data-ad-cartchrome380','1');e.style.setProperty('background-color','#181a1b','important');e.style.setProperty('background-image','none','important');e.style.setProperty('border','1px solid #6c7073','important');e.style.setProperty('border-radius','999px','important');e.style.setProperty('box-shadow','none','important');e.style.setProperty('color','#e8e6e3','important');e.style.setProperty('-webkit-text-fill-color','#e8e6e3','important');e.style.setProperty('opacity','1','important');n++;}var D=document.querySelectorAll('button,[role=button],a,span,div');for(var di=0;di<D.length&&di<3000;di++){var de=D[di],dt=String(de.textContent||'').replace(/\\s+/g,' ').trim();if(dt!=='Delete')continue;var dh=(de.closest&&de.closest('button,[role=button],a'))||de,dr=dh.getBoundingClientRect(),best=null,bd=999;for(var ci=0;ci<E.length&&ci<4200;ci++){var x=E[ci];if(!x||x===dh||dh.contains(x)||x.contains&&x.contains(dh))continue;var xr=x.getBoundingClientRect();if(xr.width<38||xr.width>128||xr.height<24||xr.height>58)continue;if(Math.abs((xr.top+xr.height/2)-(dr.top+dr.height/2))>11)continue;var gap=xr.left-dr.right;if(gap<4||gap>95||gap>=bd)continue;var tx=String(x.textContent||x.value||'').replace(/\\s+/g,' ').trim().toLowerCase();if(/delete|save for later|checkout|add to cart|quantity|ask a question/.test(tx))continue;if(!light(x))continue;best=x;bd=gap;}if(best){paint(best);best.setAttribute('data-ad-cartdirect380','1');direct++;}}window.__AD_CART380__='fixed='+n+' direct='+direct;}catch(e){window.__AD_CART380__='err '+e;}}"
         ...
         "}catch(e){}}""badgeFix();dotFix();packFix();stockActionFix380();compareStock380();cartChrome380();shareFix377();sponsorFix376();ratingFix376();""clr();setTimeout(clr,300);setTimeout(clr,1200);setTimeout(clr,2500);""setTimeout(badgeFix,300);setTimeout(badgeFix,1200);setTimeout(badgeFix,2500);""setTimeout(dotFix,120);setTimeout(dotFix,500);setTimeout(dotFix,1400);setTimeout(dotFix,2800);""setTimeout(packFix,120);setTimeout(packFix,500);setTimeout(packFix,1400);setTimeout(packFix,2800);setTimeout(stockActionFix380,20);setTimeout(stockActionFix380,120);setTimeout(stockActionFix380,500);setTimeout(stockActionFix380,1600);setTimeout(compareStock380,20);setTimeout(compareStock380,120);setTimeout(compareStock380,500);setTimeout(compareStock380,1500);setTimeout(cartChrome380,20);setTimeout(cartChrome380,180);setTimeout(cartChrome380,700);setTimeout(cartChrome380,1800);setTimeout(sponsorFix376,20);setTimeout(sponsorFix376,180);setTimeout(sponsorFix376,700);setTimeout(sponsorFix376,1800);setTimeout(ratingFix376,40);setTimeout(ratingFix376,240);setTimeout(ratingFix376,900);setTimeout(ratingFix376,2200);setTimeout(shareFix377,40);setTimeout(shareFix377,180);setTimeout(shareFix377,700);setTimeout(shareFix377,1800);addEventListener('scroll',function(){clearTimeout(window.__bgT);window.__bgT=setTimeout(function(){badgeFix();dotFix();packFix();stockActionFix380();compareStock380();cartChrome380();shareFix377();sponsorFix376();ratingFix376();},80);},{passive:true,capture:true});""addEventListener('scroll',function(){clearTimeout(window.__bxS);window.__bxS=setTimeout(clr,120);},{passive:true,capture:true});"
         "new MutationObserver(function(){clearTimeout(window.__bxT);"
           "window.__bxT=setTimeout(function(){clr();dotFix();packFix();stockActionFix380();compareStock380();cartChrome380();shareFix377();sponsorFix376();ratingFix376();},20);}).observe(document.documentElement,"
       ...
       "})();}catch(e){}"
       "/*V5314FIX*//*V5315FIX*/"
"/*V5317FIX*//*V5376FIX*//*V5377FIX*//*V5378FIX*//*V5379FIX*//*V5380FIX*/"
       ...
       // P43V380: exact Delete-neighbor Cart pill fix + tame regression counters.
       "try{var C43=document.querySelectorAll('[data-ad-cartchrome380]'),D43=document.querySelectorAll('[data-ad-cartdirect380]'),K43=document.querySelectorAll('[data-ad-tame-fast362],[data-ad-tame-bgfast364]');out.push('P43V380[cart='+C43.length+' direct='+D43.length+' tame='+K43.length+' cartState='+String(window.__AD_CART380__||'-')+' parent='+String(window.__AD_PARENTSTRIP379__||'-')+']');}catch(e43){out.push('P43V380[err '+(e43&&e43.message||e43)+']');}"
