...
#define AD_VERSION "v7.157-three-lane-stylesheet-probe"
...
static NSString *ADTWBJS(void){
    ...
    return [NSString stringWithFormat:
        ...
        @"*=checkbox]):not([class*=heart]):not([class*=wishlist]):not([class*=icon]):not([class*=glyph]):not([class*=badge]){filter:brightness(%.3f)!important;}\");return;}var p='';try{p=String(location.pathname||'');}catch(_){}var s=null;if(p==='/autocomplete'||p.indexOf('/autocomplete/')===0){s=put('ad7-search-pane-twb',\"#attach-to-me img.s-image,#attach-to-me img.s-product-image,.s-suggestion-container img.s-image,.s-suggestion-container img.s-product-image{filter:none!important;-webkit-filter:none!important;opacity:%.3f!important;}\");}else if(p==='/s'||p.indexOf('/s/')===0){s=put('ad7-product-feed-twb',\"#search img.s-image,#search img.scx-stt-image,#search img.s-product-image,#search [data-component-type=s-product-image] img,#search img.ufs_tiles_card_widget-sug-image,#search img.nice-cat-card_image,#search img.haul-puis-portrait-img,#search img._c2Itd_image_pQREQ{filter:none!important;-webkit-filter:none!important;opacity:%.3f!important;}#search video.sbv-video-player-ecx,#search video._"
...
static NSString *ADSearchResultsProbePath7139(void){
    @try {
        ...
        if(docs.length)return [docs stringByAppendingPathComponent:@"AmazonDark-v7.157-three-lane-stylesheet-probe.txt"];
    } @catch(...) {}
    return [NSTemporaryDirectory() stringByAppendingPathComponent:@"AmazonDark-v7.157-three-lane-stylesheet-probe.txt"];
}
