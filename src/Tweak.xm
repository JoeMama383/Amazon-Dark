...
static NSString *ADFixesLiteral(void){
    ...
    return [NSString stringWithFormat:
            @"{css:'"
             ...
             "'[class*=copilot-compare]','[class*=copilot-compare] *','ul.a-pagination.a-dots li.a-selected','li.dot-selected-t2'],"
...
static NSString *ADProbeWebJS(void){
    return
       ...
       // P16HEART (v5.345): verify the fallback overlay is alive and anchored
       // to the real Share + pagination geometry; visual confirmation remains primary.
       "try{var h16=document.getElementById('ad-pdp-heart-overlay'),s16=document.querySelector('.ssf-share-trigger'),d16=document.querySelector('ul.a-pagination.a-dots.dots-list-t2,ul.a-pagination.a-dots');var hs16=h16?h16.getBoundingClientRect():null,ss16=s16?s16.getBoundingClientRect():null,ds16=d16?d16.getBoundingClientRect():null;out.push('P16HEART[state='+(window.__AD_HEARTOVERLAY__||'-')+' overlay='+(hs16?(Math.round(hs16.left)+','+Math.round(hs16.top)+','+Math.round(hs16.width)+'x'+Math.round(hs16.height)):'none')+' share='+(ss16?(Math.round(ss16.left)+','+Math.round(ss16.top)):'none')+' dots='+(ds16?(Math.round(ds16.left)+','+Math.round(ds16.top)):'none')+']');}catch(e16){out.push('P16HEART[err '+(e16&&e16.message||e16)+']');}"
       ...
       "try{(function(){"
         ...
         // PDP heart fallback (v5.345). P15 proved the visible black heart is not
         // exposed as its own UIKit/CALayer paint leaf (90 live layers, zero eligible
         // layers in the measured slot), while Share + dots give us stable DOM anchors.
         // Cover only the 32px heart slot with the already-themed page background and
         // redraw a light outline above it. pointer-events:none preserves the real
         // Amazon control underneath, so tap behavior is untouched.
         "function heartOverlayFix(){try{var sh=null,Q=document.querySelectorAll('.ssf-share-trigger');for(var i=0;i<Q.length;i++){var q=Q[i],r=q.getBoundingClientRect();if(r.width>8&&r.height>8&&r.bottom>0&&r.top<(window.innerHeight||900)){sh=q;break;}}var du=document.querySelector('ul.a-pagination.a-dots.dots-list-t2,ul.a-pagination.a-dots'),o=document.getElementById('ad-pdp-heart-overlay');if(!sh||!du){if(o)o.style.display='none';window.__AD_HEARTOVERLAY__='off';return;}var sr=sh.getBoundingClientRect(),dr=du.getBoundingClientRect(),sy=sr.top+sr.height/2,dy=dr.top+dr.height/2;if(Math.abs(sy-dy)>42){if(o)o.style.display='none';window.__AD_HEARTOVERLAY__='misaligned';return;}if(!o){o=document.createElement('div');o.id='ad-pdp-heart-overlay';o.setAttribute('aria-hidden','true');o.innerHTML='<svg viewBox=\"0 0 24 24\" width=\"28\" height=\"28\" aria-hidden=\"true\"><path d=\"M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3.33.74-4.5 2.05A6 6 0 0 0 7.5 3 5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z\" fill=\"none\" stroke=\"#e8e6e3\" stroke-width=\"2.2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"/></svg>';o.style.cssText='position:fixed;width:32px;height:32px;display:flex;align-items:center;justify-content:center;pointer-events:none;z-index:2147483646;background:#181a1b;filter:none!important;mix-blend-mode:normal!important;';(document.body||document.documentElement).appendChild(o);}var L=Math.round(sr.left-43),T=Math.round(sr.top-4),sig=L+','+T;if(o.getAttribute('data-ad-pos')!==sig){o.style.setProperty('left',L+'px','important');o.style.setProperty('top',T+'px','important');o.setAttribute('data-ad-pos',sig);}o.style.display='flex';window.__AD_HEARTOVERLAY__='on@'+sig+' sh='+Math.round(sr.left)+','+Math.round(sr.top)+' dots='+Math.round(dr.left)+','+Math.round(dr.top);}catch(e){window.__AD_HEARTOVERLAY__='err '+e;}}"
         ...
         "}catch(e){}}""badgeFix();dotFix();packFix();heartOverlayFix();""clr();setTimeout(clr,300);setTimeout(clr,1200);setTimeout(clr,2500);""setTimeout(badgeFix,300);setTimeout(badgeFix,1200);setTimeout(badgeFix,2500);""setTimeout(dotFix,120);setTimeout(dotFix,500);setTimeout(dotFix,1400);setTimeout(dotFix,2800);""setTimeout(packFix,120);setTimeout(packFix,500);setTimeout(packFix,1400);setTimeout(packFix,2800);""setTimeout(heartOverlayFix,120);setTimeout(heartOverlayFix,500);setTimeout(heartOverlayFix,1400);setTimeout(heartOverlayFix,2800);""addEventListener('scroll',function(){clearTimeout(window.__bgT);window.__bgT=setTimeout(function(){badgeFix();dotFix();packFix();heartOverlayFix();},120);},{passive:true,capture:true});""addEventListener('scroll',function(){clearTimeout(window.__bxS);window.__bxS=setTimeout(clr,120);},{passive:true,capture:true});"
         "new MutationObserver(function(){clearTimeout(window.__bxT);"
           "window.__bxT=setTimeout(function(){clr();dotFix();packFix();heartOverlayFix();},60);}).observe(document.documentElement,"
