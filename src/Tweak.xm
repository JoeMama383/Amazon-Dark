...
static NSString *ADFixesLiteral(void){
    ...
    return [NSString stringWithFormat:
            @"{css:'"
             ...
             // v5.390: ONE product-control chrome system. Heart, down-arrow,
             // two-card/list action and Compare checkbox all get the exact same
             // 34x34 #181a1b disc and exact 1.5px rgba(255,255,255,.65) bezel.
             // The disc is paint-only; Amazon keeps the real glyph/click target.
             "[data-ad-productctl390=\"1\"]{position:relative !important;background-color:transparent !important;"
             "border:0 !important;box-shadow:none !important;overflow:visible !important;isolation:isolate !important;}"
             "[data-ad-productdisc390=\"1\"]{position:absolute !important;width:34px !important;height:34px !important;"
             "background:#181a1b !important;background-image:none !important;border:1.5px solid rgba(255,255,255,.65) !important;"
             "border-radius:50%% !important;box-shadow:none !important;box-sizing:border-box !important;"
             "pointer-events:none !important;z-index:1 !important;display:block !important;opacity:1 !important;}"
             "[data-ad-productglyph390=\"1\"]{position:relative !important;z-index:2 !important;visibility:visible !important;"
             "opacity:1 !important;background-color:transparent !important;color:#fff !important;"
             "-webkit-text-fill-color:#fff !important;fill:#fff !important;stroke:#fff !important;}"
             "[data-ad-productglyph390=\"1\"][data-ad-productraster390=\"1\"]{filter:brightness(0) invert(1) !important;}"
             "[data-ad-compareorig390=\"1\"]{visibility:hidden !important;opacity:0 !important;pointer-events:none !important;}"
             "[data-ad-compareinput390=\"1\"]{opacity:0 !important;position:absolute !important;inset:0 !important;"
             "width:100%% !important;height:100%% !important;margin:0 !important;z-index:6 !important;cursor:pointer !important;}"
             "[data-ad-checkboxglyph390=\"1\"]{position:absolute !important;left:50%% !important;top:50%% !important;"
             "width:14px !important;height:14px !important;transform:translate(-50%%,-50%%) !important;"
             "border:2px solid #fff !important;border-radius:2px !important;background:transparent !important;"
             "box-shadow:none !important;box-sizing:border-box !important;pointer-events:none !important;z-index:2 !important;}"
             "[data-ad-productctl390=\"1\"][data-ad-productselected390=\"1\"] [data-ad-checkboxglyph390=\"1\"]::after,"
             "[data-ad-compare380=\"1\"] [data-ad-checkboxglyph390=\"1\"]::after,"
             "[data-ad-comparelegacy387=\"1\"] [data-ad-checkboxglyph390=\"1\"]::after"
             "{content:\"\" !important;position:absolute !important;left:3px !important;top:-1px !important;"
             "width:6px !important;height:10px !important;border:solid #fff !important;"
             "border-width:0 2px 2px 0 !important;transform:rotate(45deg) !important;box-sizing:border-box !important;}"
             // Retire the old square/blue Compare painter. Its selection/click state machine
             // remains active; only the visual is handed to the shared v5.390 chrome.
             "[data-ad-compare380]::before,[data-ad-compare380]::after,"
             "[data-ad-comparelegacy387]::before,[data-ad-comparelegacy387]::after"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         "window.__AMZDARK_FIXCONTRAST__=function(){try{"
           ...
           "}catch(e){window.__AD_DARKGLYPH__='err '+e;}"
                      // v5.390: historical ACTION-BUTTON DISC retired. Product-control
           // chrome is now owned exclusively by window.__AD_PRODUCTCTRL390RUN__, so a
           // row-sized lists root can never stretch and a square root cannot double-paint.
           ...
           "try{if(window.__AD_PRODUCTCTRL390RUN__)window.__AD_PRODUCTCTRL390RUN__();}catch(e){}"
           "return n+'/'+bfix+'/'+lfix+'/'+gfix+'/'+bigfix+pr;}catch(e){return -1;}};"
         // v5.390: authoritative PRODUCT CONTROL CHROME. One visual system owns
         // Heart, product down-arrow, two-card/list and Compare checkbox paint.
         // Exact shared geometry: 34x34 black disc + 1.5px rgba(255,255,255,.65).
         // Click targets and Amazon's real Heart/cards/arrow artwork are preserved.
         "window.__AD_PRODUCTCTRL390RUN__=function(){try{if(window.__ADFRAME_MODE__)return 0;var made=0,heart=0,cards=0,arrow=0,check=0,anch=0,miss=0;"
           ...
           "function card(e){return e&&e.closest&&e.closest('[class*=puis-card],[class*=s-result-item],[data-asin],[class*=product-image],[class*=npack-asin-card],[class*=s-card]');}"
           "function struct(e){if(!e||e.nodeType!==1||!e.appendChild)return false;var t=String(e.tagName||'').toUpperCase();return !/^(IMG|I|INPUT|SVG|PATH|USE|POLYGON)$/.test(t);}"
           "function sq(e,lo,hi,d){var r=rr(e);return !!(r&&r.width>=lo&&r.width<=hi&&r.height>=lo&&r.height<=hi&&Math.abs(r.width-r.height)<=d);}"
           "function hostable(e){if(!struct(e))return false;var c=cn(e),cs=getComputedStyle(e),bi=String(cs.backgroundImage||'none'),mi=String(cs.maskImage||cs.webkitMaskImage||'none');if((bi!=='none'||mi!=='none')&&/icon|unfill|fill|chevron|caret|arrow/i.test(c))return false;return true;}"
           "function badimg(e){var z=String((e&&e.currentSrc)||e&&e.src||'')+' '+cn(e);return /placehold|spacer|grey-pixel|gray-pixel|transparent-pixel/i.test(z);}"
           "function paintGlyph(e){if(!e||e.nodeType!==1||badimg(e))return 0;var r=rr(e);if(!r||r.width<3||r.height<3||r.width>34||r.height>34)return 0;var t=String(e.tagName||'').toUpperCase(),cs=getComputedStyle(e),bi=String(cs.backgroundImage||'none'),mi=String(cs.maskImage||cs.webkitMaskImage||'none');e.setAttribute('data-ad-productglyph390','1');e.style.setProperty('visibility','visible','important');e.style.setProperty('opacity','1','important');e.__adGlyph=1;e.__adBy='productctl390';if(t==='IMG'||t==='I'||bi!=='none'||mi!=='none')e.setAttribute('data-ad-productraster390','1');if(t==='SVG'||t==='PATH'||t==='USE'||t==='POLYGON'){e.style.setProperty('color','#fff','important');e.style.setProperty('fill','#fff','important');e.style.setProperty('stroke','#fff','important');}return 1;}"
           "function glyphs(h){if(!h||!h.querySelectorAll)return 0;var n=0,G=h.querySelectorAll('img,i,svg,path,use,polygon,[class*=a-icon],[class*=lists-framework-unfill],[class*=lists-framework-fill],[class*=chevron],[class*=caret],[class*=arrow]');for(var i=0;i<G.length&&i<60;i++)n+=paintGlyph(G[i]);return n;}"
           "function prep(h,type){if(!struct(h))return null;h.setAttribute('data-ad-productctl390','1');h.setAttribute('data-ad-producttype390',type);var cs=getComputedStyle(h);if(cs.position==='static')h.style.setProperty('position','relative','important');h.style.setProperty('background-color','transparent','important');h.style.setProperty('border','0','important');h.style.setProperty('box-shadow','none','important');h.style.setProperty('overflow','visible','important');return h;}"
           "function disc(h,type,x,y){h=prep(h,type);if(!h)return null;var d=null,Q=h.children||[];for(var i=0;i<Q.length;i++)if(Q[i].hasAttribute&&Q[i].hasAttribute('data-ad-productdisc390')){d=Q[i];break;}if(!d){d=document.createElement('span');d.setAttribute('data-ad-productdisc390','1');if(h.firstChild)h.insertBefore(d,h.firstChild);else h.appendChild(d);made++;}d.setAttribute('data-ad-productdisc390','1');d.setAttribute('data-ad-producttype390',type);d.style.setProperty('width','34px','important');d.style.setProperty('height','34px','important');d.style.setProperty('background','#181a1b','important');d.style.setProperty('background-image','none','important');d.style.setProperty('border','1.5px solid rgba(255,255,255,.65)','important');d.style.setProperty('border-radius','50%%','important');d.style.setProperty('box-shadow','none','important');d.style.setProperty('box-sizing','border-box','important');d.style.setProperty('position','absolute','important');d.style.setProperty('pointer-events','none','important');d.style.setProperty('z-index','1','important');if(typeof x==='number'&&typeof y==='number'){d.style.setProperty('left',x+'px','important');d.style.setProperty('top',y+'px','important');}else{d.style.setProperty('left','50%%','important');d.style.setProperty('top','50%%','important');}d.style.setProperty('transform','translate(-50%%,-50%%)','important');return d;}"
           "function centerIn(root,g){var a=rr(root),b=rr(g);if(!a||!b)return null;return [b.left+b.width/2-a.left,b.top+b.height/2-a.top];}"
           "function around(root,g,lo,hi){if(!root||!g)return null;var p=g,best=null,u=0;while(p&&u++<8){if(hostable(p)&&sq(p,lo,hi,8)&&!(p.closest&&p.closest('[class*=mlt-icon-container],[role=checkbox],input[type=checkbox]')))best=p;if(p===root)break;p=p.parentElement;}return best;}"
           "function anchorAt(root,g,type){if(!struct(root)||!g)return null;var cp=centerIn(root,g);if(!cp)return null;var cs=getComputedStyle(root);if(cs.position==='static')root.style.setProperty('position','relative','important');root.setAttribute('data-ad-productroot390',type);var a=null,K=root.children||[];for(var i=0;i<K.length;i++)if(K[i].hasAttribute&&K[i].hasAttribute('data-ad-productanchor390')&&K[i].getAttribute('data-ad-producttype390')===type){a=K[i];break;}if(!a){a=document.createElement('span');a.setAttribute('data-ad-productanchor390','1');a.setAttribute('data-ad-producttype390',type);root.insertBefore(a,root.firstChild||null);made++;}a.style.setProperty('position','absolute','important');a.style.setProperty('left',cp[0]+'px','important');a.style.setProperty('top',cp[1]+'px','important');a.style.setProperty('width','1px','important');a.style.setProperty('height','1px','important');a.style.setProperty('overflow','visible','important');a.style.setProperty('pointer-events','none','important');a.style.setProperty('z-index','1','important');disc(a,type);anch++;return a;}"
           "function actionGlyph(root){if(!root||!root.querySelectorAll)return null;var Q=root.querySelectorAll('[class*=lists-framework-unfill],[class*=lists-framework-fill],img,i,svg'),best=null,bs=-1;for(var i=0;i<Q.length&&i<80;i++){var e=Q[i],r=rr(e);if(!r||r.width<3||r.height<3||r.width>44||r.height>44||badimg(e))continue;var c=cn(e),src=String(e.currentSrc||e.src||''),sc=0;if(/lists-framework-(?:unfill|fill)/i.test(c))sc+=30;if(/01rrzVoKd5L/i.test(src))sc+=30;if(/^(IMG|I|SVG)$/i.test(String(e.tagName||'')))sc+=4;if(sc>bs){bs=sc;best=e;}}return best?{e:best,s:bs}:null;}"
           "function selected(e){try{var q=e&&e.querySelector&&e.querySelector('input[type=checkbox]');if(q)return !!q.checked;var p=e,u=0;while(p&&u++<5){var a=String(p.getAttribute&&p.getAttribute('aria-checked')||p.getAttribute&&p.getAttribute('data-checked')||p.getAttribute&&p.getAttribute('data-selected')||'').toLowerCase(),c=cn(p).toLowerCase();if(a==='true'||/(^|[ _-])(checked|selected|active)([ _-]|$)/.test(c))return true;if(a==='false')return false;p=p.parentElement;}var im=e&&e.querySelector&&e.querySelector('img');var src=im?String(im.currentSrc||im.src||'').toLowerCase():'';if(/checkbox[_-]?(?:on|checked)|checkmark|selected/.test(src))return true;}catch(x){}return false;}"
           "function checkboxHost(e){var p=e&&String(e.tagName||'').toUpperCase()==='INPUT'?e.parentElement:e,best=null,u=0;while(p&&u++<7){if(hostable(p)&&sq(p,20,50,10))best=p;if(card(p)&&best)break;p=p.parentElement;}return best;}"
           // Heart.
           "var H=document.querySelectorAll('[class*=puis-heart-position]');for(var hi=0;hi<H.length&&hi<180;hi++){var hr=H[hi],hg=hr.querySelector&&hr.querySelector('[class*=lists-treatment-hear] .a-icon,.a-icon,svg,img,i');if(!hg){miss++;continue;}var hh=around(hr,hg,24,46);if(hh){disc(hh,'heart');glyphs(hh);paintGlyph(hg);heart++;}else if(anchorAt(hr,hg,'heart')){glyphs(hg);paintGlyph(hg);heart++;}else miss++;}"
           // Two-card/list action, including row-sized roots.
           "var A=document.querySelectorAll('[class*=lists-framework-action-button]');for(var ai=0;ai<A.length&&ai<180;ai++){var ar=A[ai];if(ar.closest&&ar.closest('[class*=heart],[class*=puis-heart-position]'))continue;var apick=actionGlyph(ar);if(!apick){miss++;continue;}var ag=apick.e,ah=around(ar,ag,18,52);if(ah){disc(ah,'cards');glyphs(ah);paintGlyph(ag);cards++;}else if(apick.s>=30&&anchorAt(ar,ag,'cards')){glyphs(ag);paintGlyph(ag);cards++;}else miss++;}"
           // Product-view down arrow only; FAQ/accordion chevrons outside a product card are excluded.
           "var E=document.querySelectorAll('[aria-label*=expand i],[aria-label*=collapse i],[aria-label*=details i],[aria-expanded] [class*=a-icon],[aria-expanded][class*=a-icon],[class*=a-icon-extender],[class*=a-icon-dropdown],[class*=expander] [class*=a-icon],[class*=expand] [class*=a-icon],[class*=collapse] [class*=a-icon],button [class*=chevron],button [class*=caret],button [class*=arrow],[role=button] [class*=chevron],[role=button] [class*=caret],[role=button] [class*=arrow]'),seenE=[];for(var ei=0;ei<E.length&&ei<360;ei++){var eg=E[ei],ec=card(eg);if(!ec)continue;if(eg.closest&&eg.closest('[class*=heart],[class*=puis-heart-position],[class*=lists-framework-action-button],[class*=mlt-icon-container],[role=checkbox],input[type=checkbox]'))continue;var eh=around(ec,eg,20,50);if(!eh||seenE.indexOf(eh)>=0)continue;seenE.push(eh);disc(eh,'arrow');glyphs(eh);paintGlyph(eg);arrow++;}"
           // Compare checkbox. Preserve the real checkbox/input hit target but hide any
           // Amazon white bitmap and paint the common black/chrome disc immediately.
           "var C=document.querySelectorAll('[class*=mlt-icon-container],[role=checkbox],input[type=checkbox],i.a-icon-checkbox,[class*=a-icon-checkbox]'),seenC=[];for(var ci=0;ci<C.length&&ci<420;ci++){var ce=C[ci],ch=checkboxHost(ce);if(!ch||seenC.indexOf(ch)>=0)continue;var near=card(ch),hc=cn(ch);if(!near&&!/mlt-icon-container/i.test(hc))continue;seenC.push(ch);disc(ch,'checkbox');ch.setAttribute('data-ad-productselected390',selected(ch)?'1':'0');var O=ch.querySelectorAll('img,i,svg,path,[class*=a-icon-checkbox],[class*=icon],[class*=image]');for(var oi=0;oi<O.length&&oi<50;oi++){var ox=O[oi];if(ox.hasAttribute&&ox.hasAttribute('data-ad-checkboxglyph390'))continue;var orr=rr(ox);if(!orr||orr.width>50||orr.height>50)continue;ox.setAttribute('data-ad-compareorig390','1');}var inp=ch.querySelector('input[type=checkbox]');if(inp)inp.setAttribute('data-ad-compareinput390','1');var cg=null,K=ch.children||[];for(var ck=0;ck<K.length;ck++)if(K[ck].hasAttribute&&K[ck].hasAttribute('data-ad-checkboxglyph390')){cg=K[ck];break;}if(!cg){cg=document.createElement('span');cg.setAttribute('data-ad-checkboxglyph390','1');ch.appendChild(cg);made++;}check++;}"
           "window.__AD_PRODUCTCTRL390__='made='+made+' heart='+heart+' cards='+cards+' arrow='+arrow+' checkbox='+check+' anchored='+anch+' miss='+miss;return heart+cards+arrow+check;"
         "}catch(e){window.__AD_PRODUCTCTRL390__='err '+e;return -1;}};"
         "window.__AMZDARK_APPLY__=function(){try{"
           ...
           "if(window.__AD_PRODUCTCTRL390RUN__)window.__AD_PRODUCTCTRL390RUN__();"
         "}catch(e){}};"
         ...
         "try{var _t=null;new MutationObserver(function(muts){var structural=0;"
           ...
           "try{_adCollegeFast59();}catch(e){}try{clearTimeout(window.__AD_PC390T__);window.__AD_PC390T__=setTimeout(function(){try{if(window.__AD_PRODUCTCTRL390RUN__)window.__AD_PRODUCTCTRL390RUN__();}catch(e){}},35);}catch(e){}if(structural){clearTimeout(_t);_t=setTimeout(function(){try{if(!document.hidden&&!window.__ADSCROLLING__)window.__AMZDARK_FIXCONTRAST__();}catch(e){}},1900);}})"
         ...
         "try{setTimeout(function(){try{window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__();}catch(e){}},80);setTimeout(function(){try{window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__();}catch(e){}},350);setTimeout(function(){try{window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__();}catch(e){}},1200);}catch(e){}"
         "try{window.addEventListener('scroll',function(){clearTimeout(window.__AD_PC390S__);window.__AD_PC390S__=setTimeout(function(){try{window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__();}catch(e){}},120);},{passive:true,capture:true});}catch(e){}"
...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{(function(){"
         ...
         "}catch(e){}}""homeAmbient386();badgeFix();dotFix();packFix();compareStock380();legacyCompare387();cartChrome382();shareFix382();chevronFix383();sponsorFix376();ratingFix376();""clr();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());setTimeout(function(){clr();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},300);setTimeout(function(){clr();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},1200);setTimeout(function(){clr();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},2500);""setTimeout(homeAmbient386,300);setTimeout(homeAmbient386,1200);setTimeout(homeAmbient386,2500);""setTimeout(badgeFix,300);setTimeout(badgeFix,1200);setTimeout(badgeFix,2500);""setTimeout(dotFix,120);setTimeout(dotFix,500);setTimeout(dotFix,1400);setTimeout(dotFix,2800);""setTimeout(packFix,120);setTimeout(packFix,500);setTimeout(packFix,1400);setTimeout(packFix,2800);setTimeout(function(){compareStock380();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},0);setTimeout(function(){compareStock380();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},120);setTimeout(function(){compareStock380();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},500);setTimeout(function(){compareStock380();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},1500);setTimeout(function(){legacyCompare387();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},0);setTimeout(function(){legacyCompare387();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},120);setTimeout(function(){legacyCompare387();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},500);setTimeout(function(){legacyCompare387();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},1500);setTimeout(cartChrome382,20);setTimeout(cartChrome382,180);setTimeout(cartChrome382,700);setTimeout(cartChrome382,1800);setTimeout(sponsorFix376,20);setTimeout(sponsorFix376,180);setTimeout(sponsorFix376,700);setTimeout(sponsorFix376,1800);setTimeout(ratingFix376,40);setTimeout(ratingFix376,240);setTimeout(ratingFix376,900);setTimeout(ratingFix376,2200);setTimeout(shareFix382,40);setTimeout(shareFix382,180);setTimeout(shareFix382,700);setTimeout(shareFix382,1800);setTimeout(chevronFix383,20);setTimeout(chevronFix383,180);setTimeout(chevronFix383,700);setTimeout(chevronFix383,1800);addEventListener('scroll',function(){clearTimeout(window.__bgT);window.__bgT=setTimeout(function(){homeAmbient386();badgeFix();dotFix();packFix();compareStock380();legacyCompare387();cartChrome382();shareFix382();chevronFix383();sponsorFix376();ratingFix376();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},100);},{passive:true,capture:true});""addEventListener('scroll',function(){clearTimeout(window.__bxS);window.__bxS=setTimeout(function(){clr();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},120);},{passive:true,capture:true});"
         "new MutationObserver(function(){clearTimeout(window.__bxT);"
           "window.__bxT=setTimeout(function(){clr();dotFix();packFix();compareStock380();legacyCompare387();cartChrome382();shareFix382();chevronFix383();sponsorFix376();ratingFix376();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},20);}).observe(document.documentElement,"
         ...
         "try{document.addEventListener('change',function(e){try{if(e&&e.target&&(String(e.target.type||'').toLowerCase()==='checkbox'||(e.target.closest&&e.target.closest('[data-ad-compare380],[class*=mlt-icon-container]'))))setTimeout(function(){compareStock380();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},0);}catch(x){}},true);document.addEventListener('click',function(e){try{if(!e||!e.target||!e.target.closest)return;var h=e.target.closest('[data-ad-compare380]');if(!h){var m=e.target.closest('[class*=mlt-icon-container]');while(m&&m.parentElement&&m.parentElement.closest&&m.parentElement.closest('[class*=mlt-icon-container]'))m=m.parentElement.closest('[class*=mlt-icon-container]');h=m;}if(!h)return;var cur=String(h.getAttribute('data-ad-compare380')||'0')==='1';h.__adManual380=cur?0:1;var card=h.closest&&h.closest('[data-asin]');h.__adManualSig380=card&&card.getAttribute('data-asin')?'a:'+card.getAttribute('data-asin'):'';h.setAttribute('data-ad-compare380',cur?'0':'1');setTimeout(function(){compareStock380();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},0);setTimeout(function(){compareStock380();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},120);setTimeout(function(){compareStock380();(window.__AD_PRODUCTCTRL390RUN__&&window.__AD_PRODUCTCTRL390RUN__());},500);}catch(x){}},true);}catch(e){}"
       "})();}catch(e){}"
       "/*V5314FIX*//*V5315FIX*//*V5390FIX*/"
"/*V5317FIX*//*V5376FIX*//*V5377FIX*//*V5378FIX*//*V5379FIX*//*V5380FIX*//*V5381FIX*//*V5382FIX*//*V5383FIX*//*V5384FIX*//*V5385FIX*//*V5386FIX*//*V5387FIX*//*V5388FIX*/"
       ...
       // P53V360CTRL (v5.389 legacy diagnostic): superseded by P54CTRL390 for final chrome ownership.
       ...
       // P54CTRL390: authoritative shared-chrome probe for all four product controls.
       "try{var D54=document.querySelectorAll('[data-ad-productdisc390]'),H54=document.querySelectorAll('[data-ad-productctl390][data-ad-producttype390=heart]'),A54=document.querySelectorAll('[data-ad-productctl390][data-ad-producttype390=cards]'),E54=document.querySelectorAll('[data-ad-productctl390][data-ad-producttype390=arrow]'),C54=document.querySelectorAll('[data-ad-productctl390][data-ad-producttype390=checkbox]'),badSize54=0,badRing54=0,badBg54=0,dup54=0,Q54=[];"
         "function dark54(v){var m=/rgba?\\((\\d+),\\s*(\\d+),\\s*(\\d+)/i.exec(String(v||''));return m?((.2126*(+m[1])+.7152*(+m[2])+.0722*(+m[3]))/255):1;}"
         "for(var i54=0;i54<D54.length;i54++){var d54=D54[i54],r54=d54.getBoundingClientRect(),s54=getComputedStyle(d54);if(Math.abs(r54.width-34)>1||Math.abs(r54.height-34)>1||Math.abs(r54.width-r54.height)>1)badSize54++;if(Math.abs(parseFloat(s54.borderTopWidth||'0')-1.5)>.15)badRing54++;if(dark54(s54.backgroundColor)>.20)badBg54++;if(Q54.length<12)Q54.push(String(d54.getAttribute('data-ad-producttype390')||'-')+'@'+Math.round(r54.left)+','+Math.round(r54.top)+'/'+Math.round(r54.width)+'x'+Math.round(r54.height)+'|br='+s54.borderTopWidth);var p54=d54.parentElement;if(p54&&p54.querySelectorAll('[data-ad-productdisc390]').length>1)dup54++;}"
         "var roots54=document.querySelectorAll('[class*=lists-framework-action-button]'),bareA54=0;for(var a54=0;a54<roots54.length;a54++){var ra54=roots54[a54],ga54=ra54.querySelector&&ra54.querySelector('[class*=lists-framework-unfill],[class*=lists-framework-fill],img[src*=01rrzVoKd5L]');if(!ga54)continue;var gr54=ga54.getBoundingClientRect();if(gr54.width<3||gr54.height<3||gr54.width>44||gr54.height>44)continue;if(!(ra54.hasAttribute('data-ad-productctl390')||(ra54.querySelector&&ra54.querySelector('[data-ad-productctl390][data-ad-producttype390=cards]'))))bareA54++;}var cmp54=document.querySelectorAll('[data-ad-compare380],[data-ad-comparelegacy387]'),bareC54=0;for(var c54=0;c54<cmp54.length;c54++){var rc54=cmp54[c54],cr54=rc54.getBoundingClientRect();if(cr54.width<3||cr54.height<3)continue;if(!(rc54.hasAttribute('data-ad-productctl390')||(rc54.closest&&rc54.closest('[data-ad-productctl390][data-ad-producttype390=checkbox]'))))bareC54++;}"
         "out.push('P54CTRL390[disc='+D54.length+' heart='+H54.length+' cards='+A54.length+' arrow='+E54.length+' checkbox='+C54.length+' badSize='+badSize54+' badRing='+badRing54+' badBg='+badBg54+' dup='+dup54+' bareCards='+bareA54+' bareCheck='+bareC54+(Q54.length?' '+Q54.join(' ~~ '):'')+' state='+String(window.__AD_PRODUCTCTRL390__||'-')+']');"
       "}catch(e54){out.push('P54CTRL390[err '+(e54&&e54.message||e54)+']');}"
