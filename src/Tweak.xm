...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         // v5.406 LOCK BOUNDARY: HEART / CHECKBOX / DOWN-ARROW FROZEN.
         // TWO-CARDS ONLY. Restore the actual v5.333 lists-framework action contract.
         // v5.405's ancestor-host climb is deleted. v5.333 styled the action-button
         // element itself, only when that exact element was 18..52px and near-square.
         // The visual writes below are copied directly from v5.333. The only addition
         // is a read-only current-checkbox exclusion before those copied writes.
         "window.__AD_CARDS406__=function(){try{if(window.__ADFRAME_MODE__||!document.body)return 0;function incb(e){var p=e,u=0;while(p&&u++<7){if(String(p.getAttribute&&p.getAttribute('data-ad-stock403')||'')==='c')return 1;if(String(p.getAttribute&&p.getAttribute('data-ad-product391')||'')===('check'+'box'))return 1;if(p.getAttribute&&p.getAttribute('aria-checked')!==null)return 1;if(String(p.tagName||'').toUpperCase()==='INPUT'&&String(p.type||'').toLowerCase()===('check'+'box'))return 1;p=p.parentElement;}return 0;}var old=document.getElementById('adcards405');if(old)old.remove();var stale=document.querySelectorAll('[data-ad-cards405],[data-ad-cardsraster405],[data-ad-cardsvector405],[data-ad-cards406]');for(var z=0;z<stale.length&&z<240;z++){stale[z].removeAttribute('data-ad-cards405');stale[z].removeAttribute('data-ad-cardsraster405');stale[z].removeAttribute('data-ad-cardsvector405');stale[z].removeAttribute('data-ad-cards406');}var DB=document.querySelectorAll('[class*=lists-framework-action-button]');var ndisc=0,dskip=0,cbskip=0;for(var db=0;db<DB.length&&db<80;db++){var de=DB[db];if(incb(de)){cbskip++;continue;}var dr=de.getBoundingClientRect();if(dr.width<18||dr.width>52||dr.height<18||dr.height>52){dskip++;continue;}if(Math.abs(dr.width-dr.height)>10){dskip++;continue;}de.setAttribute('data-ad-cards406','1');de.style.setProperty('background-color','#181a1b','important');de.style.setProperty('border-radius','50%%','important');de.style.setProperty('border','1.5px solid rgba(255,255,255,0.65)','important');de.style.setProperty('box-shadow','none','important');de.style.setProperty('box-sizing','border-box','important');ndisc++;}window.__AD_CARDS406_STATE__='on='+ndisc+' skipped='+dskip+' checkbox='+cbskip;return ndisc;}catch(e){window.__AD_CARDS406_STATE__='err '+(e&&e.message||e);return -1;}};"
         "try{if(document&&!document.getElementById('adcards406')){var c406=document.createElement('style');c406.id='adcards406';c406.textContent='[data-ad-cards406] img,[data-ad-cards406] i,[data-ad-cards406] svg,[data-ad-cards406] [class*=lists-framework-unfill],[data-ad-cards406] [class*=lists-framework-fill]{filter:brightness(0) invert(1) !important;background-color:transparent !important;}[data-ad-cards406],[data-ad-cards406] *{color:#ffffff !important;fill:#ffffff !important;}';(document.head||document.documentElement).appendChild(c406);}}catch(e){}"
         "try{window.__AD_PRODUCTCTRL391_PRE406__=window.__AD_PRODUCTCTRL391RUN__;window.__AD_PRODUCTCTRL391RUN__=function(){var r=window.__AD_PRODUCTCTRL391_PRE406__?window.__AD_PRODUCTCTRL391_PRE406__():0;try{window.__AD_CARDS406__();}catch(x){}return r;};window.__AD_CARDS406__();setTimeout(window.__AD_CARDS406__,80);setTimeout(window.__AD_CARDS406__,350);setTimeout(window.__AD_CARDS406__,1100);if(!window.__AD406CARDSCROLL__){window.__AD406CARDSCROLL__=1;var t406=0;addEventListener('scroll',function(){if(t406)return;t406=1;var f406=function(){t406=0;try{window.__AD_CARDS406__();}catch(x){}};if(requestAnimationFrame)requestAnimationFrame(f406);else setTimeout(f406,0);},{passive:true,capture:true});}}catch(e){}"
...
"/*V5317FIX*//*V5376FIX*//*V5377FIX*//*V5378FIX*//*V5379FIX*//*V5380FIX*//*V5381FIX*//*V5382FIX*//*V5383FIX*//*V5384FIX*//*V5385FIX*//*V5386FIX*//*V5387FIX*//*V5388FIX*//*V5391FIX*//*V5393FIX*/"
       ...
       // P75CARDS406: v5.333 direct action-button host; inCheckbox MUST be 0.
       "try{var C75=document.querySelectorAll('[data-ad-cards406]'),badBg75=0,badRing75=0,badShape75=0,badGlyph75=0,inCb75=0,Q75=[];for(var i75=0;i75<C75.length&&i75<80;i75++){var e75=C75[i75],r75=e75.getBoundingClientRect(),s75=getComputedStyle(e75),bg75=String(s75.backgroundColor||'').replace(/\\s+/g,''),bw75=parseFloat(s75.borderTopWidth||'0'),br75=parseFloat(s75.borderTopLeftRadius||'0');if(bg75!=='rgb(24,26,27)'&&bg75!=='rgba(24,26,27,1)')badBg75++;if(Math.abs(bw75-1.5)>.26)badRing75++;if(br75<Math.min(r75.width,r75.height)*.40)badShape75++;var p75=e75,u75=0,cb75=0;while(p75&&u75++<7){if(String(p75.getAttribute&&p75.getAttribute('data-ad-stock403')||'')==='c'||String(p75.getAttribute&&p75.getAttribute('data-ad-product391')||'')===('check'+'box')||(p75.getAttribute&&p75.getAttribute('aria-checked')!==null)){cb75=1;break;}p75=p75.parentElement;}if(cb75)inCb75++;var G75=e75.querySelectorAll('img,i,svg,[class*=lists-framework-unfill],[class*=lists-framework-fill]'),good75=0;for(var j75=0;j75<G75.length;j75++){var gs75=getComputedStyle(G75[j75]);if(String(gs75.filter||'').indexOf('invert')>=0||/255/.test(String(gs75.fill||'')+String(gs75.color||'')))good75=1;}if(!good75)badGlyph75++;if(Q75.length<8)Q75.push(Math.round(r75.width)+'x'+Math.round(r75.height)+'|bg='+bg75+'|bw='+bw75+'|br='+br75+'|g='+G75.length+'|cb='+cb75);}out.push('P75CARDS406[n='+C75.length+' badBg='+badBg75+' badRing='+badRing75+' badShape='+badShape75+' badGlyph='+badGlyph75+' inCheckbox='+inCb75+' state='+String(window.__AD_CARDS406_STATE__||'-')+(Q75.length?' '+Q75.join(' ~~ '):'')+']');}catch(e75){out.push('P75CARDS406[err '+(e75&&e75.message||e75)+']');}"
