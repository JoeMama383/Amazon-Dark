 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         "window.__AMZDARK_FIXCONTRAST__=function(){try{"
           ...
           "function artChk(e9){try{"
             // v5.439 DEVICE-PROVEN CHECKBOX EXCLUSION. The Shopping capture
             // reported the real 23px a-icon-checkbox sprite with by=gfix1's
             // brightness(0) invert(1), which is the white box. Treat every
             // native checkbox/Compare subtree as protected art before any of
             // the broad glyph writers run; stockCheckbox434 is its sole owner.
             "if(e9&&e9.closest&&e9.closest('[class*=a-checkbox],[class*=a-icon-checkbox],input[type=checkbox],[role=checkbox],[class*=copilot-compare],button[aria-label*=ompare],[data-csa-c-content-id*=ompare]'))return true;"
...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{(function(){"
         ...
         // v5.440 DEVICE-CAPTURED STOCK CHECKBOX. Amazon remains the sole owner of
         // geometry, hit testing, state, and the checked blue/checkmark sprite.
         // JavaScript discovers the stock host/art and removes writes left by the
         // retired custom painters, but it never paints or mirrors checked state.
         // Device P85/CB87 proved that Shopping's sprite exists but a broad glyph
         // writer leaves brightness(0) invert(1), producing the white box. It also
         // proved Cart and Shopping use the same grayscale sprite sheet: invert(1)
         // preserves an inverted gray rim, while brightness(0) collapses every
         // unchecked sprite pixel to pure black. Checked state releases the filter
         // synchronously, so Amazon's blue/checkmark sprite cannot flash orange.
         "function stockCheckbox434(){if(window.__ADFRAME_MODE__||!document.body||window.__AD_CHECKBOX434_RUNNING__)return 0;window.__AD_CHECKBOX434_RUNNING__=1;try{"
           ...
           "var prior434=document.getElementById('adcheckbox434');if(prior434&&prior434.getAttribute('data-ad-native-state')!=='440'){if(prior434.parentNode)prior434.parentNode.removeChild(prior434);prior434=null;}"
           "if(!prior434){var s434=document.createElement('style');s434.id='adcheckbox434';s434.setAttribute('data-ad-native-state','440');"
             "s434.textContent='[data-ad-checkbox434-host]{--ad-checkbox434-filter:brightness(0);}'"
               "+'[data-ad-checkbox434-host]:is(input[type=checkbox]:checked,[aria-checked=true],[aria-pressed=true],[aria-selected=true],[data-checked=true],[data-selected=true],[data-state=checked],[data-state=on]),[data-ad-checkbox434-host]:has(input[type=checkbox]:checked,[aria-checked=true],[aria-pressed=true],[aria-selected=true],[data-checked=true],[data-selected=true],[data-state=checked],[data-state=on]),[data-ad-checkbox434-host][class*=checked]:not([class*=unchecked]),[data-ad-checkbox434-host][class*=selected]:not([class*=unselected]),[data-ad-checkbox434-host]:has([class*=checked]:not([class*=unchecked]),[class*=selected]:not([class*=unselected])){--ad-checkbox434-filter:none;}'"
               "+'[data-ad-checkbox434-art]{filter:var(--ad-checkbox434-filter,brightness(0)) !important;}'"
               "+'[data-ad-checkbox434-art]:is(input[type=checkbox]:checked,[aria-checked=true],[aria-pressed=true],[aria-selected=true],[data-checked=true],[data-selected=true],[data-state=checked],[data-state=on]),[data-ad-checkbox434-art][class*=checked]:not([class*=unchecked]),[data-ad-checkbox434-art][class*=selected]:not([class*=unselected]),[data-ad-checkbox434-art][src*=checkbox-on],[data-ad-checkbox434-art][src*=checkbox_checked],[data-ad-checkbox434-art][src*=checkmark],[data-ad-checkbox434-art][src*=selected],[data-ad-checkbox434-art][data-src*=checkbox-on],[data-ad-checkbox434-art][data-src*=checkbox_checked],[data-ad-checkbox434-art][data-src*=checkmark],[data-ad-checkbox434-art][data-src*=selected]{filter:none !important;}'"
           ...
           "function group434(e){if(!e||!scope434(e)||foreign434(e))return null;var exact=(e.matches&&e.matches('[class*=a-icon-checkbox]'))?e:(e.querySelector&&e.querySelector('[class*=a-icon-checkbox]')),ep=e.parentElement,eu=0;while(!exact&&ep&&eu++<4){exact=ep.querySelector&&ep.querySelector('[class*=a-icon-checkbox]');if(exact||ep.matches&&ep.matches(scopeSel434))break;ep=ep.parentElement;}if(exact&&!foreign434(exact)){var ah=exact.closest&&exact.closest('div.a-checkbox,[class~=a-checkbox]');if(ah&&scope434(ah)&&!foreign434(ah))return ah;var ch=exact.closest&&exact.closest('[class*=copilot-compare]');if(ch&&scope434(ch)&&sq434(ch,16,76)&&!foreign434(ch))return ch;var bh=exact.closest&&exact.closest('button[aria-label*=ompare],[role=button][aria-label*=ompare],[data-csa-c-content-id*=ompare],[data-testid*=ompare]');if(bh&&scope434(bh)&&sq434(bh,16,76)&&!foreign434(bh))return bh;}var tg0=String(e.tagName||'').toUpperCase(),p=/^(IMG|I|INPUT|SVG|PATH|USE|POLYGON)$/.test(tg0)?e.parentElement:e,sem=null,best=null,u=0;while(p&&u++<8){if(foreign434(p))return null;var tg=String(p.tagName||'').toUpperCase();if(!sem&&p.matches&&p.matches(semanticSel434)&&sq434(p,16,76))sem=p;if(!best&&!/^(IMG|I|INPUT|SVG|PATH|USE|POLYGON)$/.test(tg)&&sq434(p,16,76))best=p;if(p.matches&&p.matches(scopeSel434))break;p=p.parentElement;}return sem||best||((tg0==='INPUT')?e:(e.parentElement||e));}"
           ...
           "function shell434(h,art){if(!cart434||!h||!art)return;var p=h,u=0;while(p&&u++<4){var tg=String(p.tagName||'').toUpperCase();if(p!==art&&p.contains&&p.contains(art)&&sq434(p,18,76)&&!/^(IMG|I|INPUT|SVG|PATH|USE)$/.test(tg)&&!visual434(p)){p.setAttribute('data-ad-checkbox434-shell','cart');if(shells434.indexOf(p)<0){shells434.push(p);cartShell434++;}}if(p.matches&&p.matches(scopeSel434))break;p=p.parentElement;}}"
...
"/*V5317FIX*//*V5376FIX*//*V5377FIX*//*V5378FIX*//*V5379FIX*//*V5380FIX*//*V5381FIX*//*V5382FIX*//*V5383FIX*//*V5384FIX*//*V5385FIX*//*V5386FIX*//*V5387FIX*//*V5388FIX*//*V5391FIX*//*V5393FIX*/"
           ...
           "try{var V87=/cart/i.test(location.pathname)?'cart':(/\\/s(\\b|\\?|$)/.test(location.pathname+location.search)?'shop':'other');"
             ...
             "for(var j87=0;j87<C87.length&&W87.length<3;j87++){var b87=(C87[j87].closest&&C87[j87].closest('[data-ad-checkbox434-host]'))||C87[j87];if(S87.indexOf(b87)>=0)continue;S87.push(b87);"
               ...
               "W87.push(V87+'|host='+k87(b87)+'|art='+String(g87.tagName)+'.'+k87(g87)"
                 ...
                 "+'|geom='+Math.round(q87.width)+'x'+Math.round(q87.height));}"
       ...
       // P85CHECKBOX434: reports the v5.439 native-state contract. The host/art
       // markers are stable; checked/unchecked comes directly from Amazon's input,
       // ARIA, class, or sprite URL and must agree with the final computed filter.
       "try{var A85=document.querySelectorAll('[data-ad-checkbox434-art]'),S85=document.querySelectorAll('[data-ad-checkbox434-shell=\"cart\"]'),unchecked85=0,checked85=0,badFilter85=0,legacy85=0,foreign85=0,miss85=0,nativePaint85=0,Q85=[];"
       "function on85(a){try{var h=(a.closest&&a.closest('[data-ad-checkbox434-host]'))||a,q=(h.matches&&h.matches('input[type=checkbox]'))?h:h.querySelector('input[type=checkbox]');if(q&&q.checked)return true;var Z=[h],D=h.querySelectorAll?h.querySelectorAll('[role=checkbox],[aria-checked],[aria-pressed],[aria-selected],[data-checked],[data-selected],[data-state]'):[];for(var d85=0;d85<D.length&&d85<32;d85++)Z.push(D[d85]);for(var z85=0;z85<Z.length;z85++){var e85=Z[z85],v85=String(e85.getAttribute('aria-checked')||e85.getAttribute('aria-pressed')||e85.getAttribute('aria-selected')||e85.getAttribute('data-checked')||e85.getAttribute('data-selected')||e85.getAttribute('data-state')||'').toLowerCase(),c85=String(e85.className&&e85.className.baseVal!==undefined?e85.className.baseVal:(e85.className||'')).toLowerCase();if(v85==='true'||v85==='checked'||v85==='on'||(/checked|selected/.test(c85)&&!/unchecked|unselected/.test(c85)))return true;if(v85==='false'||v85==='unchecked'||v85==='off')return false;}if(q)return false;var s85=String(a.currentSrc||a.src||a.getAttribute('data-src')||'').toLowerCase();return /checkbox[_-]?(?:on|checked)|checkmark|selected/.test(s85)&&!/unchecked|unselected/.test(s85);}catch(x85){return false;}}"
       "for(var i85=0;i85<A85.length&&i85<120;i85++){var a85=A85[i85],h85=(a85.closest&&a85.closest('[data-ad-checkbox434-host]'))||a85,n85=(h85.matches&&h85.matches('input[type=checkbox]'))?h85:(h85.querySelector&&h85.querySelector('input[type=checkbox]')),on=on85(a85),s85=getComputedStyle(a85),f85=String(s85.filter||'none');if(on){checked85++;if(f85!=='none')badFilter85++;}else{unchecked85++;if(f85!=='brightness(0)')badFilter85++;}if(n85){var ns85=getComputedStyle(n85),op85=parseFloat(ns85.opacity||'1'),ap85=String(ns85.appearance||ns85.webkitAppearance||'');if(op85>.1&&String(ns85.display||'')!=='none'&&String(ns85.visibility||'')!=='hidden'&&ap85!=='none')nativePaint85++;}if(a85.closest&&a85.closest('[class*=mlt-icon-container],[class*=lists-framework-action-button],[class*=puis-heart-position],[class*=puis-mab-chevron]'))foreign85++;if(Q85.length<10){var r85=a85.getBoundingClientRect(),c85=a85.className,il85='',ip85='';try{il85=a85.style.getPropertyValue('filter')||'-';ip85=a85.style.getPropertyPriority('filter')||'-';}catch(_){}c85=String(c85&&c85.baseVal!==undefined?c85.baseVal:(c85||''));Q85.push((on?'on':'off')+'|'+a85.tagName+'.'+c85.slice(0,28)+'@'+Math.round(r85.width)+'x'+Math.round(r85.height)+'|f='+f85.slice(0,30)+'|by='+String(a85.__adBy||'-')+'|inline='+String(il85).replace(/\\s+/g,'').slice(0,24)+'|pri='+ip85+'|bp='+String(s85.backgroundPosition||'-').replace(/\\s+/g,'').slice(0,18)+'|chk='+(n85&&n85.checked?1:0));}}"
       ...
       "var cartPage85=/proceed to checkout/i.test(String((document.body&&document.body.innerText)||'')),C85=document.querySelectorAll('input[type=checkbox],[role=checkbox],[aria-checked],[class*=a-checkbox],[class*=a-icon-checkbox],[class*=copilot-compare],button[aria-label*=ompare],[role=button][aria-label*=ompare],[data-csa-c-content-id*=ompare],[data-testid*=ompare]');for(var c85i=0;c85i<C85.length&&c85i<700;c85i++){var e85=C85[c85i];if(e85.closest&&e85.closest('[class*=mlt-icon-container],[class*=lists-framework-action-button],[class*=puis-heart-position],[class*=puis-mab-chevron]'))continue;var scoped85=cartPage85||(e85.closest&&e85.closest('[class*=puis-card],[class*=s-result-item],[data-component-type=\"s-search-result\"],[data-asin],[class*=s-product-image],[class*=product-image],[class*=sc-list-item],[class*=sc-item]'));if(!scoped85)continue;var g85=(e85.closest&&e85.closest('[data-ad-checkbox434-host]'))||e85;if(!(g85.hasAttribute&&g85.hasAttribute('data-ad-checkbox434-art'))&&!(g85.querySelector&&g85.querySelector('[data-ad-checkbox434-art]')))miss85++;}"
       "var css85=document.getElementById('adcheckbox434'),ct85=css85?String(css85.textContent||''):'',artBad85=/data-ad-checkbox434-art[^}]*\\{[^}]*(?:background|border|radius|shadow|width|height|position|transform|opacity|visibility|content|display|pointer-events)\\s*:/i.test(ct85)?1:0,shellGeom85=/data-ad-checkbox434-shell[^}]*\\{[^}]*(?:width|height|position|inset|transform|margin|padding|border-radius|display|pointer-events)\\s*:/i.test(ct85)?1:0,timer85=/data-ad-checkbox434-art=.[\"']?(?:checked|unchecked)/i.test(ct85)?1:0;"
       "var pane85=/compare with similar|keep selecting/i.test(String((document.body&&document.body.innerText)||''))?1:0;"
       "out.push('P85CHECKBOX434[art='+A85.length+' unchecked='+unchecked85+' checked='+checked85+' badFilter='+badFilter85+' legacy='+legacy85+' foreign='+foreign85+' miss='+miss85+' nativePaint='+nativePaint85+' cartShell='+S85.length+' artBad='+artBad85+' shellGeom='+shellGeom85+' timer='+timer85+' pane='+pane85+' state='+String(window.__AD_CHECKBOX434_STATE__||'-')+(Q85.length?' '+Q85.join(' ~~ '):'')+']');"
