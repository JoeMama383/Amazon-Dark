...
static NSString *ADFixesLiteral(void){
    ...
    return [NSString stringWithFormat:
            @"{css:'"
             ...
             "'[class*=copilot-compare]','[class*=copilot-compare] *','ul.a-pagination.a-dots li.a-selected','li.dot-selected-t2','[data-ad-cardborder]'],"
...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         "window.__AMZDARK_FIXCONTRAST__=function(){try{"
           ...
           "}catch(e){}"
                      // v5.353: .a-icon-alt is the hidden label, not the sprite painter.
           // Starting from that confirmed semantic leaf, walk a few ancestors and
           // invert the first glyph-sized element that actually paints a sprite,
           // mask, or pseudo-element sprite. This reaches the parent i.a-icon used
           // by Amazon's College carousel without touching product artwork.
           "try{var AIA=document.querySelectorAll('[class*=a-cardui] .a-icon-alt');"
             "__ck('AIA');"
             "for(var aj=0;aj<AIA.length&&aj<160&&((aj&15)||!ovr());aj++){var aa=AIA[aj],ap=aa,ad=0;"
               "while(ap&&ad++<5){var ars=ap.getBoundingClientRect();if(ars.width>=6&&ars.width<=56&&ars.height>=6&&ars.height<=56){"
                 "var ass=getComputedStyle(ap),abf=getComputedStyle(ap,'::before'),aaf=getComputedStyle(ap,'::after');"
                 "var abi=String(ass.backgroundImage||'none'),ami=String(ass.webkitMaskImage||ass.maskImage||'none');"
                 "var pbi=(abf&&String(abf.backgroundImage||'none').indexOf('url(')>=0)||(aaf&&String(aaf.backgroundImage||'none').indexOf('url(')>=0);"
                 "var pmi=(abf&&String(abf.webkitMaskImage||abf.maskImage||'none')!=='none')||(aaf&&String(aaf.webkitMaskImage||aaf.maskImage||'none')!=='none');"
                 "if(abi.indexOf('url(')>=0||ami!=='none'||pbi||pmi){"
                   "if(artChk(ap)){ap=ap.parentElement;continue;}"
                   "ap.style.setProperty('filter','brightness(0) invert(1)','important');"
                   "ap.style.setProperty('opacity','1','important');ap.__adGlyph=1;ap.__adBy='cardchev';"
                   "ap.setAttribute('data-ad-nav-chevron','1');ap.setAttribute('data-ad-nav-label','alt-parent');break;}}"
                 "ap=ap.parentElement;}"
             "}"
           "}catch(e){}"
           // v5.353: some College/Recommended cards paint their rounded border on
           // an anonymous wrapper ABOVE the recognizable card family. Seed from
           // known card roots, then normalize the root plus up to four ancestors.
           "try{var CBF=document.querySelectorAll('[class*=a-cardui],[class*=npack-asin-card],[class*=gwm-asin-tile],[class*=gwm-window-layout],[class*=window-container],[class*=theming-card],[class*=canvas-card],[class*=cXVhZ],[class*=puis-card],[class*=gwm-tile]');"
             "__ck('CBF');var BSD=['Top','Right','Bottom','Left'],BSE=[];"
             "for(var bj=0;bj<CBF.length&&bj<220&&((bj&15)||!ovr());bj++){var bx=CBF[bj],bd=0;while(bx&&bd++<5){if(BSE.indexOf(bx)<0)BSE.push(bx);bx=bx.parentElement;}}"
             "for(var bk=0;bk<BSE.length&&bk<700&&((bk&15)||!ovr());bk++){var be=BSE[bk],br=be.getBoundingClientRect();"
               "if(br.width<120||br.height<50)continue;var bcs=getComputedStyle(be),brad=Math.max(parseFloat(bcs.borderTopLeftRadius)||0,parseFloat(bcs.borderTopRightRadius)||0);if(brad<4)continue;"
               "var bdid=0;for(var bsj=0;bsj<4;bsj++){var bsn=BSD[bsj],bw=parseFloat(bcs['border'+bsn+'Width'])||0;"
                 "var bst=String(bcs['border'+bsn+'Style']||'');if(bw<0.5||bst==='none'||bst==='hidden')continue;"
                 "be.style.setProperty('border-'+bsn.toLowerCase()+'-color','#3b4043','important');bdid++;}"
               "var bow=parseFloat(bcs.outlineWidth)||0;if(bow>=0.5&&String(bcs.outlineStyle||'')!=='none')be.style.setProperty('outline-color','#3b4043','important');"
               "if(bdid){be.__adBy='cardborder';be.setAttribute('data-ad-cardborder','1');}}"
           "}catch(e){}"
...
static NSString *ADProbeWebJS(void){
    return
       ...
       // P21CHEV (v5.351): verify only the explicitly marked College navigation
       // sprite leaves receive the invert filter; read-only telemetry.
       "try{var C21=document.querySelectorAll('[data-ad-nav-chevron=\"1\"]'),Q21=[];for(var i21=0;i21<C21.length&&i21<8;i21++){var c21=C21[i21],r21=c21.getBoundingClientRect(),s21=getComputedStyle(c21);Q21.push(String(c21.getAttribute('data-ad-nav-label')||'-')+'|'+c21.tagName+'.'+String(c21.className||'').replace(/\\s+/g,'.').slice(0,34)+'@'+Math.round(r21.width)+'x'+Math.round(r21.height)+'|bgi='+(s21.backgroundImage&&s21.backgroundImage!=='none'?'Y':'-')+'|f='+String(s21.filter||'none').replace(/\\s+/g,' ').slice(0,60));}out.push('P21CHEV[n='+Q21.length+(Q21.length?' '+Q21.join(' ~~ '):'')+']');}catch(e21){out.push('P21CHEV[err '+(e21&&e21.message||e21)+']');}"
