/*
 * AmazonDark v7.335 — process-scoped launch readiness
 *
 * Architecture:
...
#define AD_VERSION "v7.335-v6185-warm-switcher-v7330-cold"
...
// v7.335: no app-switcher overlay ownership. v6.185 lets UIKit snapshot the
...
// v7.335: warm foreground behavior is intentionally returned to the v6.185 contract.
...
static NSString *ADLaunchAppProbePath7317(void){
    static NSString *p; static dispatch_once_t once; dispatch_once(&once,^{NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];p=[(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:@"AmazonDark-v7.335-launch-app-probe.txt"];}); return p;
}
...
static void ADOwnAmazonSplash7335(UIViewController *vc){
    if(!gP.enabled||!vc||!vc.view)return;
    @try { ADSetViewBackground7226(vc.view,ADOLED(),YES); } @catch(...) {}
}

static BOOL gADLaunchLifecycleLoggerInstalled7335=NO;
static void ADInstallLaunchLifecycleLogger7335(void){
    if(gADLaunchLifecycleLoggerInstalled7335)return;
    gADLaunchLifecycleLoggerInstalled7335=YES;
    @try {
...
// v7.335 deliberately does NOT restore v6.185's SplashBoard cache deletion.
...
// -----------------------------------------------------------------------------
// Launch transition handoff retained in v7.335. The SpringBoard side keeps the
...
%ctor {
    ...
    ADLoadPrefs();
    ADInstallLaunchLifecycleLogger7335();
...
static NSString *ADProbePath7233(NSUInteger run){
    @try {
        ...
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.335-person-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];
    ...
    } @catch(...) { return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.335-person-ui-probe-r%lu.txt",(unsigned long)run]]; }
}
...
static NSString *ADCartProbePath7241(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.335-cart-transition-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.335-cart-transition-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADMenuProbePath7252(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.335-menu-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.335-menu-ui-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADAlexaProbePath7269(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.335-alexa-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.335-alexa-ui-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADPersonSubmenuProbePath7298(NSUInteger run){
    @try {
        ...
        NSString *name=[NSString stringWithFormat:@"AmazonDark-v7.335-person-submenu-hybrid-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];
        return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) {
        return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.335-person-submenu-hybrid-probe-r%lu.txt",(unsigned long)run]];
    }
}
...
static NSString *ADHomeFrameProbePath7265(NSUInteger run){
    @try {
        NSDateFormatter *f=[NSDateFormatter new]; f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"]; f.timeZone=[NSTimeZone localTimeZone]; f.dateFormat=@"yyyyMMdd-HHmmss-SSS";
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.335-home-frame-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject]; return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) { return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.335-home-frame-probe-r%lu.txt",(unsigned long)run]]; }
}
...
static NSString *ADProductScrollProbePath7272(NSUInteger run){
    @try {
        NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.335-product-scroll-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.335-product-scroll-probe-r%lu.txt",(unsigned long)run]];}
}
