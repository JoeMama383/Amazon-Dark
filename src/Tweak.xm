...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
         ...
         "if(window.top!==window){try{"
           ...
           // v5.384: CLEARSTRIP correctly removes authored white/black chrome, but an
           // entirely transparent standalone document can expose WebKit's native dark
           // substrate as a visibly darker rectangle. Finish with the SAME theme floor
           // as the Home page, without touching creative images or product-ad frames.
           "window.__AD_STANDBLEND384__=function(){try{var mode=String(window.__ADFRAME_MODE__||''),ok=(mode==='standalone'&&!window.__AD_PRODUCTREF369__),bg='#181a1b';if(!ok||!document.documentElement){window.__AD_STANDBLEND384_N__=0;window.__AD_STANDBLEND384_LEFT__=0;return 0;}var n=0,left=0,R=[document.documentElement,document.body];for(var i=0;i<R.length;i++){var e=R[i];if(!e)continue;e.style.setProperty('background-color',bg,'important');e.style.setProperty('box-shadow','none','important');n++;}var E=document.querySelectorAll('body>div,body>section,body>article,body>main,body>a'),W=innerWidth||390,H=innerHeight||130;for(var j=0;j<E.length&&j<80;j++){var x=E[j],r=x.getBoundingClientRect();if(r.width<W*.70||r.height<H*.40)continue;var cs=getComputedStyle(x),bi=String(cs.backgroundImage||'none');if(bi.indexOf('url(')>=0)continue;var m=/rgba?\\((\\d+),\\s*(\\d+),\\s*(\\d+)(?:,\\s*([0-9.]+))?/i.exec(String(cs.backgroundColor||''));if(m){var rr=+m[1],gg=+m[2],bb=+m[3],aa=m[4]===undefined?1:+m[4],mx=Math.max(rr,gg,bb),mn=Math.min(rr,gg,bb),lum=(.2126*rr+.7152*gg+.0722*bb)/255,sat=(mx-mn)/255;if(aa>.05&&(lum>.20||sat>.18))continue;}x.setAttribute('data-ad-standblend384','1');x.style.setProperty('background-color',bg,'important');x.style.setProperty('box-shadow','none','important');n++;}for(var k=0;k<R.length;k++){var q=R[k];if(!q)continue;var c=String(getComputedStyle(q).backgroundColor||'');if(c.indexOf('24, 26, 27')<0&&c.indexOf('24,26,27')<0)left++;}window.__AD_STANDBLEND384_N__=n;window.__AD_STANDBLEND384_LEFT__=left;return n;}catch(e){window.__AD_STANDBLEND384_N__=-1;window.__AD_STANDBLEND384_LEFT__=-1;return 0;}};"
           ...
           "try{var __ad377rf=0;function __ad377raf(){if(++__ad377rf>180)return;try{if(window.__AD_ANYSPONINK379__)window.__AD_ANYSPONINK379__();if(window.__AD_STRIPCONF375__&&window.__AD_STRIPTEXT377__)window.__AD_STRIPTEXT377__();if(window.__AD_STRIPCONF375__&&window.__AD_STRIPCHROME379__)window.__AD_STRIPCHROME379__();if(window.__AD_STRIPCONF375__&&window.__AD_STANDDARK382__)window.__AD_STANDDARK382__();if(window.__ADFRAME_MODE__==='standalone'&&window.__AD_STANDBLEND384__)window.__AD_STANDBLEND384__();}catch(e){}requestAnimationFrame(__ad377raf);}requestAnimationFrame(__ad377raf);}catch(e){}"
            ...
            if(h>=40 && h<=760 && ww>=40 && (!w || ww<=w.bounds.size.width*1.25)){
                ...
                for(NSUInteger qi=0; qi<q.count && seen++<90; qi++){
                    ...
                    if([lo containsString:@"returns are easy"] || [lo containsString:@"send an amazon gift card"] ||
                       ...
                       [lo hasPrefix:@"keep shopping for"] || [lo containsString:@"alexa for shopping"]) product=YES;
    ...
    @try {
        ...
        while(p && up++<11){
            if([p isKindOfClass:[UIScrollView class]]){
                UIScrollView *sv=(UIScrollView *)p;
                CGFloat bw=sv.bounds.size.width,bh=sv.bounds.size.height;
                BOOL horiz=(bw>=70&&bh>=45&&bh<=380&&sv.contentSize.width>bw*1.05);
                if(horiz){
                    CFAbsoluteTime now=CFAbsoluteTimeGetCurrent();
                    NSNumber *ct=objc_getAssociatedObject(sv,kADWTCarouselTime384);
                    NSNumber *cc=objc_getAssociatedObject(sv,kADWTCarouselCtx384);
                    if(ct&&cc){CFAbsoluteTime age=now-ct.doubleValue;if((cc.intValue!=0&&age<0.80)||(cc.intValue==0&&age<0.10))return cc.intValue;}
                                ...
                                if(lo.length){
                                    ...
                                    if([lo containsString:@"medical care"]||[lo containsString:@"your amazon highlights"]||
                                       ...
                                       [lo isEqualToString:@"customer service"]) kind=1;
                                            ...
                                            [lo containsString:@"alexa for shopping"]||[lo hasPrefix:@"best deals on"]) kind=2;
                                    if(kind){
...
static int ADWTNativeContext(UIView *v){
    @try {
        ...
        // Nearest named carousel beats stale window-wide RN bands.
...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{(function(){"
         ...
         "}catch(e){}}""badgeFix();dotFix();packFix();stockActionFix384();compareStock380();cartChrome382();shareFix382();chevronFix383();sponsorFix376();ratingFix376();""clr();setTimeout(clr,300);setTimeout(clr,1200);setTimeout(clr,2500);""setTimeout(badgeFix,300);setTimeout(badgeFix,1200);setTimeout(badgeFix,2500);""setTimeout(dotFix,120);setTimeout(dotFix,500);setTimeout(dotFix,1400);setTimeout(dotFix,2800);""setTimeout(packFix,120);setTimeout(packFix,500);setTimeout(packFix,1400);setTimeout(packFix,2800);setTimeout(stockActionFix384,20);setTimeout(stockActionFix384,120);setTimeout(stockActionFix384,500);setTimeout(stockActionFix384,1600);setTimeout(compareStock380,20);setTimeout(compareStock380,120);setTimeout(compareStock380,500);setTimeout(compareStock380,1500);setTimeout(cartChrome382,20);setTimeout(cartChrome382,180);setTimeout(cartChrome382,700);setTimeout(cartChrome382,1800);setTimeout(sponsorFix376,20);setTimeout(sponsorFix376,180);setTimeout(sponsorFix376,700);setTimeout(sponsorFix376,1800);setTimeout(ratingFix376,40);setTimeout(ratingFix376,240);setTimeout(ratingFix376,900);setTimeout(ratingFix376,2200);setTimeout(shareFix382,40);setTimeout(shareFix382,180);setTimeout(shareFix382,700);setTimeout(shareFix382,1800);setTimeout(chevronFix383,20);setTimeout(chevronFix383,180);setTimeout(chevronFix383,700);setTimeout(chevronFix383,1800);addEventListener('scroll',function(){clearTimeout(window.__bgT);window.__bgT=setTimeout(function(){badgeFix();dotFix();packFix();stockActionFix384();compareStock380();cartChrome382();shareFix382();chevronFix383();sponsorFix376();ratingFix376();},80);},{passive:true,capture:true});""addEventListener('scroll',function(){clearTimeout(window.__bxS);window.__bxS=setTimeout(clr,120);},{passive:true,capture:true});"
         "new MutationObserver(function(){clearTimeout(window.__bxT);"
           "window.__bxT=setTimeout(function(){clr();dotFix();packFix();stockActionFix384();compareStock380();cartChrome382();shareFix382();chevronFix383();sponsorFix376();ratingFix376();},20);}).observe(document.documentElement,"
       ...
       "})();}catch(e){}"
       "/*V5314FIX*//*V5315FIX*/"
"/*V5317FIX*//*V5376FIX*//*V5377FIX*//*V5378FIX*//*V5379FIX*//*V5380FIX*//*V5381FIX*//*V5382FIX*//*V5383FIX*//*V5384FIX*/"
