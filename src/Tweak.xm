    ...
    gADBootstrap613 = [NSString stringWithFormat:
        @"(function(){try{"
         ...
         "try{window.__AD_NATIVE_SEL615__='[class*=single-creative-card],[class*=single-video-card],[class*=theming-card],[class*=canvas-card],[class*=ape-placement],[class*=ape-wrapper],[data-cel-widget*=ape],[id*=ape_],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent],[class*=sponsored-products]';"
           ...
           "window.__AD_MARK_NATIVE615__(document);if(!window.__AD_NATIVE_OBS615__&&document.documentElement){window.__AD_NATIVE_OBS615__=1;new MutationObserver(function(ms){try{for(var i=0;i<ms.length&&i<48;i++){var A=ms[i].addedNodes||[];for(var j=0;j<A.length&&j<24;j++)if(A[j]&&A[j].nodeType===1){window.__AD_MARK_NATIVE615__(A[j]);if(window.__AD_VIDEOSTOCK6063__)window.__AD_VIDEOSTOCK6063__(A[j]);}}}catch(e){}}).observe(document.documentElement,{childList:true,subtree:true});}}catch(e){}"
         // v6.0.63: stock play/pause isolation.  Do not paint the video control.
         // Detect Amazon's own compact play/pause button, strip only AmazonDark/Dark
         // Reader ownership that may already have landed, then mark that tiny subtree
         // as Amazon-native so the existing generic contrast/glyph writers stay away.
         ...
         "try{var _old62=document.getElementById('advidctl6062');if(_old62&&_old62.parentNode)_old62.parentNode.removeChild(_old62);"
           "function adVStockScrub6063(root){try{if(!root||root.nodeType!==1)return 0;var E=[root],D=root.querySelectorAll?root.querySelectorAll('*'):[],n=0;for(var di=0;di<D.length&&di<48;di++)E.push(D[di]);for(var i=0;i<E.length;i++){var e=E[i],st=e.style;if(!st)continue;var old=e.hasAttribute&&e.hasAttribute('data-ad-videoctl6062'),by=String(e.__adBy||''),glyph=!!e.__adGlyph;if(old||glyph||/^gfix/.test(by)||by==='videoCtl6062'){var P=['filter','background','background-color','border','border-color','border-radius','outline','outline-color','box-shadow','color','fill','stroke','opacity','visibility','-webkit-text-fill-color','isolation'];for(var p=0;p<P.length;p++)st.removeProperty(P[p]);delete e.__adGlyph;if(/^gfix/.test(by)||by==='videoCtl6062')delete e.__adBy;n++;}else{var f=String(st.getPropertyValue('filter')||'').replace(/\s+/g,'').toLowerCase();if(st.getPropertyPriority('filter')==='important'&&f==='brightness(0)invert(1)'){st.removeProperty('filter');n++;}var bg=String(st.getPropertyValue('background-color')||'').replace(/\s+/g,'').toLowerCase();if(st.getPropertyPriority('background-color')==='important'&&(bg==='#181a1b'||bg==='rgb(24,26,27)'||bg==='rgba(0,0,0,0.72)')){st.removeProperty('background-color');n++;}var bi=String(st.getPropertyValue('background-image')||'').trim().toLowerCase();if(st.getPropertyPriority('background-image')==='important'&&bi==='none'){st.removeProperty('background-image');n++;}var ink=['color','fill','stroke','-webkit-text-fill-color'];for(var k=0;k<ink.length;k++){var iv=String(st.getPropertyValue(ink[k])||'').replace(/\s+/g,'').toLowerCase();if(st.getPropertyPriority(ink[k])==='important'&&(iv==='#fff'||iv==='#ffffff'||iv==='#e8e6e3'||iv==='rgb(255,255,255)'||iv==='rgb(232,230,227)')){st.removeProperty(ink[k]);n++;}}}if(e.removeAttribute)e.removeAttribute('data-ad-videoctl6062');}if(window.__AD_STRIP_DR615__)window.__AD_STRIP_DR615__(root);return n;}catch(x){return 0;}}"
           "window.__AD_VIDEOSTOCK_ONE6063__=function(v){try{if(!v||String(v.tagName||'').toUpperCase()!=='VIDEO')return 0;var vr=v.getBoundingClientRect();if(vr.width<100||vr.height<70)return 0;var host=v,pd=0;while(host.parentElement&&pd++<4){var hp=host.parentElement,hr=hp.getBoundingClientRect();if(hr.width>=vr.width*.70&&hr.width<=vr.width*1.70&&hr.height>=vr.height*.70&&hr.height<=vr.height*1.75)host=hp;else break;}var A=host.querySelectorAll?host.querySelectorAll('button,[role=button],[aria-label],[title],svg,i,[class*=play],[class*=pause]'):[],n=0;for(var i=0;i<A.length&&i<180;i++){var e=A[i],r=e.getBoundingClientRect();if(r.width<10||r.width>84||r.height<10||r.height>84)continue;var cx=r.left+r.width/2,cy=r.top+r.height/2;if(cx<vr.left-24||cx>vr.right+24||cy<vr.top-24||cy>vr.bottom+24)continue;var sem='',sp=e,u=0;while(sp&&u++<3){var c=sp.className;c=String(c&&c.baseVal!==undefined?c.baseVal:(c||''));sem+=' '+c+' '+String((sp.getAttribute&&sp.getAttribute('aria-label'))||(sp.getAttribute&&sp.getAttribute('title'))||'');sp=sp.parentElement;}if(/mute|volume|sound|caption|fullscreen/i.test(sem)||!/play|pause/i.test(sem))continue;var q=(e.closest&&e.closest('button,[role=button]'))||e,qr=q.getBoundingClientRect();if(qr.width<10||qr.width>96||qr.height<10||qr.height>96)q=e;if(q.getAttribute&&q.getAttribute('data-ad-videostock6063')==='1')continue;q.setAttribute('data-ad-videostock6063','1');q.setAttribute('data-ad-native615','1');adVStockScrub6063(q);n++;}return n;}catch(e){return 0;}};"
           "window.__AD_VIDEOSTOCK6063__=function(root){try{var V=[],n=0;function addFrom(b){if(!b)return;try{if(String(b.tagName||'').toUpperCase()==='VIDEO'&&V.indexOf(b)<0)V.push(b);var q=b.querySelectorAll?b.querySelectorAll('video'):[];for(var i=0;i<q.length&&V.length<36;i++)if(V.indexOf(q[i])<0)V.push(q[i]);}catch(x){}}addFrom(root);if(!V.length&&root&&root.nodeType===1){var p=root.parentElement,d=0;while(p&&d++<4&&!V.length){addFrom(p);p=p.parentElement;}}if(!V.length&&root===document)addFrom(document);for(var j=0;j<V.length&&j<36;j++)n+=window.__AD_VIDEOSTOCK_ONE6063__(V[j]);return n;}catch(e){return 0;}};"
           "var vcs63=function(ev){try{var t=ev&&ev.target;if(t&&String(t.tagName||'').toUpperCase()==='VIDEO')window.__AD_VIDEOSTOCK_ONE6063__(t);}catch(e){}};document.addEventListener('loadedmetadata',vcs63,true);document.addEventListener('loadeddata',vcs63,true);document.addEventListener('canplay',vcs63,true);document.addEventListener('play',vcs63,true);document.addEventListener('playing',vcs63,true);document.addEventListener('pause',vcs63,true);"
           "document.addEventListener('click',function(ev){try{var p=ev&&ev.target,d=0,sem='';while(p&&d++<4){var c=p.className;c=String(c&&c.baseVal!==undefined?c.baseVal:(c||''));sem+=' '+c+' '+String((p.getAttribute&&p.getAttribute('aria-label'))||(p.getAttribute&&p.getAttribute('title'))||'');p=p.parentElement;}if(!/play|pause/i.test(sem)||/mute|volume|sound|caption|fullscreen/i.test(sem))return;setTimeout(function(){window.__AD_VIDEOSTOCK6063__(document);},0);setTimeout(function(){window.__AD_VIDEOSTOCK6063__(document);},90);}catch(e){}},true);"
           "var vis63=function(){try{window.__AD_VIDEOSTOCK6063__(document);}catch(e){}};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',vis63,{once:true});else vis63();window.addEventListener('pageshow',vis63,{passive:true});}catch(e){}"
