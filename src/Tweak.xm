...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         // v5.405 LOCK BOUNDARY: HEART / CHECKBOX / DOWN-ARROW FROZEN.
         // User confirmed these three controls visually correct in v5.404. No code below
         // this boundary may target their selector families. v5.405 changes ONLY the
         // lists-framework two-cards action control.
         //
         // TWO-CARDS: reproduce the successful v5.333 contract at final authority.
         // v5.404 could accidentally skin a glyph leaf as the host and then clear the
         // real parent ring. Here we resolve one OUTERMOST structural square host for
         // each lists-framework action, remove only stale cards ownership, retain
         // Amazon's stock glyph, and apply the exact v5.333 disc values.
         "window.__AD_CARDS405__=function(){try{if(window.__ADFRAME_MODE__||!document.body)return 0;function rr(e){try{return e&&e.getBoundingClientRect?e.getBoundingClientRect():null;}catch(x){return null;}}function sq(e){var r=rr(e);return !!(r&&r.width>=18&&r.width<=52&&r.height>=18&&r.height<=52&&Math.abs(r.width-r.height)<=10);}function structural(e){return !!(e&&e.nodeType===1&&!/^(IMG|I|INPUT|SVG|PATH|USE|POLYGON)$/.test(String(e.tagName||'').toUpperCase()));}var stale=document.querySelectorAll('[data-ad-v333404=\"d\"]');for(var z=0;z<stale.length&&z<120;z++){var q=stale[z];q.removeAttribute('data-ad-v333404');q.removeAttribute('data-ad-v333raster404');q.removeAttribute('data-ad-v333vector404');['background-color','border','border-radius','box-shadow','box-sizing'].forEach(function(k){q.style.removeProperty(k);});}var A=document.querySelectorAll('[class*=lists-framework-action-button]'),seen=[],n=0,ras=0,vec=0,miss=0;for(var i=0;i<A.length&&i<180;i++){var a=A[i],p=a,best=null,barea=-1,u=0;while(p&&u++<4){if(sq(p)&&structural(p)){var pr=rr(p),area=pr.width*pr.height;if(area>barea){barea=area;best=p;}}p=p.parentElement;}if(!best&&sq(a)&&structural(a))best=a;if(!best){miss++;continue;}if(seen.indexOf(best)>=0)continue;seen.push(best);best.setAttribute('data-ad-cards405','1');best.style.setProperty('background-color','#181a1b','important');best.style.setProperty('border-radius','50%%','important');best.style.setProperty('border','1.5px solid rgba(255,255,255,0.65)','important');best.style.setProperty('box-shadow','none','important');best.style.setProperty('box-sizing','border-box','important');var G=best.querySelectorAll('img,i,svg,path,use,polygon,[class*=lists-framework-unfill],[class*=lists-framework-fill],[class*=a-icon]');for(var j=0;j<G.length&&j<80;j++){var g=G[j],gr=rr(g);if(gr&&(gr.width<3||gr.height<3||gr.width>48||gr.height>48))continue;var tg=String(g.tagName||'').toUpperCase(),cs=getComputedStyle(g),bi=String(cs.backgroundImage||'none'),mi=String(cs.maskImage||cs.webkitMaskImage||'none');g.style.setProperty('background-color','transparent','important');if(tg==='SVG'||tg==='PATH'||tg==='USE'||tg==='POLYGON'){g.setAttribute('data-ad-cardsvector405','1');vec++;}else if(tg==='IMG'||tg==='I'||bi!=='none'||mi!=='none'){g.setAttribute('data-ad-cardsraster405','1');g.__adGlyph=1;g.__adBy='cards405';ras++;}}n++;}window.__AD_CARDS405_STATE__='n='+n+' raster='+ras+' vector='+vec+' miss='+miss;return n;}catch(e){window.__AD_CARDS405_STATE__='err '+(e&&e.message||e);return -1;}};"
         "try{if(document&&!document.getElementById('adcards405')){var c405=document.createElement('style');c405.id='adcards405';c405.textContent='[data-ad-cards405]{background-color:#181a1b !important;border-radius:50%% !important;border:1.5px solid rgba(255,255,255,.65) !important;box-shadow:none !important;box-sizing:border-box !important;}[data-ad-cards405] *{color:#fff !important;fill:#fff !important;stroke:#fff !important;}[data-ad-cardsvector405],[data-ad-cardsvector405] *{color:#fff !important;fill:#fff !important;stroke:#fff !important;}[data-ad-cardsraster405]{filter:brightness(0) invert(1) !important;background-color:transparent !important;}';(document.head||document.documentElement).appendChild(c405);}}catch(e){}"
         "try{window.__AD_PRODUCTCTRL391_PRE405__=window.__AD_PRODUCTCTRL391RUN__;window.__AD_PRODUCTCTRL391RUN__=function(){var r=window.__AD_PRODUCTCTRL391_PRE405__?window.__AD_PRODUCTCTRL391_PRE405__():0;try{window.__AD_CARDS405__();}catch(x){}return r;};window.__AD_CARDS405__();setTimeout(window.__AD_CARDS405__,120);setTimeout(window.__AD_CARDS405__,500);setTimeout(window.__AD_CARDS405__,1500);if(!window.__AD405CARDSCROLL__){window.__AD405CARDSCROLL__=1;var t405=0;addEventListener('scroll',function(){if(t405)return;t405=1;var f405=function(){t405=0;try{window.__AD_CARDS405__();}catch(x){}};if(requestAnimationFrame)requestAnimationFrame(f405);else setTimeout(f405,0);},{passive:true,capture:true});}}catch(e){}"
...
"/*V5317FIX*//*V5376FIX*//*V5377FIX*//*V5378FIX*//*V5379FIX*//*V5380FIX*//*V5381FIX*//*V5382FIX*//*V5383FIX*//*V5384FIX*//*V5385FIX*//*V5386FIX*//*V5387FIX*//*V5388FIX*//*V5391FIX*//*V5393FIX*/"
       ...
       // P74CARDS405: exact v5.333 two-cards visual contract, now measured on the
       // actual final host rather than on a passive class/marker.
       "try{var C74=document.querySelectorAll('[data-ad-cards405]'),badBg74=0,badRing74=0,badShape74=0,badGlyph74=0,Q74=[];for(var i74=0;i74<C74.length&&i74<80;i74++){var e74=C74[i74],r74=e74.getBoundingClientRect(),s74=getComputedStyle(e74),bg74=String(s74.backgroundColor||'').replace(/\\s+/g,''),bw74=parseFloat(s74.borderTopWidth||'0'),br74=parseFloat(s74.borderTopLeftRadius||'0');if(bg74!=='rgb(24,26,27)'&&bg74!=='rgba(24,26,27,1)')badBg74++;if(Math.abs(bw74-1.5)>.26)badRing74++;if(br74<Math.min(r74.width,r74.height)*.40)badShape74++;var G74=e74.querySelectorAll('[data-ad-cardsraster405],[data-ad-cardsvector405]'),good74=0;for(var j74=0;j74<G74.length;j74++){var g74=G74[j74],gs74=getComputedStyle(g74);if(g74.hasAttribute('data-ad-cardsraster405')&&String(gs74.filter||'').indexOf('invert')>=0)good74=1;if(g74.hasAttribute('data-ad-cardsvector405')&&/255/.test(String(gs74.fill||'')+String(gs74.stroke||'')+String(gs74.color||'')))good74=1;}if(!good74)badGlyph74++;if(Q74.length<8)Q74.push(Math.round(r74.width)+'x'+Math.round(r74.height)+'|bg='+bg74+'|bw='+bw74+'|br='+br74+'|g='+G74.length);}out.push('P74CARDS405[n='+C74.length+' badBg='+badBg74+' badRing='+badRing74+' badShape='+badShape74+' badGlyph='+badGlyph74+' state='+String(window.__AD_CARDS405_STATE__||'-')+(Q74.length?' '+Q74.join(' ~~ '):'')+']');}catch(e74){out.push('P74CARDS405[err '+(e74&&e74.message||e74)+']');}"
