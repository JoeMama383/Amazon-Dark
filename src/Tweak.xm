/*
 * AmazonDark v7.357 — Search footer + Shop-by-style live-owner correction on v7.356
 *
 * Architecture:
...
#define AD_VERSION "v7.357-search-footer-shop-style-fix"
...
static NSString *ADFloorJS(void){
    ...
    return
        ...
        // v7.357 Shop-by-style: the v7.356 guessed class tokens did not match the live 2026 renderer.
        // A bounded load-time text-node marker below owns only the widget containing the exact authored heading.
        @"#search#search [data-ad7-shop-by-style=\\\"1\\\"]{background:#000!important;background-color:#000!important;background-image:none!important;border-color:#494d4d!important;outline-color:#494d4d!important;box-shadow:none!important;color:#e8e6e3!important;-webkit-text-fill-color:#e8e6e3!important;}#search#search [data-ad7-shop-by-style=\\\"1\\\"] :is(div,section,article,main,header,footer,ul,ol,li,.a-cardui,.a-carousel-container,.a-carousel-viewport,.a-carousel-card){background-color:#000!important;border-color:#494d4d!important;outline-color:#494d4d!important;box-shadow:none!important;color:#e8e6e3!important;-webkit-text-fill-color:#e8e6e3!important;}#search#search [data-ad7-shop-by-style=\\\"1\\\"] :is(h1,h2,h3,h4,h5,h6,p,span,a,strong,b,em,label,.a-color-base,.a-color-secondary,.a-text-normal){color:#e8e6e3!important;-webkit-text-fill-color:#e8e6e3!important;}#search#search [data-ad7-shop-by-style=\\\"1\\\"] :is(img,picture,source){background-color:transparent!important;mix-blend-mode:normal!important;}"
        ...
        @"\");}if(!child&&(p==='/s'||p.indexOf('/s/')===0)){function ad7357ShopByStyle(){try{var r=document.getElementById('search');if(!r)return false;if(r.querySelector('[data-ad7-shop-by-style=\"1\"]'))return true;var w=document.createTreeWalker(r,NodeFilter.SHOW_TEXT),n=null,h=null,seen=0;while((n=w.nextNode())&&seen++<7000){var v=String(n.nodeValue||'').replace(/\\s+/g,' ').trim().toLowerCase(),e=n.parentElement,pv=e?String(e.textContent||'').replace(/\\s+/g,' ').trim().toLowerCase():'';if(v==='shop by style'||pv==='shop by style'){h=e;break;}}if(!h)return false;var p0=h,best=null,vw=Math.max(1,window.innerWidth||document.documentElement.clientWidth||1);for(var d=0;p0&&d<12;d++,p0=p0.parentElement){if(p0.id==='search')break;var imgs=p0.getElementsByTagName?p0.getElementsByTagName('img').length:0,r0=p0.getBoundingClientRect?p0.getBoundingClientRect():null;if(imgs>=2&&r0&&r0.width>=vw*.68&&r0.height>=120&&r0.height<=900)best=p0;if(imgs>=2&&p0.matches&&p0.matches('.s-widget-container,.celwidget,[data-csa-c-type=widget],.a-cardui')){best=p0;break;}}if(!best)best=h.parentElement||h;best.setAttribute('data-ad7-shop-by-style','1');return true}catch(_){return false}}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',ad7357ShopByStyle,{once:true});window.addEventListener('load',ad7357ShopByStyle,{once:true});}else ad7357ShopByStyle();window.addEventListener('pageshow',ad7357ShopByStyle,{passive:true});}if(document.rea"
...
static NSString *ADTWBJS(void){
    ...
    NSString *built=[NSString stringWithFormat:
        ...
        @"*=checkbox]):not([class*=heart]):not([class*=wishlist]):not([class*=icon]):not([class*=glyph]):not([class*=badge]):not(:where([data-testid=prime-badge] *)):not(:where([data-testid=ratings-stars] *)):not(:where([data-ad-feedback-label-id] *)):not(:where([class*=sponsored] *)):not(:where([class*=ad-feedback] *)):not(:where([class*=adFeedback] *)),html[data-ad7-twb-child=\\\"1\\\"] [data-testid=simple-brand-logo-picture] img{filter:brightness(%.3f)!important;}\");return;}var p='';try{p=String(location.pathname||'');}catch(_){}var s=null;if(p==='/autocomplete'||p.indexOf('/autocomplete/')===0){s=put('ad7-search-pane-twb',\"img.ufs_tiles_card_widget-sug-image,img.s-entity-pd-carousel-tile-element-image,#attach-to-me img.s-image,#attach-to-me img.s-product-image,.s-suggestion-container img.s-image,.s-suggestion-container img.s-product-image{filter:none!important;-webkit-filter:none!important;opacity:%.3f!important;}.cards_carousel_widget-sug-container-top{--ad7-cards-twb:%.3f;}.cards_carousel_widget-sug-container-top img{filter:brightness(var(--ad7-cards-twb)) saturate(1)!important;-webkit-filter:brightness(var(--ad7-cards-twb)) saturate(1)!important;opacity:1!important;visibility:visible!important;mix-blend-mode:normal!important;}\");}else if(p==='/s'||p.indexOf('/s/')===0){s=put('ad7-product-feed-twb',\"#search img.scx-stt-image,#search .s-trft-image-container img,#search img._c2Itd_image_3UiYm,#search [class*=_bXVsd_image_],#search [class*=_bXVsd_lifestyleImage_],#search [class*=_bXVsd_lifestyleimage_],#search img.s-image,#search img.s-product-image,#search [data-component-type=s-product-image] img,#search img.ufs_tiles_card_widget-sug-image,#search img.haul-puis-portrait-img,#search img._c2Itd_image_pQREQ,#search ._c2Itd_cardContent_3OGkG.sbv-ad-content-container img:not([class*=_trackingPixel_]):not([class*=ad-feedback]):not([class*=sprite]),#search [data-csa-c-painter=store-spotlight-v2-creative-mobile-cards] img:not([class*=_pixel_]):not([class*=tracking]),#search .ies-wrapper .ccs-ies-card-image-container img,#search [data-ad7-shop-by-style=\\\"1\\\"] img:not([class*=icon]):not([class*=glyph]):not([class*=badge]):not([class*=pixel]):not([class*=tracking]){filter:brightness(%.3f)!important;-webkit-filter:brightness(%.3f)!important;opacity:1!important;mix-blend-mode:normal!important;}#search .mlt-icon-container img,#search .lists-framework-action-button.puis-heart-icon-container img,#search .puis-mab-chevron img,#search .puis-mab-chevron-glyph img,#search .s-pc-certification-faceout img.s-image{filter:none!important;-webkit-filter:none!important;opacity:1!important;mix-blend-mode:normal!important;}#search video.sbv-video-player-ecx,#search video._"
