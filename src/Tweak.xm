...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         "window.__AMZDARK_FIXCONTRAST__=function(){try{"
           ...
           "}catch(e){}"
                      // v5.352: P9VIS on-device showed the College arrows are exactly
           // cardui .a-icon-alt background-image leaves (inkL=0, dr=0). The
           // prior accessibility-label matcher therefore never ran. Retheme
           // the observed paint leaf directly, but only when it is glyph-sized,
           // owns a background image and is dark. Guard + __adBy satisfy the
           // artwork regression gate and keep merchandise images out.
           "try{var AIA=document.querySelectorAll('[class*=a-cardui] .a-icon-alt');"
             "__ck('AIA');"
             "for(var aj=0;aj<AIA.length&&aj<160&&((aj&15)||!ovr());aj++){var aa=AIA[aj];"
               "var ars=aa.getBoundingClientRect();if(ars.width<6||ars.width>52||ars.height<6||ars.height>52)continue;"
               "var ass=getComputedStyle(aa),abi=String(ass.backgroundImage||'none');if(abi==='none'||abi.indexOf('url(')<0)continue;"
               "if(artChk(aa))continue;var acp=AF.p(ass.color);if(acp&&AF.l(acp)>0.40)continue;"
               "aa.style.setProperty('filter','brightness(0) invert(1)','important');"
               "aa.style.setProperty('opacity','1','important');"
               "aa.__adGlyph=1;aa.__adBy='cardchev';aa.setAttribute('data-ad-nav-chevron','1');aa.setAttribute('data-ad-nav-label','cardui-alt');}"
           "}catch(e){}"
           // v5.352: normalize mismatched brown/gray rounded card outlines.
           // The desired gray sampled from the screenshot is rgb(59,64,67).
           // Only sizeable rounded card containers with a real painted border
           // qualify; badges, controls and borderless content never enter.
           "try{var CBF=document.querySelectorAll('[class*=a-cardui],[class*=npack-asin-card],[class*=gwm-asin-tile],[class*=gwm-window-layout],[class*=window-container],[class*=theming-card],[class*=canvas-card],[class*=cXVhZ],[class*=puis-card],[class*=gwm-tile]');"
             "__ck('CBF');var BSD=['Top','Right','Bottom','Left'];"
             "for(var bj=0;bj<CBF.length&&bj<220&&((bj&15)||!ovr());bj++){var be=CBF[bj],br=be.getBoundingClientRect();"
               "if(br.width<120||br.height<50)continue;var bcs=getComputedStyle(be),brad=parseFloat(bcs.borderTopLeftRadius)||0;if(brad<4)continue;"
