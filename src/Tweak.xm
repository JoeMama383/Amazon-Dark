    ...
    return
       ...
       "try{var CD=[];"
         "for(var z=0;z<N;z++){var e5=all[z];var r5=e5.getBoundingClientRect();"
           "if(r5.width<300||r5.height<12)continue;"
           "var s5=getComputedStyle(e5),hit5='';"
           "for(var v=0;v<4;v++){var sd=['Top','Right','Bottom','Left'][v];"
             "var w5=parseFloat(s5['border'+sd+'Width'])||0;if(w5<0.5)continue;"
             "var y5=s5['border'+sd+'Style'];if(y5==='none'||y5==='hidden')continue;"
             "var L5=lum(s5['border'+sd+'Color']);"
             "if(L5>0.55){hit5=sd.charAt(0)+' L='+L5.toFixed(2)+' '+s5['border'+sd+'Color'];break;}}"
           "if(!hit5)continue;"
           "var mt=0;try{mt=e5.matches('[class*=puis-card],[class*=gwm-tile],[class*=ape-placement],[class*=mosaic-container],[class*=plus-container],[class*=thumbnail-conta]')?1:0;}catch(em){}"
           "var rd5=parseFloat(s5.borderTopLeftRadius)||0;"
           "CD.push({w:r5.width,t:e5.tagName+'|'+cls(e5)+'|'+Math.round(r5.width)+'x'+Math.round(r5.height)+'|r'+Math.round(rd5)+'|'+hit5+'|mine='+mt});}"
         "CD.sort(function(a,b){return b.w-a.w;});var co=[];"
         "for(var z2=0;z2<CD.length&&z2<6;z2++)co.push(CD[z2].t);"
         "out.push('P9CARD[n='+CD.length+(co.length?' '+co.join(' ~ '):'')+']');}catch(ec){out.push('P9CARD[err]');}"
       "return out.join(' ');"
