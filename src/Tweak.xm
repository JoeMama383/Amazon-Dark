...
static NSString *ADFixesLiteral(void){
    ...
    gADFixesLiteral613 = [NSString stringWithFormat:
            @"{css:'"
             ...
             "[data-component-type=s-search-result] :where(div,span,section):has(> [data-csa-c-content-id=variation-options-link]),"
             "[data-component-type=s-search-result] :where(div,span,section):has(> [class*=s-variation-options-link]),"
             "[data-component-type=s-search-result] :where(div,span,section):has(> [class*=s-color-swatch-container-list-view]),"
             "[data-component-type=s-search-result] :where(div,span,section):has(> [class*=puis-csi-with-label-container]),"
             "[data-component-type=s-search-result] :where(div,span,section):has(> [class*=rush-component] [class*=s-variation-options-link]),"
             "[data-component-type=s-search-result] :where(div,span,section):has(> [class*=rush-component] [class*=s-color-swatch-container-list-view]),"
             ...
             "[data-component-type=s-search-result] :where(div,span,section):has(> [data-component-type=s-status-badge-component]),"
             "[data-component-type=s-search-result] :where(div,span,section):has(> span [data-component-type=s-status-badge-component])"
...
static NSString *ADDarkReaderBootstrap(void){
    ...
    gADBootstrap613 = [NSString stringWithFormat:
        @"(function(){try{"
         ...
         "window.__AMZDARK_FIXCONTRAST__=function(root){return 0;/*v6.0.216 retired*/try{var base=(root&&root.nodeType===1)?root:(document.body||document.documentElement);"
           "var FG='%@';"
         ...
         // v6.0.215: AmazonDark's own three MutationObservers are retired. Each one
         // observed document.documentElement with childList+subtree, so every DOM
         // mutation on the page ran a callback -- and Search and PDP hydrate
         // continuously, which is the same shape as the Dark Reader observer that
         // v6.0.211 proved was the input latency. Ours were smaller but identical in
         // kind, and the goal is now zero per-mutation JavaScript.
         //
         // They are replaced by an inert constructor rather than deleted line by line:
         // every call site keeps its shape, the one-shot initial passes still run, and
         // nothing observes. `new F()` where F returns an object yields that object,
         // so this substitutes cleanly for `new (window.__AD_OBS215__||MutationObserver)(...)`.
         //
         // WHAT THIS COSTS: anything that only worked because an observer re-ran on
         // later mutations no longer updates. Coverage for late-arriving nodes comes
         // from CSS instead -- the document-start sheet, the Dark Reader fixes sheet,
         // and the generated stylesheet published on settle. Those are matched by the
         // style engine during normal layout, at no per-node cost.
         "window.__AD_OBS215__=function(){return {observe:function(){},disconnect:function(){},takeRecords:function(){return [];}};};"
