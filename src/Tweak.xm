...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{(function(){"
         ...
         "function dealText(e){try{var t=String(e.textContent||'').replace(/\\s+/g,' ').trim();return /^(?:\\d+%\\s*off|limited\\s+time\\s+deal)$/i.test(t);}catch(_){return false;}}"
         ...
         "function badgeFix(){try{"
           ...
           "var S=document.querySelectorAll('[class*=sponsored-products] *'),n=0;for(var j=0;j<S.length&&j<2500&&n<80;j++){var x=S[j];if(x.childElementCount!==0)continue;var t=String(x.textContent||'').replace(/\\s+/g,' ').trim();"
             ...
             "else if(/^limited\\s+time\\s+deal$/i.test(t)){x.style.setProperty('color','#e8e6e3','important');x.style.setProperty('-webkit-text-fill-color','#e8e6e3','important');n++;}}window.__AD_SPONSORED_BADGEFIX__=n;"
