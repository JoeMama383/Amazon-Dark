...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{(function(){"
         ...
         // v5.351 COLLEGE CAROUSEL CHEVRONS. P9VIS on-device identified the
         // remaining black arrows as background-image a-icon-alt glyphs (dr=0).
         // Do not filter cardui generically: the product-art guard is intentionally
         // broad. Instead use Amazon's own accessibility label (Previous/Next) and
         // require an a-cardui ancestor, then mark only
         // the glyph-sized background-image paint leaf. CSS above owns the filter.
         "function navFix(){try{var A=document.querySelectorAll('.a-icon-alt'),n=0;for(var i=0;i<A.length&&i<160;i++){var x=A[i],t=String(x.textContent||'').replace(/\\s+/g,' ').trim();if(!/^(?:previous|next|back|forward)(?:\\s+(?:page|item|slide|card))?$/i.test(t))continue;var card=null;try{card=x.closest&&x.closest('[class*=a-cardui]');}catch(_){}if(!card)continue;var p=x,j=0,paint=null;while(p&&j++<4){var r=p.getBoundingClientRect(),s=getComputedStyle(p),bi=String(s.backgroundImage||'none');if(r.width>=6&&r.width<=52&&r.height>=6&&r.height<=52&&bi!=='none'){paint=p;break;}p=p.parentElement;}if(!paint)continue;paint.setAttribute('data-ad-nav-chevron','1');paint.setAttribute('data-ad-nav-label',t.toLowerCase());n++;}window.__AD_NAVCHEV__=n;}catch(e){}}"
         ...
         "}catch(e){}}""badgeFix();dotFix();packFix();navFix();""clr();setTimeout(clr,300);setTimeout(clr,1200);setTimeout(clr,2500);""setTimeout(badgeFix,300);setTimeout(badgeFix,1200);setTimeout(badgeFix,2500);""setTimeout(dotFix,120);setTimeout(dotFix,500);setTimeout(dotFix,1400);setTimeout(dotFix,2800);""setTimeout(packFix,120);setTimeout(packFix,500);setTimeout(packFix,1400);setTimeout(packFix,2800);""setTimeout(navFix,120);setTimeout(navFix,500);setTimeout(navFix,1400);setTimeout(navFix,2800);""addEventListener('scroll',function(){clearTimeout(window.__bgT);window.__bgT=setTimeout(function(){badgeFix();dotFix();packFix();navFix();},120);},{passive:true,capture:true});""addEventListener('scroll',function(){clearTimeout(window.__bxS);window.__bxS=setTimeout(clr,120);},{passive:true,capture:true});"
         "new MutationObserver(function(){clearTimeout(window.__bxT);"
           "window.__bxT=setTimeout(function(){clr();dotFix();packFix();navFix();},60);}).observe(document.documentElement,"
