...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{var Z=[],ZE=document.querySelectorAll('svg,[class*=chevron],[class*=arrow],i');"
         "for(var z=0;z<ZE.length&&Z.length<6;z++){var ze=ZE[z];"
           "var zr=ze.getBoundingClientRect();if(zr.width<6||zr.height<6)continue;"
           "if(zr.bottom<0||zr.top>(window.innerHeight||900))continue;"
           "var zs=getComputedStyle(ze);var zt=String(ze.textContent||'').trim();"
           "if(zr.width>40||zr.height>40)continue;"
           "var zb=-1,zp=ze,zd=0;while(zp&&zd++<8){var zq=lum(getComputedStyle(zp).backgroundColor);"
             "if(zq>=0){zb=zq;break;}zp=zp.parentElement;}"
           "var zk=[],zn=ze,ze2=0;while(zn&&ze2<3){zk.push((cls(zn)||zn.tagName).slice(0,18));zn=zn.parentElement;ze2++;}"
           "Z.push(Math.round(zr.width)+'x'+Math.round(zr.height)+'|fill='+zs.fill+'|op='+zs.opacity+'|col='+zs.color+'|bgUp='+(zb<0?'-':zb.toFixed(2))+'|'+zk.join('>'));}"
         "out.push('P9CHEV['+(Z.length?Z.join(' ~ '):'none')+']');}catch(zx){out.push('P9CHEV[err]');}"
       "try{var Z=[],ZE=document.querySelectorAll('[class*=npack-asin-card] span,[class*=canvas-card] span,[class*=theming-card] span');"
         "for(var z=0;z<ZE.length&&Z.length<6;z++){var ze=ZE[z];"
           "var zr=ze.getBoundingClientRect();if(zr.width<6||zr.height<6)continue;"
           "if(zr.bottom<0||zr.top>(window.innerHeight||900))continue;"
           "var zs=getComputedStyle(ze);var zt=String(ze.textContent||'').trim();"
           "if(ze.childElementCount>0)continue;if(zt.length<4||zt.length>34)continue;var zf=parseFloat(zs.fontSize)||0;if(zf<16)continue;"
           "var zb=-1,zp=ze,zd=0;while(zp&&zd++<8){var zq=lum(getComputedStyle(zp).backgroundColor);"
             "if(zq>=0){zb=zq;break;}zp=zp.parentElement;}"
           "var zk=[],zn=ze,ze2=0;while(zn&&ze2<3){zk.push((cls(zn)||zn.tagName).slice(0,18));zn=zn.parentElement;ze2++;}"
           "Z.push(zt.slice(0,14)+'|fs='+zf.toFixed(0)+'|col='+zs.color+'|bgUp='+(zb<0?'-':zb.toFixed(2))+'|'+zk.join('>'));}"
         "out.push('P9CARDHDR['+(Z.length?Z.join(' ~ '):'none')+']');}catch(zx){out.push('P9CARDHDR[err]');}"
       "try{var Z=[],ZE=document.querySelectorAll('[class*=cXVhZ] span,[class*=cXVhZ] div');"
         "for(var z=0;z<ZE.length&&Z.length<6;z++){var ze=ZE[z];"
           "var zr=ze.getBoundingClientRect();if(zr.width<6||zr.height<6)continue;"
           "if(zr.bottom<0||zr.top>(window.innerHeight||900))continue;"
           "var zs=getComputedStyle(ze);var zt=String(ze.textContent||'').trim();"
           "if(ze.childElementCount>0)continue;if(zt.length<3||zt.length>30)continue;var zl=lum(zs.color);if(zl<0)continue;"
           "var zb=-1,zp=ze,zd=0;while(zp&&zd++<8){var zq=lum(getComputedStyle(zp).backgroundColor);"
             "if(zq>=0){zb=zq;break;}zp=zp.parentElement;}"
           "var zk=[],zn=ze,ze2=0;while(zn&&ze2<3){zk.push((cls(zn)||zn.tagName).slice(0,18));zn=zn.parentElement;ze2++;}"
           "Z.push(zt.slice(0,12)+'|txtL='+zl.toFixed(2)+'|col='+zs.color+'|bgUp='+(zb<0?'-':zb.toFixed(2))+'|'+zk.join('>'));}"
         "out.push('P9TILETXT['+(Z.length?Z.join(' ~ '):'none')+']');}catch(zx){out.push('P9TILETXT[err]');}"
       "/*V5313FIX*/"
       "try{(function(){"
         ...
         "function clr(){try{var N=document.querySelectorAll(SEL),c=0;"
           ...
           "window.__AD_BOXCLR__=c;}catch(e){}}"
         "function badgeFix(){try{var B=document.querySelectorAll('[class*=badgeLabel]');for(var i=0;i<B.length&&i<200;i++){var b=B[i];if(b.hasAttribute&&b.hasAttribute('data-darkreader-inline-bgcolor')){b.style.removeProperty('background-color');b.removeAttribute('data-darkreader-inline-bgcolor');}b.style.setProperty('background-color','#cc0c39','important');b.style.setProperty('color','#ffffff','important');b.style.setProperty('-webkit-text-fill-color','#ffffff','important');}}catch(e){}}""badgeFix();""clr();setTimeout(clr,300);setTimeout(clr,1200);setTimeout(clr,2500);""setTimeout(badgeFix,300);setTimeout(badgeFix,1200);setTimeout(badgeFix,2500);""addEventListener('scroll',function(){clearTimeout(window.__bgT);window.__bgT=setTimeout(badgeFix,120);},{passive:true,capture:true});""addEventListener('scroll',function(){clearTimeout(window.__bxS);window.__bxS=setTimeout(clr,120);},{passive:true,capture:true});"
         "new MutationObserver(function(){clearTimeout(window.__bxT);"
           "window.__bxT=setTimeout(clr,60);}).observe(document.documentElement,"
