       ...
       "try{"
         ...
         "try{if(DBX.length){var A426='';"
           ...
           "try{var V87=/cart/i.test(location.pathname)?'cart':(/\\/s(\\b|\\?|$)/.test(location.pathname+location.search)?'shop':'other');"
             "var C87=document.querySelectorAll('[class*=a-check'+'box],[role=check'+'box]'),W87=[];"
             "for(var j87=0;j87<C87.length&&W87.length<2;j87++){var b87=C87[j87];"
               "var q87=b87.getBoundingClientRect();if(q87.width<10||q87.height<10)continue;"
               "var n87=b87.querySelector('input[type=check'+'box]');"
               "var g87=b87.querySelector('[class*=a-icon-check]')||b87;"
               "var y87=getComputedStyle(g87),z87=getComputedStyle(b87);"
               ...
               "W87.push(V87+'|host='+k87(b87)+'|art='+String(g87.tagName)+'.'+k87(g87)"
                 "+'|f='+String(y87.filter||'-').slice(0,20)+'|bgi='+im87"
                 "+'|inp='+(n87?1:0)+'|chk='+(n87&&n87.checked?1:0)"
                 "+'|aria='+String(b87.getAttribute('aria-checked')||'-')"
                 "+'|hostbg='+String(z87.backgroundColor||'').replace(/\\s+/g,'')"
                 "+'|hostbd='+parseFloat(z87.borderTopWidth||0).toFixed(1));}"
             "if(W87.length){var pv87='';try{pv87=localStorage.getItem('__ad87')||'';}catch(_){}"
               ...
               "try{localStorage.setItem('__ad87',W87.join(' ~ ')+(oth87?' || '+oth87:''));}catch(_){}}"
