/*
 * AmazonDark v7.334 — process-scoped launch readiness
 *
 * Architecture:
...
#define AD_VERSION "v7.334-snapshot-scoped-switcher-cover"
...
// v7.334 task-switcher snapshot cover. The first switcher snapshot may be requested
// as Amazon resigns active, so lifecycle code activates this cover at WillResignActive
// and keeps it through the background lifetime. SpringBoard's launch overlay owns the live
// foreground transition, but it is not part of the app-rendered scene image that
// UIKit persists for the app switcher. Keep one opaque black/logo view on each
// full-size Amazon window while inactive/backgrounded; remove it before foreground
// visibility, or at DidBecomeActive when the interruption never entered background.
static const void *kADTaskSwitcherCoverKey7329=&kADTaskSwitcherCoverKey7329;
static BOOL gADTaskSwitcherCoverActive7329=NO;
// v7.334: the switcher owner exists only long enough to be captured. Leaving it
// in the live app hierarchy until DidBecomeActive made every same-process warm
// reopen briefly show the black/logo snapshot itself. Epoch invalidates queued
// retire blocks when a newer lifecycle boundary supersedes them.
static NSUInteger gADTaskSwitcherEpoch7334=0;
static int gADTaskSwitcherReleaseToken7334=0;
static void ADLaunchAppProbeLog7317(NSString *event,NSString *detail);
static const NSTimeInterval kADTaskSwitcherCaptureWindow7334=0.45;

static UIImage *ADTaskSwitcherLogo7329(void){
...
static void ADAttachTaskSwitcherCover7329(UIWindow *window){
    if(!window||CGRectGetWidth(window.bounds)<160.0||CGRectGetHeight(window.bounds)<240.0)return;
    @try {
        UIView *cover=(UIView *)objc_getAssociatedObject(window,kADTaskSwitcherCoverKey7329);
        if(!cover){
            ...
            UIImage *image=ADTaskSwitcherLogo7329();
            if(image){
                UIImageView *logo=[[UIImageView alloc] initWithImage:image];
                logo.contentMode=UIViewContentModeScaleAspectFit;
                logo.translatesAutoresizingMaskIntoConstraints=NO;
                [cover addSubview:logo];
                CGFloat width=MAX(CGRectGetWidth(window.bounds),200.0)*0.62;
                CGFloat height=width*(image.size.height/MAX(image.size.width,1.0));
                [NSLayoutConstraint activateConstraints:@[[logo.centerXAnchor constraintEqualToAnchor:cover.centerXAnchor],[logo.centerYAnchor constraintEqualToAnchor:cover.centerYAnchor],[logo.widthAnchor constraintEqualToConstant:width],[logo.heightAnchor constraintEqualToConstant:height]]];
            }
            objc_setAssociatedObject(window,kADTaskSwitcherCoverKey7329,cover,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
static NSUInteger ADSetTaskSwitcherCoverActive7329(BOOL active){
    gADTaskSwitcherCoverActive7329=active;
    NSUInteger count=0;
    @try {
        for(UIWindow *window in UIApplication.sharedApplication.windows){
            if(active){
                ADAttachTaskSwitcherCover7329(window);
                if(objc_getAssociatedObject(window,kADTaskSwitcherCoverKey7329))count++;
            } else {
                UIView *cover=(UIView *)objc_getAssociatedObject(window,kADTaskSwitcherCoverKey7329);
                if(cover){
                    count++;
                    [cover removeFromSuperview];
                    objc_setAssociatedObject(window,kADTaskSwitcherCoverKey7329,nil,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                }
            }
        }
    } @catch(...) {}
...
static void ADInstallTaskSwitcherWarmRelease7334(void){
    if(gADTaskSwitcherReleaseToken7334>0)return;
    NSInteger pid=NSProcessInfo.processInfo.processIdentifier;
    if(pid<=0)return;
    NSString *channel=[NSString stringWithFormat:@"com.colindavidr.amazondark.switcher-release.%ld",(long)pid];
    int token=0;
    uint32_t status=notify_register_dispatch(channel.UTF8String,&token,dispatch_get_main_queue(),^(__unused int t){
        ++gADTaskSwitcherEpoch7334;
        NSUInteger removed=ADSetTaskSwitcherCoverActive7329(NO);
        ADLaunchAppProbeLog7317(@"task-switcher.remove",[NSString stringWithFormat:@"reason=warm-icon-preflight windows=%lu pid=%ld",(unsigned long)removed,(long)pid]);
    });
    if(status==NOTIFY_STATUS_OK&&token>0){
        gADTaskSwitcherReleaseToken7334=token;
        ADLaunchAppProbeLog7317(@"task-switcher.release-listener",[NSString stringWithFormat:@"pid=%ld token=%d channel=%@",(long)pid,token,channel]);
    } else if(token>0) notify_cancel(token);
}

// Do not black every UIWindow during normal foreground use. Screenshot/share/input
// windows are intentionally excluded; only Amazon's primary navigation window receives
// the ordinary OLED backing. The snapshot cover above exists only while backgrounded.
%hook UIWindow
- (void)setRootViewController:(UIViewController *)vc {
    %orig;
    if(gP.enabled && ADPrimaryAmazonWindow713(self,vc)) ADSetViewBackground7226(self,ADOLED(),YES);
    if(gP.enabled&&gADTaskSwitcherCoverActive7329)ADAttachTaskSwitcherCover7329(self);
}
...
// from an actual scene reconstruction inside the same process. SpringBoard now owns only
// the v7.334 cold first-frame shim; warm behavior is owned inside Amazon only.
//
// Stock-like contract:
...
// v7.334 probe-only app-side launch recorder. Event timestamps use system uptime so they
// correlate directly with the SpringBoard launch recorder without relying on wall-clock ordering.
...
static NSString *ADLaunchAppProbePath7317(void){
    static NSString *p; static dispatch_once_t once; dispatch_once(&once,^{NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];p=[(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:@"AmazonDark-v7.334-launch-app-probe.txt"];}); return p;
}
static void ADLaunchAppProbeLog7317(NSString *event,NSString *detail){
    @try {
...
static void ADOwnAmazonSplash7307(UIViewController *vc){
    ...
    @try {
        ...
        // own its earliest floor dark. v7.334 SpringBoard bridges only the pre-process frame.
        vc.view.hidden=NO;
        vc.view.alpha=1.0;
...
static void ADInstallWarmResumeLifecycle7307(void){
    ...
    @try {
        ADInstallTaskSwitcherWarmRelease7334();
        NSNotificationCenter *nc=[NSNotificationCenter defaultCenter];
        // v7.334: the first task-switcher card can be captured while Amazon is only
        // resigning active, before DidEnterBackground. Put the opaque snapshot owner
        // into the app hierarchy synchronously at that earlier boundary.
        [nc addObserverForName:UIApplicationWillResignActiveNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(__unused NSNotification *n){
            NSUInteger epoch=++gADTaskSwitcherEpoch7334;
            NSUInteger covered=gP.enabled?ADSetTaskSwitcherCoverActive7329(YES):0;
            @try { [CATransaction flush]; } @catch(...) {}
            ADLaunchAppProbeLog7317(@"task-switcher.attach",[NSString stringWithFormat:@"reason=will-resign-active windows=%lu logo=%d epoch=%lu",(unsigned long)covered,ADTaskSwitcherLogo7329()?1:0,(unsigned long)epoch]);
            ADLaunchAppProbeLog7317(@"UIApplicationWillResignActive",[NSString stringWithFormat:@"ever=%d bg=%d snapshotCovers=%lu epoch=%lu",gADLifecycleEverActive7307?1:0,gADLifecycleBackgrounded7307?1:0,(unsigned long)covered,(unsigned long)epoch]);
            // Give UIKit one committed dark frame for the switcher capture, then get the
            // snapshot-only view out of the live hierarchy while Amazon is still inactive.
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(kADTaskSwitcherCaptureWindow7334*NSEC_PER_SEC)),dispatch_get_main_queue(),^{
                if(epoch!=gADTaskSwitcherEpoch7334||gADLifecycleBackgrounded7307)return;
                NSUInteger removed=ADSetTaskSwitcherCoverActive7329(NO);
                ADLaunchAppProbeLog7317(@"task-switcher.retire",[NSString stringWithFormat:@"reason=capture-window-complete windows=%lu epoch=%lu",(unsigned long)removed,(unsigned long)epoch]);
            });
        }];
        [nc addObserverForName:UIApplicationDidEnterBackgroundNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(__unused NSNotification *n){
            ++gADTaskSwitcherEpoch7334;
            // Reassert/raise the same cover at the committed background boundary.
            // This is idempotent and keeps newly-created full-size windows covered too.
            NSUInteger covered=gP.enabled?ADSetTaskSwitcherCoverActive7329(YES):0;
            ADLaunchAppProbeLog7317(@"task-switcher.attach",[NSString stringWithFormat:@"reason=did-enter-background windows=%lu logo=%d",(unsigned long)covered,ADTaskSwitcherLogo7329()?1:0]);
            ADLaunchAppProbeLog7317(@"UIApplicationDidEnterBackground",[NSString stringWithFormat:@"ever=%d snapshotCovers=%lu",gADLifecycleEverActive7307?1:0,(unsigned long)covered]);
        ...
        [nc addObserverForName:UISceneWillConnectNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(__unused NSNotification *n){
            ...
            ADLaunchAppProbeLog7317(@"UISceneWillConnect",[NSString stringWithFormat:@"ever=%d bg=%d",gADLifecycleEverActive7307?1:0,gADLifecycleBackgrounded7307?1:0]);
        ...
        [nc addObserverForName:UIApplicationWillEnterForegroundNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(__unused NSNotification *n){
            ++gADTaskSwitcherEpoch7334;
            NSUInteger removed=ADSetTaskSwitcherCoverActive7329(NO);
            ADLaunchAppProbeLog7317(@"task-switcher.remove",[NSString stringWithFormat:@"windows=%lu",(unsigned long)removed]);
            ...
            ADLaunchAppProbeLog7317(@"UIApplicationWillEnterForeground",[NSString stringWithFormat:@"ever=%d bg=%d reconnect=%d ordinaryWarm=%d",gADLifecycleEverActive7307?1:0,gADLifecycleBackgrounded7307?1:0,gADSceneReconnectedWhileBackgrounded7307?1:0,gADOrdinaryWarmResume7307?1:0]);
        }];
        [nc addObserverForName:UIApplicationDidBecomeActiveNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(__unused NSNotification *n){
            // WillResignActive also fires for transient interruptions that never reach
            // DidEnterBackground. In that case there is no WillEnterForeground callback,
            // so remove the snapshot-only cover here before returning to live use.
            if(!gADLifecycleBackgrounded7307&&gADTaskSwitcherCoverActive7329){
                ++gADTaskSwitcherEpoch7334;
                NSUInteger removed=ADSetTaskSwitcherCoverActive7329(NO);
                ADLaunchAppProbeLog7317(@"task-switcher.remove",[NSString stringWithFormat:@"reason=did-become-active-no-background windows=%lu",(unsigned long)removed]);
            }
            ADLaunchAppProbeLog7317(@"UIApplicationDidBecomeActive",[NSString stringWithFormat:@"before ever=%d bg=%d ordinaryWarm=%d",gADLifecycleEverActive7307?1:0,gADLifecycleBackgrounded7307?1:0,gADOrdinaryWarmResume7307?1:0]);
...
// -----------------------------------------------------------------------------
// Launch transition handoff retained in v7.334. The SpringBoard side keeps the
// 1.40 s minimum and 0.55 s fade; Amazon releases it only after the bounded,
// stable real-Home check below. Do not purge SplashBoard's cache: iOS owns the
// pre-process launch surface, and the scene overlay now covers that surface.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// v7.334 launch hooks are owned by the exact Amazon splash controllers and the
...
static NSString *ADProbePath7233(NSUInteger run){
    @try {
        ...
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.334-person-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];
    ...
    } @catch(...) { return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.334-person-ui-probe-r%lu.txt",(unsigned long)run]]; }
}
...
static NSString *ADCartProbePath7241(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.334-cart-transition-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.334-cart-transition-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADMenuProbePath7252(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.334-menu-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.334-menu-ui-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADAlexaProbePath7269(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.334-alexa-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.334-alexa-ui-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADPersonSubmenuProbePath7298(NSUInteger run){
    @try {
        ...
        NSString *name=[NSString stringWithFormat:@"AmazonDark-v7.334-person-submenu-hybrid-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];
        return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) {
        return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.334-person-submenu-hybrid-probe-r%lu.txt",(unsigned long)run]];
    }
}
...
static NSString *ADHomeFrameProbePath7265(NSUInteger run){
    @try {
        NSDateFormatter *f=[NSDateFormatter new]; f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"]; f.timeZone=[NSTimeZone localTimeZone]; f.dateFormat=@"yyyyMMdd-HHmmss-SSS";
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.334-home-frame-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject]; return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) { return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.334-home-frame-probe-r%lu.txt",(unsigned long)run]]; }
}
...
static NSString *ADProductScrollProbePath7272(NSUInteger run){
    @try {
        NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.334-product-scroll-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.334-product-scroll-probe-r%lu.txt",(unsigned long)run]];}
}
