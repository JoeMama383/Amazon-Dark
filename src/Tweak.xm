...
static NSString *ADDarkReaderBootstrap(void){
    ...
    gADBootstrap613 = [NSString stringWithFormat:
        @"(function(){try{"
         ...
         "try{var _v64=document.getElementById('advidwrap6064');if(_v64&&_v64.parentNode)_v64.parentNode.removeChild(_v64);var _v62=document.getElementById('advidctl6062');if(_v62&&_v62.parentNode)_v62.parentNode.removeChild(_v62);"
           "function adVSem6065(e,lim){try{var s='',p=e,d=0;while(p&&d++<(lim||3)){var c=p.className;c=String(c&&c.baseVal!==undefined?c.baseVal:(c||''));s+=' '+c+' '+String((p.getAttribute&&p.getAttribute('aria-label'))||(p.getAttribute&&p.getAttribute('title'))||'');p=p.parentElement;}return s;}catch(x){return '';}}"
           "function adVCtls6065(host,re,ban){try{var A=host&&host.querySelectorAll?host.querySelectorAll('button,[role=button],[aria-label],[title],svg,i,[class*=play],[class*=pause],[class*=mute],[class*=volume]'):[],O=[];for(var i=0;i<A.length&&i<180;i++){var e=A[i],sem=adVSem6065(e,3);if(ban&&ban.test(sem))continue;if(!re.test(sem))continue;var q=(e.closest&&e.closest('button,[role=button]'))||e,r=q.getBoundingClientRect();if(r.width<20||r.width>96||r.height<20||r.height>96)continue;if(O.indexOf(q)<0)O.push(q);}return O;}catch(x){return [];}}"
           "function adVAlpha6065(c){try{c=String(c||'').replace(/\s+/g,'').toLowerCase();if(!c||c==='transparent'||c==='rgba(0,0,0,0)')return 0;var m=c.match(/^rgba?\((\d+),(\d+),(\d+)(?:,([0-9.]+))?\)$/);return m?(m[4]===undefined?1:parseFloat(m[4])):1;}catch(x){return 0;}}"
           "function adVShell6065(q){try{if(!q)return null;var Q=[q],D=q.querySelectorAll?q.querySelectorAll('*'):[];for(var i=0;i<D.length&&i<24;i++)Q.push(D[i]);var best=null,score=-1;for(var j=0;j<Q.length;j++){var e=Q[j],r=e.getBoundingClientRect();if(r.width<24||r.width>86||r.height<24||r.height>86)continue;if(Math.abs(r.width-r.height)>12)continue;var cs=getComputedStyle(e),a=adVAlpha6065(cs.backgroundColor),rad=parseFloat(cs.borderTopLeftRadius)||0,sc=(rad>=Math.min(r.width,r.height)*.28?5:0)+(a>.02?2:0)-(Math.abs(r.width-r.height)/20)-(r.width/240);if(sc>score){score=sc;best=e;}}return best;}catch(x){return null;}}"
           "function adVPair6065(host,vr){try{var M=adVCtls6065(host,/mute|unmute|volume|sound/i,/play|pause|caption|fullscreen/i),P=adVCtls6065(host,/play|pause/i,/mute|volume|sound|caption|fullscreen/i);if(!M.length||!P.length)return null;var mute=null,mScore=1e15;for(var i=0;i<M.length;i++){var mr=M[i].getBoundingClientRect(),mx=mr.left+mr.width/2,my=mr.top+mr.height/2;if(mx<vr.left-24||mx>vr.right+24||my<vr.top-24||my>vr.bottom+24)continue;var rightPenalty=Math.max(0,(vr.left+vr.width*.58)-mx);var sc=rightPenalty*rightPenalty+Math.abs((vr.right)-mx);if(sc<mScore){mScore=sc;mute=M[i];}}if(!mute)return null;var mr=mute.getBoundingClientRect(),mx=mr.left+mr.width/2,my=mr.top+mr.height/2,play=null,pScore=1e15;for(var j=0;j<P.length;j++){var pr=P[j].getBoundingClientRect(),px=pr.left+pr.width/2,py=pr.top+pr.height/2,dx=Math.abs(px-mx),dy=Math.abs(py-my);if(px<vr.left-24||px>vr.right+24||py<vr.top-24||py>vr.bottom+24)continue;if(dx>110||dy>180)continue;var sc2=dx*dx+dy*dy+(py>my?2500:0);if(sc2<pScore){pScore=sc2;play=P[j];}}return play?{play:play,mute:mute}:null;}catch(x){return null;}}"
           "function adVScrub6065(q){try{if(!q)return; q.setAttribute('data-ad-native615','1');if(window.__AD_STRIP_DR615__)window.__AD_STRIP_DR615__(q);var E=[q],D=q.querySelectorAll?q.querySelectorAll('*'):[];for(var i=0;i<D.length&&i<36;i++)E.push(D[i]);for(var j=0;j<E.length;j++){var e=E[j],st=e.style;if(!st)continue;var f=String(st.getPropertyValue('filter')||'').replace(/\s+/g,'').toLowerCase();if(st.getPropertyPriority('filter')==='important'&&f==='brightness(0)invert(1)')st.removeProperty('filter');if(e.__adGlyph){delete e.__adGlyph;}var by=String(e.__adBy||'');if(/^gfix/.test(by)||by==='videoCtl6062')delete e.__adBy;if(e.removeAttribute){e.removeAttribute('data-ad-videoctl6062');e.removeAttribute('data-ad-videowrap6064');e.removeAttribute('data-ad-videostock6064');}}}catch(x){}}"
           "function adVMatchShells6065(play,mute){try{var ps=adVShell6065(play),ms=adVShell6065(mute);if(!ps||!ms)return 0;var bg='rgba(255,255,255,0.38)';ps.style.setProperty('background-color',bg,'important');ms.style.setProperty('background-color',bg,'important');ps.style.setProperty('border-radius','50%','important');ms.style.setProperty('border-radius','50%','important');ps.setAttribute('data-ad-videoshell6065','1');ms.setAttribute('data-ad-videoshell6065','1');return 1;}catch(x){return 0;}}"
           "window.__AD_VIDEOSTOCK_ONE6065__=function(v){try{if(!v||String(v.tagName||'').toUpperCase()!=='VIDEO')return 0;var vr=v.getBoundingClientRect();if(vr.width<100||vr.height<70)return 0;var host=v,pd=0;while(host.parentElement&&pd++<4){var hp=host.parentElement,hr=hp.getBoundingClientRect();if(hr.width>=vr.width*.70&&hr.width<=vr.width*1.70&&hr.height>=vr.height*.70&&hr.height<=vr.height*1.75)host=hp;else break;}var pair=adVPair6065(host,vr);if(!pair)return 0;adVScrub6065(pair.play);adVScrub6065(pair.mute);pair.play.setAttribute('data-ad-videostock6065','1');pair.mute.setAttribute('data-ad-videostock6065','1');return adVMatchShells6065(pair.play,pair.mute);}catch(e){return 0;}};"
           "window.__AD_VIDEOSTOCK6065__=function(root){try{var V=[],n=0;function addFrom(b){if(!b)return;try{if(String(b.tagName||'').toUpperCase()==='VIDEO'&&V.indexOf(b)<0)V.push(b);var q=b.querySelectorAll?b.querySelectorAll('video'):[];for(var i=0;i<q.length&&V.length<36;i++)if(V.indexOf(q[i])<0)V.push(q[i]);}catch(x){}}addFrom(root);if(!V.length&&root&&root.nodeType===1){var p=root.parentElement,d=0;while(p&&d++<4&&!V.length){addFrom(p);p=p.parentElement;}}if(!V.length&&root===document)addFrom(document);for(var j=0;j<V.length&&j<36;j++)n+=window.__AD_VIDEOSTOCK_ONE6065__(V[j]);return n;}catch(e){return 0;}};"
           "var vcs65=function(ev){try{var t=ev&&ev.target;if(t&&String(t.tagName||'').toUpperCase()==='VIDEO'){window.__AD_VIDEOSTOCK_ONE6065__(t);setTimeout(function(){window.__AD_VIDEOSTOCK_ONE6065__(t);},90);}}catch(e){}};document.addEventListener('loadedmetadata',vcs65,true);document.addEventListener('loadeddata',vcs65,true);document.addEventListener('canplay',vcs65,true);document.addEventListener('play',vcs65,true);document.addEventListener('playing',vcs65,true);document.addEventListener('pause',vcs65,true);"
           "document.addEventListener('click',function(ev){try{var p=ev&&ev.target,d=0,sem='';while(p&&d++<4){var c=p.className;c=String(c&&c.baseVal!==undefined?c.baseVal:(c||''));sem+=' '+c+' '+String((p.getAttribute&&p.getAttribute('aria-label'))||(p.getAttribute&&p.getAttribute('title'))||'');p=p.parentElement;}if(!/play|pause|mute|unmute|volume|sound/i.test(sem)||/caption|fullscreen/i.test(sem))return;setTimeout(function(){window.__AD_VIDEOSTOCK6065__(document);},0);setTimeout(function(){window.__AD_VIDEOSTOCK6065__(document);},120);}catch(e){}},true);"
           "var vis65=function(){try{window.__AD_VIDEOSTOCK6065__(document);}catch(e){}};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',vis65,{once:true});else vis65();window.addEventListener('pageshow',vis65,{passive:true});}catch(e){}"
