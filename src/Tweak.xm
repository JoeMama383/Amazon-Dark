...
static NSString *ADProbeWebJS(void){
    return
       ...
       // P17HEART (v5.346): verify the original Amazon paint leaf itself.
       "try{var h17=document.querySelector('[class*=lists-treatment-hear] .a-icon');if(h17){var r17=h17.getBoundingClientRect(),s17=getComputedStyle(h17);out.push('P17HEART[node=.a-icon@'+Math.round(r17.width)+'x'+Math.round(r17.height)+' bgi='+(s17.backgroundImage&&s17.backgroundImage!=='none'?'Y':'-')+' filter='+String(s17.filter||'none').replace(/\\s+/g,' ').slice(0,72)+' fixed='+(h17.getAttribute('data-ad-heartfix')||'-')+']');}else out.push('P17HEART[node=none]');}catch(e17){out.push('P17HEART[err '+(e17&&e17.message||e17)+']');}"
       ...
       "try{(function(){"
         ...
         // v5.346 PDP HEART. v5.345's existing HEART probe finally named the real
         // Amazon painter: a 20x20 .a-icon background image under the
         // lists-treatment-heart shell. Repair that ORIGINAL leaf after hydration;
         // never recolour/replace its 24x24 parents and never synthesize an overlay.
         "function heartFix(){try{var H=document.querySelectorAll('[class*=lists-treatment-hear] .a-icon'),n=0;for(var i=0;i<H.length&&i<20;i++){var h=H[i],r=h.getBoundingClientRect(),cs=getComputedStyle(h);if(r.width<12||r.width>28||r.height<12||r.height>28)continue;if(!cs.backgroundImage||cs.backgroundImage==='none')continue;var f=String(cs.filter||'none'),mk=h.getAttribute&&h.getAttribute('data-ad-heartfix');if(mk!=='1'||f.indexOf('brightness(0)')<0||f.indexOf('invert(1)')<0){h.style.setProperty('filter','brightness(0) invert(1)','important');h.style.setProperty('background-color','transparent','important');h.setAttribute('data-ad-heartfix','1');}n++;}window.__AD_HEARTFIX__=n;}catch(e){window.__AD_HEARTFIX__=-1;}}"
         ...
         "}catch(e){}}""badgeFix();dotFix();packFix();heartFix();""clr();setTimeout(clr,300);setTimeout(clr,1200);setTimeout(clr,2500);""setTimeout(badgeFix,300);setTimeout(badgeFix,1200);setTimeout(badgeFix,2500);""setTimeout(dotFix,120);setTimeout(dotFix,500);setTimeout(dotFix,1400);setTimeout(dotFix,2800);""setTimeout(packFix,120);setTimeout(packFix,500);setTimeout(packFix,1400);setTimeout(packFix,2800);""setTimeout(heartFix,120);setTimeout(heartFix,500);setTimeout(heartFix,1400);setTimeout(heartFix,2800);""addEventListener('scroll',function(){clearTimeout(window.__bgT);window.__bgT=setTimeout(function(){badgeFix();dotFix();packFix();heartFix();},120);},{passive:true,capture:true});""addEventListener('scroll',function(){clearTimeout(window.__bxS);window.__bxS=setTimeout(clr,120);},{passive:true,capture:true});"
         "new MutationObserver(function(){clearTimeout(window.__bxT);"
           "window.__bxT=setTimeout(function(){clr();dotFix();packFix();heartFix();},60);}).observe(document.documentElement,"
