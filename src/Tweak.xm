...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         // v5.407 TWO-CARDS RESTORATION: no new cards painter exists here.
         // The exact preserved v5.333 authority above is intentionally the only final
         // owner for [class*=lists-framework-action-button]. Heart, checkbox and arrow
         // remain on their already-confirmed current paths.
...
"/*V5317FIX*//*V5376FIX*//*V5377FIX*//*V5378FIX*//*V5379FIX*//*V5380FIX*//*V5381FIX*//*V5382FIX*//*V5383FIX*//*V5384FIX*//*V5385FIX*//*V5386FIX*//*V5387FIX*//*V5388FIX*//*V5391FIX*//*V5393FIX*/"
       ...
       // P77CARDS408: passive read of the literal historical action-button host. No paint.
       "try{var C77=document.querySelectorAll('[class*=lists-framework-action-button]'),X77=document.querySelectorAll('[class*=mlt-icon-container],[role=checkbox],input[type=checkbox],[class*=a-icon-checkbox],[data-ad-compare380],[data-ad-comparelegacy387]'),raw77=C77.length,n77=0,bg77=0,ring77=0,shape77=0,glyph77=0,cb77=0,ov77=0,Q77=[];function cen77(e){var r=e.getBoundingClientRect();return [r.left+r.width/2,r.top+r.height/2,r.width,r.height];}for(var i77=0;i77<C77.length&&i77<80;i77++){var e77=C77[i77],r77=e77.getBoundingClientRect();if(r77.width<18||r77.width>52||r77.height<18||r77.height>52||Math.abs(r77.width-r77.height)>10)continue;n77++;if(e77.closest&&e77.closest('[class*=mlt-icon-container],[role=checkbox],input[type=checkbox],[class*=a-icon-checkbox],[data-ad-compare380],[data-ad-comparelegacy387]'))cb77++;var s77=getComputedStyle(e77),bc77=String(s77.backgroundColor||'').replace(/\\s+/g,''),bw77=parseFloat(s77.borderTopWidth||'0'),br77=parseFloat(s77.borderTopLeftRadius||'0');if(bc77!=='rgb(24,26,27)'&&bc77!=='rgba(24,26,27,1)')bg77++;if(Math.abs(bw77-1.5)>.26)ring77++;if(br77<Math.min(r77.width,r77.height)*.40)shape77++;var G77=e77.querySelectorAll('img,i,svg,path,use,polygon,[class*=lists-framework-unfill],[class*=lists-framework-fill]'),light77=0;for(var j77=0;j77<G77.length;j77++){var gs77=getComputedStyle(G77[j77]),f77=String(gs77.filter||''),ink77=String(gs77.color||'')+'|'+String(gs77.fill||'')+'|'+String(gs77.stroke||'');if(f77.indexOf('invert')>=0||/rgb\\(255,\\s*255,\\s*255\\)|#fff/i.test(ink77)){light77=1;break;}}if(!light77)glyph77++;var cc77=cen77(e77);for(var x77=0;x77<X77.length&&x77<240;x77++){var xc77=cen77(X77[x77]);if(xc77[2]<8||xc77[3]<8||xc77[2]>60||xc77[3]>60)continue;if(Math.abs(cc77[0]-xc77[0])<16&&Math.abs(cc77[1]-xc77[1])<16){ov77++;break;}}if(Q77.length<8)Q77.push(Math.round(r77.width)+'x'+Math.round(r77.height)+'|bg='+bc77+'|bw='+bw77+'|br='+br77+'|g='+G77.length+'|light='+light77);}var m77=document.querySelectorAll('[data-ad-product391=\"cards\"],[data-ad-stockaction379],[data-ad-stockaction382],[data-ad-stockaction384],[data-ad-stockaction387],[data-ad-stockroot388],[data-ad-stockdisc388]').length;out.push('P77CARDS408[raw='+raw77+' n='+n77+' badBg='+bg77+' badRing='+ring77+' badShape='+shape77+' badGlyph='+glyph77+' inCheckbox='+cb77+' overlap='+ov77+' modern='+m77+' disabled='+String(window.__AD_CARDS391_DISABLED408__||0)+' disc='+String(window.__AD_DISC__||'-')+(Q77.length?' '+Q77.join(' ~~ '):'')+']');}catch(e77){out.push('P77CARDS408[err '+(e77&&e77.message||e77)+']');}"
