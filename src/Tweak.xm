...
#define AD_VERSION "v7.386-sponsored-shell-ownership"
...
// v7.386 probe-backed Home dashboard shell floor. This remains in the single
...
// v7.386: Sponsored-content filtering follows AmznKiller's selector-level
...
// v7.386 build boundary: the large sponsored CSS payload is compiled as plain
...
static void ADAttachScriptsToUCC710(WKUserContentController *ucc){
    ...
    @try {
        ...
        if(gP.hideSponsored && !objc_getAssociatedObject(ucc,kADKillerUS7382)){
            WKUserScript *us=[[WKUserScript alloc] initWithSource:ADKillerSponsoredJS7384() injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
            [ucc addUserScript:us];
