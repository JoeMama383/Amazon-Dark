...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{(function(){"
         ...
         // v5.439 DEVICE-CAPTURED STOCK CHECKBOX. Amazon remains the sole owner of
         ...
         "function stockCheckbox434(){if(window.__ADFRAME_MODE__||!document.body||window.__AD_CHECKBOX434_RUNNING__)return 0;window.__AD_CHECKBOX434_RUNNING__=1;try{"
           ...
           "var prior434=document.getElementById('adcheckbox434');if(prior434&&prior434.getAttribute('data-ad-native-state')!=='439'){if(prior434.parentNode)prior434.parentNode.removeChild(prior434);prior434=null;}"
           "if(!prior434){var s434=document.createElement('style');s434.id='adcheckbox434';s434.setAttribute('data-ad-native-state','439');"
           ...
           "function group434(e){if(!e||!scope434(e)||foreign434(e))return null;var tg0=String(e.tagName||'').toUpperCase(),p=/^(IMG|I|INPUT|SVG|PATH|USE|POLYGON)$/.test(tg0)?e.parentElement:e,sem=null,best=null,u=0;while(p&&u++<8){if(foreign434(p))return null;var tg=String(p.tagName||'').toUpperCase();if(p.matches&&p.matches(semanticSel434)&&sq434(p,16,76))sem=p;if(!best&&!/^(IMG|I|INPUT|SVG|PATH|USE|POLYGON)$/.test(tg)&&sq434(p,16,76))best=p;if(p.matches&&p.matches(scopeSel434))break;p=p.parentElement;}return sem||best||((tg0==='INPUT')?e:(e.parentElement||e));}"
           ...
           "var stale434=document.querySelectorAll('[data-ad-checkbox434-host],[data-ad-checkbox434-art],[data-ad-checkbox434-shell]');for(var st434=0;st434<stale434.length;st434++){stale434[st434].removeAttribute('data-ad-checkbox434-host');stale434[st434].removeAttribute('data-ad-checkbox434-art');stale434[st434].removeAttribute('data-ad-checkbox434-shell');}"
           "var shells434=[],hosts434=[],unchecked434=0,checked434=0,cleaned434=0,skip434=0,cartShell434=0;"
           "function shell434(h,art){if(!cart434||!h||!art)return;var p=h,u=0;while(p&&u++<4){var tg=String(p.tagName||'').toUpperCase();if(p!==art&&p.contains&&p.contains(art)&&sq434(p,18,76)&&!/^(IMG|I|INPUT|SVG|PATH|USE)$/.test(tg)&&!visual434(p)){p.setAttribute('data-ad-checkbox434-shell','cart');shells434.push(p);cartShell434++;}if(p.matches&&p.matches(scopeSel434))break;p=p.parentElement;}}"
