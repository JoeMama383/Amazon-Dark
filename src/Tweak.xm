...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{(function(){"
         ...
         // v5.438 NATIVE-STATE STOCK CHECKBOX. Amazon remains the sole owner of
         ...
         // The unchecked-only invert is selected synchronously by Amazon's own
         // input/ARIA/class state in CSS, so a recycled Shopping row cannot show an
         // orange frame while a timer catches up. Cart-only paint shells are cleared
         // only when they contain no sprite/background art of their own.
         "function stockCheckbox434(){if(window.__ADFRAME_MODE__||!document.body||window.__AD_CHECKBOX434_RUNNING__)return 0;window.__AD_CHECKBOX434_RUNNING__=1;try{"
           ...
           "var prior434=document.getElementById('adcheckbox434');if(prior434&&prior434.getAttribute('data-ad-native-state')!=='438'){if(prior434.parentNode)prior434.parentNode.removeChild(prior434);prior434=null;}"
           "if(!prior434){var s434=document.createElement('style');s434.id='adcheckbox434';s434.setAttribute('data-ad-native-state','438');"
             "s434.textContent='[data-ad-checkbox434-host]{--ad-checkbox434-filter:invert(1);}'"
               ...
               "+'[data-ad-checkbox434-art]{filter:var(--ad-checkbox434-filter,invert(1)) !important;}'"
         ...
         "/* v5.438: no checkbox state emulation; stockCheckbox434 only discovers stock hosts/art. */"
       "})();}catch(e){}"
...
"/*V5317FIX*//*V5376FIX*//*V5377FIX*//*V5378FIX*//*V5379FIX*//*V5380FIX*//*V5381FIX*//*V5382FIX*//*V5383FIX*//*V5384FIX*//*V5385FIX*//*V5386FIX*//*V5387FIX*//*V5388FIX*//*V5391FIX*//*V5393FIX*/"
       ...
       // P85CHECKBOX434: reports the v5.438 native-state contract. The host/art
       ...
       "function on85(a){try{var h=(a.closest&&a.closest('[data-ad-checkbox434-host]'))||a,q=(h.matches&&h.matches('input[type=checkbox]'))?h:h.querySelector('input[type=checkbox]');if(q)return !!q.checked;var Z=[h],D=h.querySelectorAll?h.querySelectorAll('[role=checkbox],[aria-checked],[aria-pressed],[aria-selected],[data-checked],[data-selected],[data-state]'):[];for(var d85=0;d85<D.length&&d85<32;d85++)Z.push(D[d85]);for(var z85=0;z85<Z.length;z85++){var e85=Z[z85],v85=String(e85.getAttribute('aria-checked')||e85.getAttribute('aria-pressed')||e85.getAttribute('aria-selected')||e85.getAttribute('data-checked')||e85.getAttribute('data-selected')||e85.getAttribute('data-state')||'').toLowerCase(),c85=String(e85.className&&e85.className.baseVal!==undefined?e85.className.baseVal:(e85.className||'')).toLowerCase();if(v85==='true'||v85==='checked'||v85==='on'||(/checked|selected/.test(c85)&&!/unchecked|unselected/.test(c85)))return true;if(v85==='false'||v85==='unchecked'||v85==='off')return false;}var s85=String(a.currentSrc||a.src||a.getAttribute('data-src')||'').toLowerCase();return /checkbox[_-]?(?:on|checked)|checkmark|selected/.test(s85)&&!/unchecked|unselected/.test(s85);}catch(x85){return false;}}"
       "for(var i85=0;i85<A85.length&&i85<120;i85++){var a85=A85[i85],on=on85(a85),f85=String(getComputedStyle(a85).filter||'none');if(on){checked85++;if(f85!=='none')badFilter85++;}else{unchecked85++;if(f85.indexOf('invert')<0)badFilter85++;}if(a85.closest&&a85.closest('[class*=mlt-icon-container],[class*=lists-framework-action-button],[class*=puis-heart-position],[class*=puis-mab-chevron]'))foreign85++;if(Q85.length<10){var r85=a85.getBoundingClientRect(),c85=a85.className;c85=String(c85&&c85.baseVal!==undefined?c85.baseVal:(c85||''));Q85.push((on?'on':'off')+'|'+a85.tagName+'.'+c85.slice(0,28)+'@'+Math.round(r85.width)+'x'+Math.round(r85.height)+'|f='+f85.slice(0,30));}}"
       ...
       "out.push('P85CHECKBOX434[art='+A85.length+' unchecked='+unchecked85+' checked='+checked85+' badFilter='+badFilter85+' legacy='+legacy85+' foreign='+foreign85+' miss='+miss85+' cartShell='+S85.length+' artBad='+artBad85+' shellGeom='+shellGeom85+' timer='+timer85+' pane='+pane85+' state='+String(window.__AD_CHECKBOX434_STATE__||'-')+(Q85.length?' '+Q85.join(' ~~ '):'')+']');"
