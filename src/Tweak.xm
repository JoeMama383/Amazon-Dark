...
static NSString *ADFixesLiteral(void){
    ...
    gADFixesLiteral613 = [NSString stringWithFormat:
            @"{css:'"
             ...
             "{display:none !important;filter:none !important;opacity:0 !important;}"
             // v6.0.105: give the Search-result Heart the same one-owner first-frame
             // treatment as the proven v6.0.104 two-cards control.  The real 32px
             // action-button host paints the finished dark disc + white Heart from
             // CSS immediately; Amazon's lazy IMG/I/SVG/fill-state children remain
             // visually hidden so hydration cannot create a second render cycle.
    ...
    gADBootstrap613 = [NSString stringWithFormat:
        @"(function(){try{"
         ...
         "try{if(!document.getElementById('adfloor612')){var f=document.createElement('style');"
           ...
           "{opacity:0 !important;filter:none !important;background-color:transparent !important;"
           "transition:none !important;animation:none !important;}"
           // v6.0.105: exact same first-frame ownership for the Heart action button.
           // Duplicate it here, before page paint, and in ADFixesLiteral above so
           // Dark Reader never introduces an intermediate Heart/placeholder frame.
           "[class*=lists-framework-action-button]"
           "{background-color:#181a1b !important;border:1.5px solid rgba(255,255,255,.65) !important;"
           "border-radius:50%% !important;box-shadow:none !important;box-sizing:border-box !important;"
           "background-image:url(data:image/svg+xml;base64,) !important;"
         ...
         "function adfD6101(e){try{var c=getComputedStyle(e),r=e.getBoundingClientRect(),b=getComputedStyle(e,'::before'),a=getComputedStyle(e,'::after'),src=String((e.currentSrc||e.src||(e.getAttribute&&e.getAttribute('data-src'))||''));return{tag:e.tagName||'',cl:adfS6101(e.className&&e.className.baseVal||e.className,180),r:[Math.round(r.x),Math.round(r.y),Math.round(r.width),Math.round(r.height)],bg:adfS6101(c.backgroundColor,70),bgi:adfS6101(c.backgroundImage,160),mask:adfS6101(c.maskImage||c.webkitMaskImage,120),bs:adfS6101(c.boxShadow,120),bd:adfS6101(c.border,120),bc:adfS6101(c.borderColor,90),ol:adfS6101(c.outline,120),oc:adfS6101(c.outlineColor,90),col:adfS6101(c.color,70),f:adfS6101(c.filter,100),op:adfS6101(c.opacity,30),disp:adfS6101(c.display,40),vis:adfS6101(c.visibility,40),src:adfS6101(src,180),sty:adfS6101(e.getAttribute&&e.getAttribute('style'),220),h6105:e.getAttribute&&e.getAttribute('data-ad-heart6105'),hs427:e.getAttribute&&e.getAttribute('data-ad-heart-shell427'),sym:e.getAttribute&&e.getAttribute('data-ad-sym413'),by:adfS6101(e.__adBy||'',80),bef:[adfS6101(b.backgroundColor,70),adfS6101(b.backgroundImage,120),adfS6101(b.boxShadow,120),adfS6101(b.content,80)],aft:[adfS6101(a.backgroundColor,70),adfS6101(a.backgroundImage,120),adfS6101(a.boxShadow,120),adfS6101(a.content,80)],txt:adfS6101(String(e.textContent||'').replace(/\\s+/g,' ').trim(),160)};}catch(x){return{err:String(x)}}}"
         "function adfT6101(e){try{if(!e||e.nodeType!==1)return null;var S='[class*=puis-heart-position],[class*=lists-framework-action-button],[class*=lists-framework-heart],[data-csa-c-content-id=variation-options-link],[class*=s-variations-options-justify-content],[class*=s-variation-options-text],[class*=s-variation-options-link],[class*=s-color-swatch-container-list-view],.s-color-swatch-container,.s-color-swatch-outer-circle,[class*=puis-csi-with-label-container],[data-component-type=s-status-badge-component],.puis-status-badge-container,.a-badge-region,[class*=badgeLabel]';if(e.matches&&e.matches(S))return e;if(e.closest){var q=e.closest(S);if(q)return q;}if(e.querySelector){var d=e.querySelector(S);if(d)return d;}var c=String(e.className&&e.className.baseVal||e.className||'');if(!/(heart|lists-framework|variation|swatch|puis-csi|puis-cs-label|text-wrapper|rush-component|a-truncate|a-badge-region|badgeLabel|badge)/i.test(c))return null;var t=String(e.textContent||'').replace(/\\s+/g,' ').trim();if(t.length<600&&(/other colors?\\/patterns?/i.test(t)||/^color\\s*:/i.test(t)||/color:\\s*color:/i.test(t)||/limited time deal/i.test(t)))return e;return null;}catch(x){return null;}}"
         "function adfSnap6101(why,e){try{var q=adfT6101(e);if(!q||!q.isConnected)return;var R=q.getBoundingClientRect();if(R.width<20||R.height<5||R.width>460||R.height>180)return;var isH=/puis-heart-position|lists-framework-action-button|lists-framework-heart/i.test(String(q.className&&q.className.baseVal||q.className||''));var rec={t:Date.now(),why:why,url:String(location.href),target:adfD6101(q),bright:[],kids:[]},p=q,d=0;rec.chain=[];while(p&&d++<7){rec.chain.push(adfD6101(p));p=p.parentElement;}var A=q.querySelectorAll?q.querySelectorAll(isH?'*':'div,span,a,label,section,button,ul,li'):[];for(var i=0;i<A.length&&i<(isH?36:220);i++){var x=A[i],rx=x.getBoundingClientRect();if(isH&&rec.kids.length<24)rec.kids.push(adfD6101(x));if(rx.width<3||rx.height<3||rx.width>460||rx.height>180)continue;var cx=getComputedStyle(x),L=adfL6101(cx.backgroundColor),BL=adfL6101(cx.borderColor),OL=adfL6101(cx.outlineColor),pb=adfL6101(getComputedStyle(x,'::before').backgroundColor),pa=adfL6101(getComputedStyle(x,'::after').backgroundColor);if(rec.bright.length<28&&(L>.32||BL>.55||OL>.55||pb>.32||pa>.32||String(cx.backgroundImage||'')!=='none'||String(cx.boxShadow||'')!=='none'))rec.bright.push(adfD6101(x));}window.__AD_FLASH6101__.push(rec);if(window.__AD_FLASH6101__.length>240)window.__AD_FLASH6101__.splice(0,window.__AD_FLASH6101__.length-240);}catch(x){}}"
         "window.__AD_FLASH_TOUCH6101__=function(n,why){try{if(!n)return;if(n.nodeType===3)n=n.parentElement;if(!n||n.nodeType!==1)return;var q=adfT6101(n);if(!q)return;var h=/puis-heart-position|lists-framework-action-button|lists-framework-heart/i.test(String(q.className&&q.className.baseVal||q.className||'')),w=why||'mut';adfSnap6101(w,q);setTimeout(function(){adfSnap6101(w+'+0',q);},0);setTimeout(function(){adfSnap6101(w+'+12',q);},12);setTimeout(function(){adfSnap6101(w+'+48',q);},48);if(h){setTimeout(function(){adfSnap6101(w+'+96',q);},96);setTimeout(function(){adfSnap6101(w+'+180',q);},180);}}catch(x){}};"
         "window.__AD_FLASH6101_DUMP__=function(){try{var S='[class*=puis-heart-position],[class*=lists-framework-action-button],[class*=lists-framework-heart],[data-csa-c-content-id=variation-options-link],[class*=s-variation-options-link],[class*=s-color-swatch-container-list-view],.s-color-swatch-container,.s-color-swatch-outer-circle,[class*=puis-csi-with-label-container],[data-component-type=s-status-badge-component],.puis-status-badge-container,.a-badge-region,[class*=badgeLabel]';var A=document.querySelectorAll(S);for(var i=0;i<A.length&&i<40;i++){var r=A[i].getBoundingClientRect();if(r.bottom>0&&r.top<innerHeight&&r.width>20&&r.height>4)adfSnap6101('dump-visible',A[i]);}return JSON.stringify({url:String(location.href),n:window.__AD_FLASH6101__.length,records:window.__AD_FLASH6101__});}catch(e){return'ERR '+String(e);}};"
...
static NSString *ADFlashProbePath6101(void){
    return [NSTemporaryDirectory() stringByAppendingPathComponent:@"AmazonDark-heart-firstpaint-probe-6106.txt"];
}
...
static void ADResetFlashProbe6101(void){
    @try {
        NSString *h=[NSString stringWithFormat:@"AmazonDark Heart first-paint lifecycle probe 6106\nversion=%s\npid=%d\n\n",AD_VERSION,getpid()];
