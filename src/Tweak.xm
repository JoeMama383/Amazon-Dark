/*
 * AmazonDark v7.303 — deterministic native/WebKit first-paint ownership
 *
 * Architecture:
...
#define AD_VERSION "v7.303-deterministic-first-paint"
...
static NSString *ADFloorJS(void){
    ...
    return
        ...
        @"cument.body.style.setProperty('background-color','#000','important');document.body.style.setProperty('color-scheme','dark','important');}}catch(_){}}if(child&&document.documentElement&&document.documentElement.hasAttribute('data-ad7104-standalone'))return;var p='';try{p=String(location.pathname||'');}catch(_){}var cartBoot=!child&&(p==='/cart'||p.indexOf('/cart/')===0||p.indexOf('/gp/cart')===0);if(cartBoot&&document.documentElement){var bh=document.documentElement;put('ad7303-cart-boot',\"html[data-ad7303-cart-boot]{background:#000!important;background-color:#000!important;color-scheme:dark!important;}html[data-ad7303-cart-boot]::before{content:''!important;position:fixed!important;inset:0!important;width:100vw!important;height:100vh!important;background:#000!important;background-color:#000!important;background-image:none!important;opacity:1!important;z-index:2147483647!important;pointer-events:none!important;}\");bh.setAttribute('data-ad7303-cart-boot','1');var releaseCartBoot=function(){try{bh.removeAttribute('data-ad7303-cart-boot');}catch(_){}};if(document.readyState==='complete')releaseCartBoot();else window.addEventListener('load',releaseCartBoot,{once:true});}var s=null;if(!child&&(p==='/autocomplete'||p.indexOf('/autocomplete/')===0)){s="
        ...
        // v7.303: a document-start opaque Cart boot shield is installed above before any
        // Amazon stylesheet or pre-hydration skeleton can composite. It is removed by the
        // one-shot load event only after this dark stylesheet is present. The settled DOM
        // ownership below remains as a defense after the shield leaves; there is no observer,
        // timer, polling, or recurring scan.
...
// v7.303: Amazon's native connectivity/error surface is the exact CNMErrorView
...
static const void *kADCNMTWB7303=&kADCNMTWB7303;
static const void *kADCNMSubNavGetter7303=&kADCNMSubNavGetter7303;
static const void *kADCNMSubNavWasHidden7303=&kADCNMSubNavWasHidden7303;
static __thread BOOL gADCNMImageWrite7301=NO;
static void ADEnsureCNMTWB7303(UIImageView *iv){
    if(!iv)return;
    @try {
        CALayer *ov=objc_getAssociatedObject(iv,kADCNMTWB7303);
        if(!gP.enabled||!gP.whiteTame||!iv.window){
            if(ov){[ov removeFromSuperlayer];objc_setAssociatedObject(iv,kADCNMTWB7303,nil,OBJC_ASSOCIATION_RETAIN_NONATOMIC);}
            return;
        }
        if(!ov){
            ov=[CALayer layer];
            ov.name=@"AmazonDarkCNMTWB7303";
            ...
            objc_setAssociatedObject(iv,kADCNMTWB7303,ov,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
static void ADOwnCNMErrorHero7301(UIImageView *iv){
    ...
    @try {
        ...
        if(objc_getAssociatedObject(iv,kADCNMProcessedImage7301)!=iv.image){
            ...
            // Cache both changed and already-transparent variants. The exact hero's
            // final layout hook below retries any assignment that arrived at zero bounds.
            objc_setAssociatedObject(iv,kADCNMProcessedImage7301,iv.image,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
        // Never let successful backdrop removal bypass media taming. v7.302 returned
        // before this layer when the processed image was already current.
        ADEnsureCNMTWB7303(iv);
...
// The error probe identifies the faint text above CNMErrorView as the live native
// ANXSubNavCollectionViewCell lane (y=88...135), not a CNM label and not WebKit.
// Hide that exact collection only while a CNM root is mounted, then restore its
// prior hidden state when the error surface leaves.
static UICollectionView *ADCNMStoredSubNav7303(UIView *root){
    @try {
        UICollectionView *(^getter)(void)=objc_getAssociatedObject(root,kADCNMSubNavGetter7303);
        UICollectionView *cv=getter?getter():nil;
        return (cv&&cv.superview)?cv:nil;
    } @catch(...) { return nil; }
}
static BOOL ADCNMHasSubNavCell7303(UIView *v){
    if(!v)return NO;
    @try {
        if(ADClassNameIs7183(v,"ANXSubNavCollectionViewCell"))return YES;
        for(UIView *c in v.subviews)if(ADClassNameIs7183(c,"ANXSubNavCollectionViewCell"))return YES;
    } @catch(...) {}
    return NO;
}
static UICollectionView *ADCNMFindSubNav7303(UIView *root){
    ...
    @try {
        ...
        while(head<q.count&&visited++<320){
            UIView *v=q[head++];
            if([v isKindOfClass:[UICollectionView class]]&&ADCNMHasSubNavCell7303(v)){
                CGRect cr=[v convertRect:v.bounds toView:w];
                BOOL adjacent=CGRectGetMaxY(cr)>=CGRectGetMinY(rr)-80.0&&CGRectGetMaxY(cr)<=CGRectGetMinY(rr)+3.0;
                if(adjacent&&CGRectGetWidth(cr)>=CGRectGetWidth(rr)*0.70)return (UICollectionView *)v;
            }
            for(UIView *c in v.subviews)if(c!=root)[q addObject:c];
        }
    } @catch(...) {}
    return nil;
}
static void ADCNMSuppressSubNav7303(UIView *root){
    if(!gP.enabled||!root||!root.window||!ADClassNameIs7183(root,"CNMErrorView"))return;
    @try {
        UICollectionView *cv=ADCNMStoredSubNav7303(root);
        if(!cv){
            cv=ADCNMFindSubNav7303(root);
            if(!cv)return;
            objc_setAssociatedObject(root,kADCNMSubNavWasHidden7303,@(cv.hidden),OBJC_ASSOCIATION_RETAIN_NONATOMIC);
            __weak UICollectionView *weakCV=cv;
            UICollectionView *(^getter)(void)=^UICollectionView *{return weakCV;};
            objc_setAssociatedObject(root,kADCNMSubNavGetter7303,getter,OBJC_ASSOCIATION_COPY_NONATOMIC);
        }
        cv.hidden=YES;
    } @catch(...) {}
}
static void ADCNMRestoreSubNav7303(UIView *root){
    if(!root)return;
    @try {
        UICollectionView *cv=ADCNMStoredSubNav7303(root);
        NSNumber *wasHidden=objc_getAssociatedObject(root,kADCNMSubNavWasHidden7303);
        if(cv&&wasHidden)cv.hidden=wasHidden.boolValue;
        objc_setAssociatedObject(root,kADCNMSubNavGetter7303,nil,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        objc_setAssociatedObject(root,kADCNMSubNavWasHidden7303,nil,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } @catch(...) {}
}
static void ADRefreshCNMErrorTree7303(UIView *root){
    if(!gP.enabled||!root||!root.window||!ADClassNameIs7183(root,"CNMErrorView"))return;
    @try {
        ADOwnCNMErrorView7301(root,root.backgroundColor);
        // The probe shows one direct UIStackView and seven arranged descendants.
        // This bounded, layout-driven pass is deliberately confined to that rare tree.
...
- (void)didMoveToWindow {
    %orig;
    if(((UIView *)self).window)ADRefreshCNMErrorTree7303((UIView *)self);
    else ADCNMRestoreSubNav7303((UIView *)self);
}
- (void)layoutSubviews {
    %orig;
    // setImage:/mount often occurs while the hero still has zero bounds. Final
    // Auto Layout is the deterministic point at which the 640x524 raster is owned.
    ADRefreshCNMErrorTree7303((UIView *)self);
}
- (void)willMoveToWindow:(UIWindow *)newWindow {
    if(!newWindow)ADCNMRestoreSubNav7303((UIView *)self);
    %orig(newWindow);
}
- (void)removeFromSuperview {
    ADCNMRestoreSubNav7303((UIView *)self);
    %orig;
}
%end
...
- (void)didMoveToWindow {
    ...
    objc_setAssociatedObject(self,kADMenuFinalRasterKind7255,nil,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    objc_setAssociatedObject(self,kADReactSurfaceCache7232,nil,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if(!self.window||!ADInCNMErrorView7301((UIView *)self))ADEnsureCNMTWB7303(self);
    ADOwnCNMErrorHero7301(self);
    ADOwnSearchLeadingMagnifier7229(self);
    ADPersonOwnOrderSearchMagnifierLeaf7243(self);
    ADOwnImageView7226(self,YES);
...
- (void)layoutSubviews {
    ...
    if(ADInCNMErrorView7301((UIView *)self))ADOwnCNMErrorHero7301(self);
    ADLayoutImageOverlays7226(self);
    CALayer *cnmTWB=objc_getAssociatedObject(self,kADCNMTWB7303);
    if(cnmTWB)cnmTWB.frame=self.bounds;
}
%end
...
static NSString *ADProbePath7233(NSUInteger run){
    @try {
        ...
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.303-person-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];
    ...
    } @catch(...) { return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.303-person-ui-probe-r%lu.txt",(unsigned long)run]]; }
}
...
static NSString *ADCartProbePath7241(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.303-cart-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.303-cart-ui-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADMenuProbePath7252(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.303-menu-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.303-menu-ui-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADAlexaProbePath7269(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.303-alexa-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.303-alexa-ui-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADPersonSubmenuProbePath7298(NSUInteger run){
    @try {
        ...
        NSString *name=[NSString stringWithFormat:@"AmazonDark-v7.303-person-submenu-hybrid-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];
        return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) {
        return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.303-person-submenu-hybrid-probe-r%lu.txt",(unsigned long)run]];
    }
}
...
static NSString *ADHomeFrameProbePath7265(NSUInteger run){
    @try {
        NSDateFormatter *f=[NSDateFormatter new]; f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"]; f.timeZone=[NSTimeZone localTimeZone]; f.dateFormat=@"yyyyMMdd-HHmmss-SSS";
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.303-home-frame-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject]; return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) { return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.303-home-frame-probe-r%lu.txt",(unsigned long)run]]; }
}
...
static NSString *ADProductScrollProbePath7272(NSUInteger run){
    @try {
        NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.303-product-scroll-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.303-product-scroll-probe-r%lu.txt",(unsigned long)run]];}
}
