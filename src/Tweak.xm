...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         "window.__AMZDARK_FIXCONTRAST__=function(){try{"
           ...
           "}catch(e){}"
                      // v5.356: HOME-CAROUSEL BORDER + COLLEGE CHEVRON REPAIR.
           // P22 on 5.355 proved the section/card discovery was correct, but by=-
           // on every brown card exposed the real bug: artChk(container) is true
           // whenever the card CONTAINS product art, so the border writer skipped
           // precisely the cards we wanted. Border colour is chrome, not artwork;
           // only reject when the candidate itself is an artwork element.
           //
           // The College arrow miss was separate: P9VIS' bgimg flag described the
           // BACKDROP walk, not .a-icon-alt itself. Amazon's standard carousel markup
           // puts the visible sprite on the parent .a-icon and the hidden Next/Previous
           // label in .a-icon-alt. Anchor to the already-proven College section, inspect
           // parent .a-icon controls, and use the semantic hidden label to select only
           // navigation arrows.
           "try{var H54=document.querySelectorAll('h1,h2,h3,h4,span,div'),S54=[],SN54=[];"
             ...
             "for(var s54=0;s54<S54.length&&s54<12;s54++){var se54=S54[s54],q54=se54.querySelectorAll('*');"
               "for(var b54=-1;b54<q54.length&&b54<1200;b54++){var be54=(b54<0?se54:q54[b54]),br54=be54.getBoundingClientRect();if(br54.width<110||br54.height<55)continue;"
                 ...
                 "if(painted54){be54.style.setProperty('border-color','#3b4043','important');be54.__adBy='cardborder56';}"
                 "if(outline54){if(artChk(be54)&&/^(IMG|PICTURE|VIDEO|CANVAS|SVG)$/.test(tg54))continue;be54.style.setProperty('outline-color','#3b4043','important');be54.__adBy='cardborder56';}"
               "}"
               "if(SN54[s54]!=='college')continue;var IC56=se54.querySelectorAll('.a-icon,.a-icon-next-rounded,.a-icon-previous-rounded');"
               "for(var i56=0;i56<IC56.length&&i56<180;i56++){var ic56=IC56[i56],al56=ic56.querySelector&&ic56.querySelector('.a-icon-alt'),cc56=String(ic56.className&&ic56.className.baseVal!==undefined?ic56.className.baseVal:(ic56.className||''));"
                 "var at56=n54(al56&&al56.textContent);if(!(/next|previous/i.test(cc56)||at56.indexOf('next')>=0||at56.indexOf('previous')>=0||at56.indexOf('forward')>=0||at56.indexOf('back')>=0))continue;"
                 "var ir56=ic56.getBoundingClientRect();if(ir56.width<6||ir56.width>84||ir56.height<6||ir56.height>84)continue;"
                 "if(artChk(ic56))continue;ic56.style.setProperty('filter','brightness(0) invert(1)','important');ic56.__adGlyph=1;ic56.__adBy='collegeNav56';"
                 "ic56.setAttribute('data-ad-nav-chevron-paint','1');ic56.setAttribute('data-ad-nav-label',at56.slice(0,24));"
               "}"
             "}"
           "}catch(e){}"
...
static NSString *ADProbeWebJS(void){
    return
       ...
       // P21CHEV (v5.356): report the semantic College carousel icon parents
       // marked by the runtime pass, including final computed filter/background.
       "try{var C21=document.querySelectorAll('[data-ad-nav-chevron-paint=\"1\"],[data-ad-college-section=\"1\"] .a-icon-next-rounded,[data-ad-college-section=\"1\"] .a-icon-previous-rounded'),Q21=[];for(var i21=0;i21<C21.length&&i21<12;i21++){var c21=C21[i21],r21=c21.getBoundingClientRect(),s21=getComputedStyle(c21);Q21.push('lab='+String(c21.getAttribute('data-ad-nav-label')||'-')+'|'+c21.tagName+'.'+String(c21.className||'').replace(/\\s+/g,'.').slice(0,30)+'@'+Math.round(r21.width)+'x'+Math.round(r21.height)+'|bgi='+(s21.backgroundImage&&s21.backgroundImage!=='none'?'Y':'-')+'|f='+String(s21.filter||'none').replace(/\\s+/g,' ').slice(0,46)+'|by='+String(c21.__adBy||'-'));}out.push('P21CHEV[n='+Q21.length+(Q21.length?' '+Q21.join(' ~~ '):'')+']');}catch(e21){out.push('P21CHEV[err '+(e21&&e21.message||e21)+']');}"

       // P22BORDER (v5.356): report the final computed border colours on only
       // the card containers normalized by the guarded runtime pass.
       "try{var B22=document.querySelectorAll('[data-ad-cardborder=\"1\"]'),R22=[];for(var i22=0;i22<B22.length&&R22.length<10;i22++){var b22=B22[i22],r22=b22.getBoundingClientRect(),s22=getComputedStyle(b22);if(r22.width<120||r22.height<50)continue;R22.push(b22.tagName+'.'+String(b22.className||'').replace(/\\s+/g,'.').slice(0,32)+'@'+Math.round(r22.width)+'x'+Math.round(r22.height)+'|t='+String(s22.borderTopColor||'-')+'|l='+String(s22.borderLeftColor||'-')+'|in='+String(b22.style.getPropertyValue('border-color')||'-')+'!'+String(b22.style.getPropertyPriority('border-color')||'-')+'|sec='+String(b22.getAttribute('data-ad-border-section')||'-')+'|by='+String(b22.__adBy||'-'));}out.push('P22BORDER[n='+R22.length+(R22.length?' '+R22.join(' ~~ '):'')+']');}catch(e22){out.push('P22BORDER[err '+(e22&&e22.message||e22)+']');}"
