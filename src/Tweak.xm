...
static NSString *ADFixesLiteral(void){
    ...
    gADFixesLiteral613 = [NSString stringWithFormat:
            @"{css:'"
             ...
             // v6.0.134: one Sponsored text/glyph owner across Amazon's
             // current ad templates. Known first-paint hosts are declarative; the
             // bounded runtime owner below catches classless/lazy variants locally.
             "[class*=sponsored-label],[class*=sponsored-label] *,"
             "[class*=adFeedbackMainComponent],[class*=adFeedbackMainComponent] *,"
             "[class*=ad-feedback-text],[class*=ad-feedback-text] *,"
             "[id^=ad-feedback-text-],[id^=af-label-primary-link-],"
             "[class*=ape-feedback],[class*=ape-feedback] *,"
             "[data-ad-sponsor-text6134]"
             "{color:#ffffff !important;-webkit-text-fill-color:#ffffff !important;"
             "opacity:1 !important;visibility:visible !important;mix-blend-mode:normal !important;}"
             // Known 11/12px sprite family paints on first frame. The semantic owner
             // applies the exact same artwork to classless/lazy siblings after hydrate.
             "[class*=ad-feedback-spr],[data-ad-sponsor-glyph6134],"
             "[data-ad-sponsor-before6134]::before,[data-ad-sponsor-after6134]::after"
             "{filter:none !important;opacity:1 !important;visibility:visible !important;"
             "background-color:transparent !important;"
             "background-image:url(data:image/svg+xml;base64,) !important;"
             "background-repeat:no-repeat !important;background-position:center !important;"
             "background-size:contain !important;mix-blend-mode:normal !important;"
             "color:transparent !important;}"
             "[data-ad-sponsor-glyph6134]>*{opacity:0 !important;visibility:hidden !important;}"
             // v6.0.133 probe-derived APE ownership: expose the real page floor and
             // suppress the border/shadow that outlined the standalone strip.
             ...
             "'[class*=ad-feedback-text]','[class*=ad-feedback-text] *','[id^=ad-feedback-text-]','[id^=af-label-primary-link-]',"
             "'[class*=ape-wrapper]','[class*=ape-placement]','[class*=ape-feedback]','[class*=ape-feedback] *',"
             "'[data-ad-sponsor-text6134]','[data-ad-sponsor-glyph6134]','[data-ad-sponsor-before6134]','[data-ad-sponsor-after6134]','[data-ad-adshell6134]',"
             "'html body .puis-mab-overlay .puis-mab-overlay-row-share .puis-mab-overlay-icon-share'],"
...
static NSString *ADDarkReaderBootstrap(void){
    ...
    gADBootstrap613 = [NSString stringWithFormat:
        @"(function(){try{"
         ...
         // v6.0.134: bounded Sponsored-family owner. It runs once initially and
         // piggybacks on the existing v6.0.15 child-list observer for lazy/recycled ads.
         // No new observer, timer, scroll listener, interval or RAF path is introduced.
         "window.__AD_SPON6134__=function(root){try{if(!root)return 0;var n=0,URI='data:image/svg+xml;base64,',SEL='[class*=sponsored-label],[class*=adFeedbackMainComponent],[class*=ad-feedback-text],[id^=ad-feedback-text-],[id^=af-label-primary-link-],[class*=ape-feedback],[class*=sponsored-products]';function direct(e){try{var s='',N=e.childNodes||[];for(var i=0;i<N.length;i++)if(N[i].nodeType===3)s+=' '+String(N[i].nodeValue||'');return s.replace(/\\s+/g,' ').trim();}catch(x){return'';}}function exact(e){var t=direct(e);return /^sponsored(?: ad)?$/i.test(t);}function white(e){try{if(!e||e.nodeType!==1)return 0;e.setAttribute('data-ad-sponsor-text6134','1');e.style.setProperty('color','#ffffff','important');e.style.setProperty('-webkit-text-fill-color','#ffffff','important');e.style.setProperty('opacity','1','important');e.style.setProperty('visibility','visible','important');return 1;}catch(x){return 0;}}function glyph(e){try{if(!e||e.nodeType!==1)return 0;var tg=String(e.tagName||'').toUpperCase();e.setAttribute('data-ad-sponsor-glyph6134','1');if(tg==='IMG'){e.setAttribute('src',URI);if(e.removeAttribute)e.removeAttribute('srcset');}return 1;}catch(x){return 0;}}function pseudo(host,label){try{if(!host||host.nodeType!==1)return 0;var lr=label.getBoundingClientRect(),z=0,P=['::before','::after'];for(var k=0;k<2;k++){var cs=getComputedStyle(host,P[k]),w=parseFloat(cs.width)||0,h=parseFloat(cs.height)||0,bi=String(cs.backgroundImage||'none'),mi=String(cs.webkitMaskImage||cs.maskImage||'none'),ct=String(cs.content||'none');if(w>=6&&w<=24&&h>=6&&h<=24&&(bi!=='none'||mi!=='none'||(ct&&ct!=='none'&&ct!=='normal'))){host.setAttribute(k===0?'data-ad-sponsor-before6134':'data-ad-sponsor-after6134','1');z++;}}return z;}catch(x){return 0;}}function near(label){try{var p=label.parentElement;if(!p)return 0;var lr=label.getBoundingClientRect(),Q=p.querySelectorAll?p.querySelectorAll('i,span,div,img,svg,[role=img],[class*=feedback],[class*=sponsor],[class*=info],[class*=icon]'):[],best=null,bs=-1;for(var i=0;i<Q.length&&i<72;i++){var e=Q[i];if(e===label||e.contains&&e.contains(label))continue;var r=e.getBoundingClientRect();if(r.width<6||r.height<6||r.width>26||r.height>26)continue;var cy=r.top+r.height/2,lcy=lr.top+lr.height/2;if(Math.abs(cy-lcy)>18||r.right<lr.left-18||r.left>lr.right+42)continue;var c=String(e.className&&e.className.baseVal!==undefined?e.className.baseVal:(e.className||'')).toLowerCase(),cs=getComputedStyle(e),bi=String(cs.backgroundImage||'none'),mi=String(cs.webkitMaskImage||cs.maskImage||'none'),tg=String(e.tagName||'').toUpperCase(),s=0;if(/ad-feedback|feedback|sponsor|info/.test(c))s+=8;if(bi!=='none'||mi!=='none')s+=5;if(/^(IMG|SVG|I)$/.test(tg))s+=3;s-=Math.abs((r.left)-(lr.right+4))*.05;if(s>bs){bs=s;best=e;}}var z=0;if(best&&bs>=3)z+=glyph(best);z+=pseudo(label,label);z+=pseudo(p,label);return z;}catch(x){return 0;}}function leaf(e){try{if(!e||e.nodeType!==1)return 0;if(exact(e)){n+=white(e);n+=near(e);return 1;}var Q=e.querySelectorAll?e.querySelectorAll('span,a,p,small,div'):[];for(var i=0;i<Q.length&&i<60;i++)if(exact(Q[i])){n+=white(Q[i]);n+=near(Q[i]);return 1;}return 0;}catch(x){return 0;}}var A=[];function add(e){if(e&&A.indexOf(e)<0&&A.length<40)A.push(e);}if(root===document){var q=document.querySelectorAll(SEL);for(var i=0;i<q.length&&i<80;i++)add(q[i]);}else if(root.nodeType===1){if(root.matches&&root.matches(SEL))add(root);if(root.querySelectorAll){var q2=root.querySelectorAll(SEL);for(var j=0;j<q2.length&&j<24;j++)add(q2[j]);}var nat=root.closest&&root.closest('[data-ad-native615]');if(nat)add(nat);}for(var a=0;a<A.length;a++)leaf(A[a]);window.__AD_SPON6134_STATE__='roots='+A.length+' paint='+n;return n;}catch(e){window.__AD_SPON6134_STATE__='err '+String(e);return 0;}};window.__AD_ADSHELL6134__=function(root){try{if(window.top!==window||!root)return 0;var vw=innerWidth||390,n=0,SEL='[data-ad-sponsor-text6134],[class*=ad-feedback-spr],[class*=sponsored-label],[class*=adFeedbackMainComponent],[id^=ad-feedback-text-],[class*=ape-feedback]';function cn(e){var c=e&&e.className;if(c&&c.baseVal!==undefined)c=c.baseVal;return String(c||'').toLowerCase();}function clr(e){try{if(!e||e.nodeType!==1||/^(IMG|VIDEO|CANVAS|SVG|PICTURE)$/.test(String(e.tagName||'').toUpperCase()))return 0;var cs=getComputedStyle(e),bi=String(cs.backgroundImage||'none');if(bi&&bi!=='none')return 0;e.style.setProperty('background-color','transparent','important');e.style.setProperty('box-shadow','none','important');e.style.setProperty('border-color','transparent','important');e.style.setProperty('outline-color','transparent','important');e.setAttribute('data-ad-adshell6134','1');return 1;}catch(x){return 0;}}var A=[];function add(e){if(e&&A.indexOf(e)<0&&A.length<20)A.push(e);}if(root===document){var q=document.querySelectorAll(SEL);for(var i=0;i<q.length&&i<28;i++)add(q[i]);}else if(root.nodeType===1){if(root.matches&&root.matches(SEL))add(root);if(root.querySelector){var z=root.querySelector(SEL);if(z)add(z);}if(root.closest){var u=root.closest(SEL);if(u)add(u);}}for(var j=0;j<A.length;j++){var p=A[j],d=0;while(p&&p!==document.body&&p!==document.documentElement&&d++<8){var r=p.getBoundingClientRect(),c=cn(p),ape=/ape-placement|ape-wrapper|ape-feedback|hybrid-widget-sponsored/.test(c),strip=(r.width>=vw*.72&&r.height>=28&&r.height<=190);if(ape||strip){n+=clr(p);var C=p.children||[];for(var k=0;k<C.length&&k<10;k++){var ch=C[k],cr=ch.getBoundingClientRect();if(cr.width>=r.width*.72&&cr.height>=20)n+=clr(ch);}}if(r.width>=vw*.96&&r.height>260)break;p=p.parentElement;}}window.__AD_ADSHELL6134_STATE__='seeds='+A.length+' clear='+n;return n;}catch(e){window.__AD_ADSHELL6134_STATE__='err '+String(e);return 0;}};"
