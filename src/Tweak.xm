...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{var BG=[],AE=document.querySelectorAll('*');"
         "for(var bi=0;bi<AE.length&&BG.length<10;bi++){var eb=AE[bi];"
           "if(eb.childElementCount>1)continue;"
           "var tb=String(eb.textContent||'').trim();if(tb.length<2||tb.length>16)continue;"
           "if(tb.indexOf('%')<0&&tb.indexOf('$')<0&&tb.toLowerCase().indexOf('sponsor')<0)continue;"
           "var sb=getComputedStyle(eb);var lb=lum(sb.backgroundColor);"
           "var rb=eb.getBoundingClientRect();if(rb.width<10)continue;"
           "var ch=[],nb=eb,db=0;while(nb&&db<4){ch.push((cls(nb)||nb.tagName).slice(0,20));nb=nb.parentElement;db++;}"
           "BG.push(tb.slice(0,12)+'|'+eb.tagName+'|bg='+sb.backgroundColor+'|col='+sb.color+'|L='+(lb<0?'-':lb.toFixed(2))+'|'+ch.join('>'));}"
         "out.push('P9BADGE['+(BG.length?BG.join(' ~ '):'none')+']');}catch(eb2){out.push('P9BADGE[err]');}"
