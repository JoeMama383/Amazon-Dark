...
#define AD_VERSION "v7.475-pdp-offsite-nav-separators"
...
static void ADHideThinBarHairlines7475(UIView *v){
    ...
    @try {
        ...
        for(CALayer *l in v.layer.sublayers){
            CGRect f=l.frame;
            if((f.size.height>0.0&&f.size.height<=2.0&&f.size.width>=w*0.25)||
               (f.size.width>0.0&&f.size.width<=2.0&&f.size.height>=h*0.35)){
                l.hidden=YES;
                l.opacity=0.0f;
