...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{var CL=[],EC=document.querySelectorAll('*');"
         "for(var q2=0;q2<EC.length&&CL.length<8;q2++){var c2=EC[q2];"
           ...
           "if(x2.indexOf('%')<0&&x2.indexOf('$')<0)continue;"
             "var lw=lum(getComputedStyle(c2).backgroundColor);if(lw>=0&&lw>0.30)continue;"
       ...
       "})();}catch(e){}"
       "/*V5314FIX*//*V5315FIX*/"
       "try{var HD=[],EH=document.querySelectorAll('h1,h2,h3,h4,[class*=header] span,[class*=title]');"
         "for(var h=0;h<EH.length&&HD.length<8;h++){var eh=EH[h];"
           "var th=String(eh.textContent||'').trim();if(th.length<3||th.length>40)continue;"
           "var rh=eh.getBoundingClientRect();if(rh.width<40||rh.height<10)continue;"
           "if(rh.bottom<0||rh.top>(window.innerHeight||900))continue;"
           "var sh=getComputedStyle(eh);"
           "var kh=[],nh=eh,dh=0;while(nh&&dh<4){kh.push((cls(nh)||nh.tagName).slice(0,20));nh=nh.parentElement;dh++;}"
           "HD.push(th.slice(0,16)+'|'+eh.tagName+'|col='+sh.color+'|bg='+sh.backgroundColor+'|'+kh.join('>'));}"
         "out.push('P9HEADER['+(HD.length?HD.join(' ~ '):'none')+']');}catch(xh){out.push('P9HEADER[err]');}"
       "/*V5317FIX*/"
