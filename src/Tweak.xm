...
#define AD_VERSION "v7.161-search-seasonal-scrollbar-probe"
...
static NSString *ADFloorJS(void){
    // v7.161: preserve the proven v7.159 three-lane Search behavior and transition fix.
    // Add only exact seasonal autocomplete tile ownership and product-feed scrollbar parity.
    return
...
static NSString *ADTWBJS(void){
    // v7.161 probe: keep v7.160 route-exclusive TWB and full _c2Itd brand-carousel
    // image ownership. Add the exact seasonal autocomplete tile image found by v7.159.
    CGFloat strength=MAX(0,MIN(100,gP.whiteTameStrength));
    CGFloat t=strength/100.0;
    CGFloat shade=0.10+(0.48*t);
    CGFloat factor=1.0-shade;
...
static NSString *ADSearchResultsProbePath7139(void){
    @try {
        ...
        if(docs.length)return [docs stringByAppendingPathComponent:@"AmazonDark-v7.161-search-seasonal-scrollbar-probe.txt"];
    } @catch(...) {}
    return [NSTemporaryDirectory() stringByAppendingPathComponent:@"AmazonDark-v7.161-search-seasonal-scrollbar-probe.txt"];
}
