...
static NSString *gADFrameCache = nil;  // v5.474 subframe script; invalidated with the boot cache
...
static void ADLoadPrefs(void){
    ...
    dispatch_async(ADBootQueue(), ^{ gADBootCache = nil; gADFrameCache = nil; });   // serialized invalidation
...
// ════════════════════════════════════════════════════════════════════════════
// v5.474 FRAME SPLIT. The boot payload head and the child-frame engine are
// factored out VERBATIM (moved, never duplicated -- the frozen-surface gates
// hash these exact lines) so the lightweight subframe script can reuse them.
// ADDarkReaderBootstrapBuild reassembles the identical byte sequence, so the
// main document payload is unchanged. Subframes get ADFrameBoot474 instead:
// head + frame engine only -- no Dark Reader UMD (never enabled in child
// frames since v5.361) and none of the main-document mesh.
// ════════════════════════════════════════════════════════════════════════════
static NSString *ADBootHead474a(void){
    return [NSString stringWithFormat: @""
...
static NSString *ADBootHead474b(void){
    return [NSString stringWithFormat: @""
         // ── AD FRAME GATE ───────────────────────────────────────────────────────
...
static NSString *ADFrameCore474(void){
    return [NSString stringWithFormat: @""
...
static NSString *ADFrameBoot474(void){
    __block NSString *out474 = nil;
    dispatch_sync(ADBootQueue(), ^{
        if (!gADFrameCache){
            ...
            gADFrameCache=[s copy];
        }
        out474=gADFrameCache;
    });
    return out474;
}
...
static NSString *ADDarkReaderBootstrapBuild(void){
    ...
    NSMutableString *adBody = [NSMutableString stringWithCapacity:dr.length + 320*1024];
    [adBody appendString:
        @"(function(){try{"
    ...
    @""];
    [adBody appendString:ADBootHead474a()];
    [adBody appendFormat:@"%@\n", dr]; // DarkReader UMD
...
static void ADEnsureUserScript(WKWebView *wv){
    @try {
        ...
        if (js.length && WKUS && ucc){
            ...
            NSString *fr474 = ADFrameBoot474();
            if (fr474.length){
                WKUserScript *fus474 = [[WKUS alloc] initWithSource:fr474
                                                      injectionTime:WKUserScriptInjectionTimeAtDocumentStart
                                                   forMainFrameOnly:NO];
                [ucc addUserScript:fus474];
            }
            static dispatch_once_t once474; dispatch_once(&once474, ^{
                ADLog(@"P99SPLIT474[main=%lu frame=%lu]", (unsigned long)js.length, (unsigned long)fr474.length); });
            ...
            if (js.length && WKUS){
                ...
                if (fr474.length){
                    WKUserScript *fus474 = [[WKUS alloc] initWithSource:fr474
                                                          injectionTime:WKUserScriptInjectionTimeAtDocumentStart
                                                       forMainFrameOnly:NO];
                    [cfg.userContentController addUserScript:fus474];
                }
            }
        }
    } @catch(...) {}
...
"/*V5317FIX*//*V5376FIX*//*V5377FIX*//*V5378FIX*//*V5379FIX*//*V5380FIX*//*V5381FIX*//*V5382FIX*//*V5383FIX*//*V5384FIX*//*V5385FIX*//*V5386FIX*//*V5387FIX*//*V5388FIX*//*V5391FIX*//*V5393FIX*/"
       ...
       // P85CHECKBOX441: reports the native-state/chrome contract. Marker names
       // stay stable; checked/unchecked comes directly from Amazon's input,
       // ARIA, class, or sprite URL and must agree with filter + chrome release.
       "try{var A85=document.querySelectorAll('[data-ad-checkbox434-art]'),S85=document.querySelectorAll('[data-ad-checkbox434-shell=\"cart\"]'),unchecked85=0,checked85=0,badFilter85=0,badChrome85=0,legacy85=0,foreign85=0,miss85=0,nativePaint85=0,Q85=[];"
