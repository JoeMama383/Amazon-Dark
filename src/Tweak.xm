...
static NSString *ADProbeWebJS(void){
    return
       ...
       "try{var CL=[],EC=document.querySelectorAll('*');"
         "for(var q2=0;q2<EC.length&&CL.length<8;q2++){var c2=EC[q2];"
           ...
           "var o3=1,f3='-',a3=c2,d3=0;while(a3&&d3<8){var g3=getComputedStyle(a3);o3*=parseFloat(g3.opacity||1);if(g3.filter&&g3.filter!=='none')f3=g3.filter.slice(0,20);a3=a3.parentElement;d3++;}"
             "CL.push(x2.slice(0,12)+'|'+c2.tagName+'|op='+o3.toFixed(2)+'|f='+f3+'|bg='+y2.backgroundColor+'|col='+y2.color+'|'+k2.join('>'));}"
       ...
       "})();}catch(e){}"
       "/*V5314FIX*//*V5315FIX*/"
       "try{var HD=[],EH=document.querySelectorAll('h1,h2,h3,h4,span,div');"
         "function hc(e){var c=e.className;return String(c&&c.baseVal!==undefined?c.baseVal:(c||'')).slice(0,20);}"
         "for(var h=0;h<EH.length&&HD.length<10;h++){var eh=EH[h];"
           "if(eh.childElementCount>0)continue;"
           "var th=String(eh.textContent||'').trim();if(th.length<4||th.length>34)continue;"
           "var rh=eh.getBoundingClientRect();if(rh.width<60||rh.height<14)continue;"
           "if(rh.bottom<0||rh.top>(window.innerHeight||900))continue;"
           "var fs=parseFloat(getComputedStyle(eh).fontSize)||0;if(fs<15)continue;"
           "var sh=getComputedStyle(eh);"
           "var kh=[],nh=eh,dh=0;while(nh&&dh<3){kh.push((hc(nh)||nh.tagName).slice(0,18));nh=nh.parentElement;dh++;}"
           "HD.push(th.slice(0,16)+'|fs='+fs.toFixed(0)+'|col='+sh.color+'|'+kh.join('>'));}"
         "out.push('P9HEADER['+(HD.length?HD.join(' ~ '):'none')+']');}catch(xh){out.push('P9HEADER[err '+(xh&&xh.message||xh)+']');}"
"/*V5317FIX*/"
       "try{var R=[],EL=document.querySelectorAll('*');"
         ...
         "out.push('P9CAROBOX[sub='+(__sub?1:0)+' '+(R.length?R.join(' ~ '):'none')+']');}catch(x){out.push('P9CAROBOX[err]');}"
       "try{var R=[],EL=document.querySelectorAll('*');"
         "for(var q=0;q<EL.length&&R.length<8;q++){var e=EL[q];"
           "var t=String(e.textContent||'').trim();"
           "var r=e.getBoundingClientRect();if(r.width<8||r.height<6)continue;"
