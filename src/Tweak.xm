...
static NSString *ADProbeWebJS(void){
    return
       ...
       // P9VIS (v5.332): generic low-contrast text/glyph scan, tagged by card
       // family. Replaces P9CHEV/P9CARDHDR/P9TILETXT, which keyed on rotating
       // hashes (cXVhZ) and fixed tags and so returned [none]. Finds ANY ink
       // whose contrast against its real backdrop is < .22 inside a card,
       // labels it cXVhZ vs npack (the working reference), and notes an image
       // backdrop where contrast is indeterminate. Validated in jsdom against a
       // synthetic cXVhZ+npack DOM: flags all 3 symptoms, no npack false
       // positives, silent (n=0) once corrected.
       "try{"
       ...
       "    return {L:-1,img:0};}"
       "  var all9=document.querySelectorAll('*'),N9=Math.min(all9.length,5000),H9=[];"
       "  for(var i9=0;i9<N9&&H9.length<10;i9++){var e9=all9[i9];"
       ...
       "    var te9=(tx9.length>=2&&tx9.length<=40&&e9.childElementCount===0);"
       "    if(!gl9&&!te9)continue;"
       "    var ic9=gl9?(s9.fill&&s9.fill!=='none'?s9.fill:s9.color):s9.color;"
       "    var il9=lum9(ic9);if(il9<0)continue;"
       "    var b9=bd9(e9);"
       "    if(b9.L<0){if(b9.img&&(il9>0.6||il9<0.4)){H9.push('IMG '+f9+'|'+tg9+'|'+(gl9?'glyph':tx9.slice(0,10))+'|ink='+il9.toFixed(2)+'|bgimg|'+cls9(e9));}continue;}"
       "    var d9=Math.abs(il9-b9.L);"
       "    if(d9<0.22){H9.push(f9+'|'+tg9+'|'+(gl9?'glyph':tx9.slice(0,10))+'|inkL='+il9.toFixed(2)+'|bgL='+b9.L.toFixed(2)+'|d='+d9.toFixed(2)+(b9.img?'|+img':'')+'|'+cls9(e9));}}"
