...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         "window.__AD_PRODUCTCTRL391RUN__=function(){try{if(window.__ADFRAME_MODE__||!document.body)return 0;"
           ...
           // Heart: use a real square descendant/host, never the wide heart-position wrapper.
           "var H=document.querySelectorAll('[class*=\"puis-heart-position\"]');for(var hi=0;hi<H.length&&hi<180;hi++){var hr=H[hi],hg=hr.querySelector&&hr.querySelector('[class*=\"lists-treatment-hear\"] .a-icon,.a-icon,svg,img,i');if(!hg){miss++;continue;}var hh=around(hr,hg,24,48);if(!hh&&square(hr,24,48))hh=hr;if(!hh){miss++;continue;}if(skin(hh,'heart')){glyphs(hh);paint(hg,hh);heart++;}}"
