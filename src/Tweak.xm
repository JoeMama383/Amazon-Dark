/*
 * AmazonDark v7.320 — launch-transition probe on v7.316 production behavior
 *
 * Architecture:
...
#define AD_VERSION "v7.320-icon-tap-cold-bridge"
...
// the v7.320 cold first-frame shim; warm behavior is owned inside Amazon only.
//
// Stock-like contract:
...
// v7.320 probe-only app-side launch recorder. Event timestamps use system uptime so they
...
static NSString *ADLaunchAppProbePath7317(void){
    static NSString *p; static dispatch_once_t once; dispatch_once(&once,^{NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];p=[(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:@"AmazonDark-v7.320-launch-app-probe.txt"];}); return p;
}
...
static void ADOwnAmazonSplash7307(UIViewController *vc){
    ...
    @try {
        ...
        // own its earliest floor dark. v7.320 SpringBoard bridges only the pre-process frame.
        vc.view.hidden=NO;
        vc.view.alpha=1.0;
...
// v7.320: SpringBoard no longer waits for Home/WebKit readiness. Its only job is
...
// -----------------------------------------------------------------------------
// v7.320 launch handoff is now owned by the exact Amazon splash controllers above.
...
static NSString *ADProbePath7233(NSUInteger run){
    @try {
        ...
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.320-person-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];
    ...
    } @catch(...) { return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.320-person-ui-probe-r%lu.txt",(unsigned long)run]]; }
}
...
static NSString *ADCartProbePath7241(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.320-cart-transition-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.320-cart-transition-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADMenuProbePath7252(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.320-menu-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.320-menu-ui-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADAlexaProbePath7269(NSUInteger run){
    @try {NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.320-alexa-ui-probe-%@-r%lu.txt",stamp,(unsigned long)run];NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];} @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.320-alexa-ui-probe-r%lu.txt",(unsigned long)run]];}
}
...
static NSString *ADPersonSubmenuProbePath7298(NSUInteger run){
    @try {
        ...
        NSString *name=[NSString stringWithFormat:@"AmazonDark-v7.320-person-submenu-hybrid-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];
        return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) {
        return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.320-person-submenu-hybrid-probe-r%lu.txt",(unsigned long)run]];
    }
}
...
static NSString *ADHomeFrameProbePath7265(NSUInteger run){
    @try {
        NSDateFormatter *f=[NSDateFormatter new]; f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"]; f.timeZone=[NSTimeZone localTimeZone]; f.dateFormat=@"yyyyMMdd-HHmmss-SSS";
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.320-home-frame-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject]; return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) { return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.320-home-frame-probe-r%lu.txt",(unsigned long)run]]; }
}
...
static NSString *ADProductScrollProbePath7272(NSUInteger run){
    @try {
        NSDateFormatter *f=[NSDateFormatter new];f.locale=[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];f.timeZone=[NSTimeZone localTimeZone];f.dateFormat=@"yyyyMMdd-HHmmss-SSS";
        NSString *stamp=[f stringFromDate:[NSDate date]]?:@"unknown",*name=[NSString stringWithFormat:@"AmazonDark-v7.320-product-scroll-probe-%@-r%lu.txt",stamp,(unsigned long)run];
        NSString *docs=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) firstObject];return [(docs.length?docs:NSTemporaryDirectory()) stringByAppendingPathComponent:name];
    } @catch(...) {return [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"AmazonDark-v7.320-product-scroll-probe-r%lu.txt",(unsigned long)run]];}
}
