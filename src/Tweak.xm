    ...
    gADBootstrap613 = [NSString stringWithFormat:
        @"(function(){try{"
         ...
         "try{window.__AD_NATIVE_SEL615__='[class*=single-creative-card],[class*=single-video-card],[class*=theming-card],[class*=canvas-card],[class*=ape-placement],[class*=ape-wrapper],[data-cel-widget*=ape],[id*=ape_],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent],[class*=sponsored-products]';"
           ...
           "window.__AD_MARK_NATIVE615__(document);if(!window.__AD_NATIVE_OBS615__&&document.documentElement){window.__AD_NATIVE_OBS615__=1;new MutationObserver(function(ms){try{for(var i=0;i<ms.length&&i<48;i++){var A=ms[i].addedNodes||[];for(var j=0;j<A.length&&j<24;j++)if(A[j]&&A[j].nodeType===1){window.__AD_MARK_NATIVE615__(A[j]);if(window.__AD_VIDEOSTOCK6064__)window.__AD_VIDEOSTOCK6064__(A[j]);}}}catch(e){}}).observe(document.documentElement,{childList:true,subtree:true});}}catch(e){}"
         // v6.0.64: stock video-control normalization.
         // Keep Amazon's own play/pause glyph.  Re-scrub the control on every state
         // transition (not just first discovery), neutralize only compact outer
         // wrappers that can become the red square, and derive the circular shell
         // tint from Amazon's neighboring mute control instead of hard-coding paint.
         // The VIDEO pixels remain independently owned by the direct TWB media path.
         "try{var _old62=document.getElementById('advidctl6062');if(_old62&&_old62.parentNode)_old62.parentNode.removeChild(_old62);"
           "if(!document.getElementById('advidwrap6064')){var vw64=document.createElement('style');vw64.id='advidwrap6064';vw64.textContent='[data-ad-videowrap6064]{background-color:transparent!important;background-image:none!important;border:0!important;outline:0!important;box-shadow:none!important;}[data-ad-videowrap6064]::before,[data-ad-videowrap6064]::after{background-color:transparent!important;background-image:none!important;border:0!important;outline:0!important;box-shadow:none!important;}';(document.head||document.documentElement).appendChild(vw64);}"
           "function adVSem6064(e,lim){try{var s='',p=e,d=0;while(p&&d++<(lim||3)){var c=p.className;c=String(c&&c.baseVal!==undefined?c.baseVal:(c||''));s+=' '+c+' '+String((p.getAttribute&&p.getAttribute('aria-label'))||(p.getAttribute&&p.getAttribute('title'))||'');p=p.parentElement;}return s;}catch(x){return '';}}"
           "function adVCtl6064(host,re,ban){try{var A=host&&host.querySelectorAll?host.querySelectorAll('button,[role=button],[aria-label],[title],svg,i,[class*=play],[class*=pause],[class*=mute],[class*=volume]'):[];for(var i=0;i<A.length&&i<180;i++){var e=A[i],sem=adVSem6064(e,3);if(ban&&ban.test(sem))continue;if(!re.test(sem))continue;var q=(e.closest&&e.closest('button,[role=button]'))||e,r=q.getBoundingClientRect();if(r.width>=10&&r.width<=96&&r.height>=10&&r.height<=96)return q;}return null;}catch(x){return null;}}"
           "function adVStockScrub6064(root){try{if(!root||root.nodeType!==1)return 0;var E=[root],D=root.querySelectorAll?root.querySelectorAll('*'):[],n=0;for(var di=0;di<D.length&&di<48;di++)E.push(D[di]);for(var i=0;i<E.length;i++){var e=E[i],st=e.style;if(!st)continue;var old=e.hasAttribute&&e.hasAttribute('data-ad-videoctl6062'),by=String(e.__adBy||''),glyph=!!e.__adGlyph;if(old||glyph||/^gfix/.test(by)||by==='videoCtl6062'){var P=['filter','background','background-color','border','border-color','border-radius','outline','outline-color','box-shadow','color','fill','stroke','opacity','visibility','-webkit-text-fill-color','isolation'];for(var p=0;p<P.length;p++)st.removeProperty(P[p]);delete e.__adGlyph;if(/^gfix/.test(by)||by==='videoCtl6062')delete e.__adBy;n++;}else{var f=String(st.getPropertyValue('filter')||'').replace(/\\s+/g,'').toLowerCase();if(st.getPropertyPriority('filter')==='important'&&f==='brightness(0)invert(1)'){st.removeProperty('filter');n++;}var bi=String(st.getPropertyValue('background-image')||'').trim().toLowerCase();if(st.getPropertyPriority('background-image')==='important'&&bi==='none'){st.removeProperty('background-image');n++;}var ink=['color','fill','stroke','-webkit-text-fill-color'];for(var k=0;k<ink.length;k++){var iv=String(st.getPropertyValue(ink[k])||'').replace(/\\s+/g,'').toLowerCase();if(st.getPropertyPriority(ink[k])==='important'&&(iv==='#fff'||iv==='#ffffff'||iv==='#e8e6e3'||iv==='rgb(255,255,255)'||iv==='rgb(232,230,227)')){st.removeProperty(ink[k]);n++;}}}if(e.removeAttribute)e.removeAttribute('data-ad-videoctl6062');}if(window.__AD_STRIP_DR615__)window.__AD_STRIP_DR615__(root);return n;}catch(x){return 0;}}"
           "function adVWrap6064(q,vr){try{if(!q)return 0;var qr=q.getBoundingClientRect(),p=q.parentElement,d=0,n=0;while(p&&d++<3){var r=p.getBoundingClientRect();if(r.width<qr.width*.92||r.height<qr.height*.92||r.width>Math.min(116,qr.width*1.75)||r.height>Math.min(116,qr.height*1.75))break;var cx=r.left+r.width/2,cy=r.top+r.height/2;if(cx<vr.left-28||cx>vr.right+28||cy<vr.top-28||cy>vr.bottom+28)break;p.setAttribute('data-ad-videowrap6064','1');var st=p.style;st.setProperty('background-color','transparent','important');st.setProperty('background-image','none','important');st.setProperty('border','0','important');st.setProperty('outline','0','important');st.setProperty('box-shadow','none','important');n++;p=p.parentElement;}return n;}catch(x){return 0;}}"
           "function adVAlpha6064(c){try{c=String(c||'').replace(/\\s+/g,'').toLowerCase();if(!c||c==='transparent'||c==='rgba(0,0,0,0)')return 0;var m=c.match(/^rgba?\\((\\d+),(\\d+),(\\d+)(?:,([0-9.]+))?\\)$/);return m?(m[4]===undefined?1:parseFloat(m[4])):1;}catch(x){return 0;}}"
           "function adVShell6064(q){try{if(!q)return null;var Q=[q],D=q.querySelectorAll?q.querySelectorAll('*'):[];for(var i=0;i<D.length&&i<24;i++)Q.push(D[i]);var best=null,score=-1;for(var j=0;j<Q.length;j++){var e=Q[j],r=e.getBoundingClientRect();if(r.width<24||r.width>86||r.height<24||r.height>86)continue;if(Math.abs(r.width-r.height)>12)continue;var cs=getComputedStyle(e),a=adVAlpha6064(cs.backgroundColor);if(a<=.02)continue;var rad=parseFloat(cs.borderTopLeftRadius)||0,sc=(rad>=Math.min(r.width,r.height)*.28?4:0)+(a>.15?2:0)-(r.width/200);if(sc>score){score=sc;best=e;}}return best;}catch(x){return null;}}"
           "function adVSyncShell6064(play,mute){try{var ps=adVShell6064(play),ms=adVShell6064(mute);if(!ps||!ms||ps===ms)return 0;var cs=getComputedStyle(ms),bg=String(cs.backgroundColor||'');if(adVAlpha6064(bg)<=.02)return 0;ps.style.setProperty('background-color',bg,'important');var br=String(cs.borderRadius||'');if(br&&br!=='0px')ps.style.setProperty('border-radius',br,'important');return 1;}catch(x){return 0;}}"
           "window.__AD_VIDEOSTOCK_ONE6064__=function(v){try{if(!v||String(v.tagName||'').toUpperCase()!=='VIDEO')return 0;var vr=v.getBoundingClientRect();if(vr.width<100||vr.height<70)return 0;var host=v,pd=0;while(host.parentElement&&pd++<4){var hp=host.parentElement,hr=hp.getBoundingClientRect();if(hr.width>=vr.width*.70&&hr.width<=vr.width*1.70&&hr.height>=vr.height*.70&&hr.height<=vr.height*1.75)host=hp;else break;}var play=adVCtl6064(host,/play|pause/i,/mute|volume|sound|caption|fullscreen/i);if(!play)return 0;var pr=play.getBoundingClientRect(),pcx=pr.left+pr.width/2,pcy=pr.top+pr.height/2;if(pcx<vr.left-24||pcx>vr.right+24||pcy<vr.top-24||pcy>vr.bottom+24)return 0;play.setAttribute('data-ad-videostock6064','1');play.setAttribute('data-ad-native615','1');adVStockScrub6064(play);adVWrap6064(play,vr);var mute=adVCtl6064(host,/mute|unmute|volume|sound/i,/play|pause|caption|fullscreen/i);if(mute){mute.setAttribute('data-ad-native615','1');if(window.__AD_STRIP_DR615__)window.__AD_STRIP_DR615__(mute);adVSyncShell6064(play,mute);}return 1;}catch(e){return 0;}};"
           "window.__AD_VIDEOSTOCK6064__=function(root){try{var V=[],n=0;function addFrom(b){if(!b)return;try{if(String(b.tagName||'').toUpperCase()==='VIDEO'&&V.indexOf(b)<0)V.push(b);var q=b.querySelectorAll?b.querySelectorAll('video'):[];for(var i=0;i<q.length&&V.length<36;i++)if(V.indexOf(q[i])<0)V.push(q[i]);}catch(x){}}addFrom(root);if(!V.length&&root&&root.nodeType===1){var p=root.parentElement,d=0;while(p&&d++<4&&!V.length){addFrom(p);p=p.parentElement;}}if(!V.length&&root===document)addFrom(document);for(var j=0;j<V.length&&j<36;j++)n+=window.__AD_VIDEOSTOCK_ONE6064__(V[j]);return n;}catch(e){return 0;}};"
           "var vcs64=function(ev){try{var t=ev&&ev.target;if(t&&String(t.tagName||'').toUpperCase()==='VIDEO')window.__AD_VIDEOSTOCK_ONE6064__(t);}catch(e){}};document.addEventListener('loadedmetadata',vcs64,true);document.addEventListener('loadeddata',vcs64,true);document.addEventListener('canplay',vcs64,true);document.addEventListener('play',vcs64,true);document.addEventListener('playing',vcs64,true);document.addEventListener('pause',vcs64,true);"
           "document.addEventListener('click',function(ev){try{var p=ev&&ev.target,d=0,sem='';while(p&&d++<4){var c=p.className;c=String(c&&c.baseVal!==undefined?c.baseVal:(c||''));sem+=' '+c+' '+String((p.getAttribute&&p.getAttribute('aria-label'))||(p.getAttribute&&p.getAttribute('title'))||'');p=p.parentElement;}if(!/play|pause/i.test(sem)||/mute|volume|sound|caption|fullscreen/i.test(sem))return;setTimeout(function(){window.__AD_VIDEOSTOCK6064__(document);},0);setTimeout(function(){window.__AD_VIDEOSTOCK6064__(document);},90);setTimeout(function(){window.__AD_VIDEOSTOCK6064__(document);},260);}catch(e){}},true);"
           "var vis64=function(){try{window.__AD_VIDEOSTOCK6064__(document);}catch(e){}};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',vis64,{once:true});else vis64();window.addEventListener('pageshow',vis64,{passive:true});}catch(e){}"
