...
"if(!(function(x){try{return x.closest&&x.closest('[class*=a-cardui-header],[class*=sponsored-products],[class*=hybrid-widget-sponsored],[class*=adFeedbackMainComponent]');}catch(e){return false;}})(e2)){"
         ...
         "if(window.top!==window){try{"
           ...
           "try{addEventListener('message',function(ev){try{var d=ev.data;if(d&&d.__amzAdMode){window.__ADFRAME_MODE__=d.__amzAdMode;window.__AD_HEROFAST365__&&window.__AD_HEROFAST365__(document.documentElement);window.__AMZDARK_ADTHEME__&&window.__AMZDARK_ADTHEME__();}}catch(e){};});}catch(e){}"
           ...
           "window.__AMZDARK_ADTAME__=function(){try{"
             ...
             "var dt365=String((document.body&&document.body.innerText)||'').replace(/\s+/g,' ').trim();var photoOnly365=standalone&&dt365.replace(/sponsored(?: ad)?/ig,'').trim().length<24;"
             ...
             "for(var j=0;j<B.length&&j<800&&bg<60;j++){var q=B[j],cs=getComputedStyle(q),bi=String(cs.backgroundImage||'none');"
               ...
               "if(rr.width<minB||rr.height<minB)continue;var full365=standalone&&rr.width>(innerWidth||390)*0.72&&rr.height>(innerHeight||80)*0.72;if(full365&&!photoOnly365){if(q.__adPhoto365){q.style.removeProperty('background-color');q.style.removeProperty('background-blend-mode');q.__adTame362=0;q.__adPhoto365=0;}continue;}"
 ...
 // imposed, so the creative renders exactly as Amazon shipped it.
         ...
         "function _adBorderFast367(root){try{if(!root||root.nodeType!==1)return 0;function pc(v){var m=/rgba?\(([0-9.]+),\s*([0-9.]+),\s*([0-9.]+)/.exec(String(v||''));return m?[+m[1],+m[2],+m[3]]:null;}function brown(v){var c=pc(v);if(!c)return false;var r=c[0],g=c[1],b=c[2],mx=Math.max(r,g,b),mn=Math.min(r,g,b);return r>=55&&r<=125&&g>=48&&g<=115&&b>=38&&b<=105&&(mx-mn)<=32&&r>=g-4&&g>=b-5&&(r-b)>=4;}var A=[];if(/^(DIV|LI|SECTION|ARTICLE|A)$/i.test(String(root.tagName||'')))A.push(root);try{var q=root.querySelectorAll('div,li,section,article,a');for(var i=0;i<q.length&&i<90;i++)A.push(q[i]);}catch(e){}var n=0,S=['Top','Right','Bottom','Left'];for(var j=0;j<A.length&&j<96;j++){var e=A[j],r=e.getBoundingClientRect();if(r.width<100||r.height<52)continue;var c=e.className;c=String(c&&c.baseVal!==undefined?c.baseVal:(c||''));if(/badge|deal|prime|button|coupon|discount|alert|warning/i.test(c))continue;var cs=getComputedStyle(e),rad=Math.max(parseFloat(cs.borderTopLeftRadius)||0,parseFloat(cs.borderTopRightRadius)||0,parseFloat(cs.borderBottomLeftRadius)||0,parseFloat(cs.borderBottomRightRadius)||0);if(rad<3&&!/card|tile|mosaic|window|container/i.test(c))continue;var hit=false;for(var k=0;k<4;k++){var sd=S[k],w=parseFloat(cs['border'+sd+'Width'])||0,st=String(cs['border'+sd+'Style']||'');if(w>=0.5&&st!=='none'&&st!=='hidden'&&brown(cs['border'+sd+'Color'])){hit=true;break;}}if(!hit){var ow=parseFloat(cs.outlineWidth)||0;if(ow>=0.5&&brown(cs.outlineColor))hit=true;}if(!hit)continue;e.setAttribute('data-ad-cardborder','1');e.setAttribute('data-ad-border-section','fast367');e.style.setProperty('border-color','#3b4043','important');e.style.setProperty('outline-color','#3b4043','important');e.__adBy='cardborder367';n++;}window.__AD_BORDERFAST367__=n;return n;}catch(e){return 0;}}"
         "try{window._adProductAds367=_adProductAds367;window._adBorderFast367=_adBorderFast367;_adProductAds367(document.documentElement);_adBorderFast367(document.documentElement);}catch(e){}"
         ...
         "try{var _t=null;new MutationObserver(function(muts){var structural=0;"
           "try{for(var m2=0;m2<muts.length&&m2<40;m2++){var mm2=muts[m2];if(mm2.type==='attributes'){if(mm2.target){_adTameFast362(mm2.target);_adBorderFast367(mm2.target);if(mm2.target.hasAttribute&&mm2.target.hasAttribute('data-ad-sponsored-light363'))_adSponsorPin366(mm2.target);if(mm2.target.hasAttribute&&mm2.target.hasAttribute('data-ad-reviewink367')){mm2.target.style.setProperty('color','#e8e6e3','important');mm2.target.style.setProperty('-webkit-text-fill-color','#e8e6e3','important');}}continue;}structural=1;var ad=mm2.addedNodes;for(var a2=0;a2<ad.length&&a2<20;a2++){_adPin(ad[a2]);_adTameFast362(ad[a2]);_adTextPins363(ad[a2]);if(ad[a2]&&ad[a2].nodeType===1){var tx367=String(ad[a2].textContent||'');if(tx367.length<7000&&/sponsored/i.test(tx367))_adProductAds367(ad[a2]);_adBorderFast367(ad[a2]);}}try{_adVideoCtlMain362();}catch(e){}}}catch(e){}"
...
static void ADPrimeNativeWhiteTame363(UIView *v, UIImage *incoming){
    @try {
        ...
        int ctx=ADWTStableContext365(v);
        CALayer *ov=objc_getAssociatedObject(v,kADWhiteTameOverlayKey);
        if(ctx==1||ADWTNoTameGlyph367(v)){ if(ov){[ov removeFromSuperlayer];objc_setAssociatedObject(v,kADWhiteTameOverlayKey,nil,OBJC_ASSOCIATION_RETAIN_NONATOMIC);} objc_setAssociatedObject(v,kADWTForcedImage364,nil,OBJC_ASSOCIATION_RETAIN_NONATOMIC); return; }
...
"/*V5317FIX*/"
       ...
       "try{var pa31=document.querySelectorAll('[data-ad-productad367]').length,ri31=document.querySelectorAll('[data-ad-reviewink367]').length,bf31=document.querySelectorAll('[data-ad-border-section=\"fast367\"]').length;out.push('P31FIX[productAd='+pa31+' reviewInk='+ri31+' borderFast='+bf31+']');}catch(e31){out.push('P31FIX[err '+(e31&&e31.message||e31)+']');}"
