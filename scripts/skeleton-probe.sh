...
# v7.445 export is current-session-only and ZIP-compressed; historical captures are never bundled.
set -eu
AD_PROBE_VERSION=7.445
...
AD_PROBE_NAME=AmazonDark-v7.445
...
case "${1:-}" in
 arm)
    ...
    case "$AD_PROBE_INSTALLED" in 7.445~*) ;; *) printf 'Install the v7.445 Actions package first. Installed: %s\n' "$AD_PROBE_INSTALLED" >&2; exit 1;; esac
    ...
    if [ "$AD_PROBE_LABEL" = launch ]; then
        ...
        printf 'Reproduce the target transition within 5 minutes. One fresh process records up to 120 seconds and stays armed across background/foreground cycles. Export returns only the newest transition capture created by this arm.\n'
    else
