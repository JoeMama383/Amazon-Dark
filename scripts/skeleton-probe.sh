...
# v7.501 export is current-session-only and plain TAR; historical captures are never bundled.
# FULL, VIEWPORT, and TRANSITION deliberately share the same archive format.
set -eu
AD_PROBE_VERSION=7.501
...
AD_PROBE_NAME=AmazonDark-v7.501
...
case "${1:-}" in
 arm)
    ...
    case "$AD_PROBE_INSTALLED" in 7.501~*) ;; *) printf 'Install the v7.501 Actions package first. Installed: %s\n' "$AD_PROBE_INSTALLED" >&2; exit 1;; esac
