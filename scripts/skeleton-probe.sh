...
AD_PROBE_ARM="$AD_PROBE_ROOT/AmazonDark-v7.340-skeleton.arm"
...
case "${1:-}" in
    ...
    status)
        ...
        for AD_PROBE_FILE in "$AD_PROBE_ROOT"/AmazonDark-v7.340-skeleton-*.jsonl; do
    ...
    export)
        ...
        [ "$#" -gt 0 ] || { printf 'No v7.340 skeleton capture found. Force-close Amazon, arm, then reopen it.\n' >&2; exit 1; }
        AD_PROBE_ARCHIVE="$AD_PROBE_DOCS/AmazonDark-v7.340-skeleton-probes-$(date +%Y%m%d-%H%M%S).tar.gz"
