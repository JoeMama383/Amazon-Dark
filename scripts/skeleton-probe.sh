...
AD_PROBE_NAME=AmazonDark-v7.437
...
ad_report() {
    ...
    if [ -f "$AD_PROBE_ROOT/AmazonDark-v7.437-launch-sb-probe.txt" ]; then
        ls -ln "$AD_PROBE_ROOT/AmazonDark-v7.437-launch-sb-probe.txt"
        tail -n 12 "$AD_PROBE_ROOT/AmazonDark-v7.437-launch-sb-probe.txt"
    else printf 'Missing: AmazonDark-v7.437-launch-sb-probe.txt\n'; fi
}
case "${1:-}" in
 arm)
    ...
    case "$AD_PROBE_INSTALLED" in 7.437~*) ;; *) printf 'Install the v7.437 Actions package first. Installed: %s\n' "$AD_PROBE_INSTALLED" >&2; exit 1;; esac
    umask 077
    while IFS= read -r AD_PROBE_DIR; do
 ...
 export)
    ...
    if [ -f "$AD_PROBE_ROOT/AmazonDark-v7.437-launch-sb-probe.txt" ]; then
        tail -c 4194304 "$AD_PROBE_ROOT/AmazonDark-v7.437-launch-sb-probe.txt" > "$AD_PROBE_STAGE/launch-springboard-last4MiB.txt"
    fi
