...
AD_PROBE_NAME=AmazonDark-v7.378
...
AD_PROBE_RECEIPTS=0
for AD_PROBE_RECEIPT in "$AD_PROBE_CONTAINERS"/*/Documents/"$AD_PROBE_NAME-probe-status.json" "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.377-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.376-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.375-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.374-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.373-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.372-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.371-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.370-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.369-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.368-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.367-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.366-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.365-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.364-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.363-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.362-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.361-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.360-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.359-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.358-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.357-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.356-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.355-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.354-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.350-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.346-probe-status.json "$AD_PROBE_CONTAINERS"/*/Documents/AmazonDark-v7.344-probe-status.json; do
    ...
    if LC_ALL=C grep -Eq '"bundle"[[:space:]]*:[[:space:]]*"com[.]amazon[.]Amazon"' "$AD_PROBE_RECEIPT" &&
       ...
       LC_ALL=C grep -Eq '"version"[[:space:]]*:[[:space:]]*"v7[.](344|346|347|348|349|350|351|352|353|354|355|356|357|358|359|360|361|362|363|364|365|366|367|368|369|370|371|372|373|374|375|376|377|378)-' "$AD_PROBE_RECEIPT"; then
...
ad_report() {
    ...
    if [ -f "$AD_PROBE_ROOT/AmazonDark-v7.378-launch-sb-probe.txt" ]; then
        ls -ln "$AD_PROBE_ROOT/AmazonDark-v7.378-launch-sb-probe.txt"
        tail -n 12 "$AD_PROBE_ROOT/AmazonDark-v7.378-launch-sb-probe.txt"
    else printf 'Missing: AmazonDark-v7.378-launch-sb-probe.txt\n'; fi
}
case "${1:-}" in
 arm)
    ...
    case "$AD_PROBE_INSTALLED" in 7.378~*) ;; *) printf 'Install the v7.378 Actions package first. Installed: %s\n' "$AD_PROBE_INSTALLED" >&2; exit 1;; esac
    umask 077
    while IFS= read -r AD_PROBE_DIR; do
    ...
    if [ "$AD_PROBE_LABEL" = launch ]; then
        ...
        printf 'Reproduce the target transition within 5 minutes. The armed trace records startup/native lifecycle evidence for up to 45 seconds per fresh process and stops on background. For app-switcher/warm-resume issues, leave Amazon on a recognizable screen, enter the switcher, then return to Amazon.\n'
    else
 ...
 export)
    ...
    if [ -f "$AD_PROBE_ROOT/AmazonDark-v7.378-launch-sb-probe.txt" ]; then
        tail -c 4194304 "$AD_PROBE_ROOT/AmazonDark-v7.378-launch-sb-probe.txt" > "$AD_PROBE_STAGE/launch-springboard-last4MiB.txt"
    fi
