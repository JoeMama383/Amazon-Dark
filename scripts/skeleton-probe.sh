...
# v7.491 export is current-session-only and plain TAR; historical captures are never bundled.
# FULL, VIEWPORT, and TRANSITION deliberately share the same archive format.
set -eu
AD_PROBE_VERSION=7.491
...
AD_PROBE_NAME=AmazonDark-v7.491
...
case "${1:-}" in
 arm)
    ...
    case "$AD_PROBE_INSTALLED" in 7.491~*) ;; *) printf 'Install the v7.491 Actions package first. Installed: %s\n' "$AD_PROBE_INSTALLED" >&2; exit 1;; esac
    ...
    if [ "$AD_PROBE_LABEL" = launch ]; then
        ...
        printf 'Force-close and reopen Amazon within 5 minutes, then reproduce the complete transition in both directions (present and dismiss). One fresh process records up to 120 seconds and stays armed across background/foreground cycles. Export returns only the newest transition capture created by this arm.\n'
    else
