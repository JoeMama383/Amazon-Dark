#!/bin/sh
# AmazonDark v7.378 universal UI probe helper.
...
case "${1:-}" in
  arm)
    case "$installed" in 7.378~*) ;; *) printf 'Install/open the v7.378 package first. Installed: %s\n' "$installed" >&2; exit 1;; esac
  ...
  export)
    ...
    while IFS= read -r d; do
      for kind in ui-full-probe ui-viewport-probe; do
        newest=""
        for f in $(ls -1t "$d"/AmazonDark-v7.378-${kind}-*.txt 2>/dev/null || true); do
  ...
  status)
    printf 'Installed: %s\n' "$installed"
    while IFS= read -r d; do printf 'Amazon Documents: %s\n' "$d"; ls -1t "$d"/AmazonDark-v7.378-ui-*-probe-*.txt 2>/dev/null | head -6 || true; [ -f "$d/$NAME-ui-viewport.arm" ] && printf 'Viewport arm: '; [ -f "$d/$NAME-ui-viewport.arm" ] && cat "$d/$NAME-ui-viewport.arm"; done < "$TARGETS"
    ;;
  disarm)
