#!/bin/sh
# AmazonDark v7.369 universal UI probe helper.
...
for r in "$CONTAINERS"/*/Documents/"$NAME-probe-status.json" "$CONTAINERS"/*/Documents/AmazonDark-v7.368-probe-status.json "$CONTAINERS"/*/Documents/AmazonDark-v7.367-probe-status.json "$CONTAINERS"/*/Documents/AmazonDark-v7.366-probe-status.json "$CONTAINERS"/*/Documents/AmazonDark-v7.365-probe-status.json "$CONTAINERS"/*/Documents/AmazonDark-v7.364-probe-status.json "$CONTAINERS"/*/Documents/AmazonDark-v7.363-probe-status.json "$CONTAINERS"/*/Documents/AmazonDark-v7.362-probe-status.json; do
  [ -f "$r" ] || continue
...
case "${1:-}" in
  arm)
    case "$installed" in 7.369~*) ;; *) printf 'Install/open the v7.369 package first. Installed: %s\n' "$installed" >&2; exit 1;; esac
  ...
  export)
    ...
    while IFS= read -r d; do
      for kind in ui-full-probe ui-viewport-probe; do
        newest=""
        for f in $(ls -1t "$d"/AmazonDark-v7.369-${kind}-*.txt 2>/dev/null || true); do
    ...
    if [ "$found" -eq 0 ]; then
      [ "$partial" -eq 0 ] || { printf 'Newest v7.369 universal UI probe is still sweeping. Keep Amazon foregrounded, then rerun export.\n' >&2; exit 1; }
      printf 'No completed v7.369 universal UI captures found.\n' >&2; exit 1
    fi
    ;;
  status)
    printf 'Installed: %s\n' "$installed"
    while IFS= read -r d; do printf 'Amazon Documents: %s\n' "$d"; ls -1t "$d"/AmazonDark-v7.369-ui-*-probe-*.txt 2>/dev/null | head -6 || true; [ -f "$d/$NAME-ui-viewport.arm" ] && printf 'Viewport arm: '; [ -f "$d/$NAME-ui-viewport.arm" ] && cat "$d/$NAME-ui-viewport.arm"; done < "$TARGETS"
    ;;
  disarm)
