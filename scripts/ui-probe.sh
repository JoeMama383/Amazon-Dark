#!/bin/sh
# AmazonDark v7.363 universal UI probe helper.
...
for r in "$CONTAINERS"/*/Documents/"$NAME-probe-status.json" "$CONTAINERS"/*/Documents/AmazonDark-v7.362-probe-status.json; do
  [ -f "$r" ] || continue
...
case "${1:-}" in
  arm)
    case "$installed" in 7.363~*) ;; *) printf 'Install/open the v7.363 package first. Installed: %s\n' "$installed" >&2; exit 1;; esac
  ...
  export)
    ...
    while IFS= read -r d; do
      for kind in ui-full-probe ui-viewport-probe; do
        newest=$(ls -1t "$d"/AmazonDark-v7.363-${kind}-*.txt 2>/dev/null | head -1 || true)
        [ -n "$newest" ] || continue
        cp -f "$newest" "$SHARED/"; chmod 666 "$SHARED/${newest##*/}" 2>/dev/null || true; ls -lh "$SHARED/${newest##*/}"; found=$((found+1))
      done
    done < "$TARGETS"
    [ "$found" -gt 0 ] || { printf 'No v7.363 universal UI captures found.\n' >&2; exit 1; }
    ;;
  status)
    printf 'Installed: %s\n' "$installed"
    while IFS= read -r d; do printf 'Amazon Documents: %s\n' "$d"; ls -1t "$d"/AmazonDark-v7.363-ui-*-probe-*.txt 2>/dev/null | head -6 || true; [ -f "$d/$NAME-ui-viewport.arm" ] && printf 'Viewport arm: '; [ -f "$d/$NAME-ui-viewport.arm" ] && cat "$d/$NAME-ui-viewport.arm"; done < "$TARGETS"
    ;;
  disarm)
