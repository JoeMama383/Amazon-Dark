# AmazonDark v7.467 commands

## Push
```sh
cd /var/mobile/Amazon-Dark-phone &&
AD_DOCS=/private/var/mobile/Containers/Shared/AppGroup/D846D8DE-EE0F-4B82-9676-C68769E519CD/Documents &&
AD_STAGE=$(mktemp -d /var/mobile/ad7467.XXXXXX) &&
unzip -q "$AD_DOCS/AmazonDark-v7.467-pdp-ad-book-strict-repair-source.zip" -d "$AD_STAGE" &&
cp -a "$AD_STAGE/AmazonDark-v7.467-pdp-ad-book-strict-repair-source/." . &&
chmod 755 layout/DEBIAN/postinst scripts/ui-probe.sh scripts/skeleton-probe.sh scripts/validate.sh &&
AD_STRICT_VALIDATE=0 sh scripts/validate.sh &&
git add -A &&
if ! git diff --cached --quiet; then git commit -m "v7.467: repair strict PDP compatibility"; fi &&
...
```
Return to Amazon with the exact target visible, then background Amazon once.
```sh
