...
AD_STAGE=$(mktemp -d /var/mobile/ad7488.XXXXXX) &&
unzip -q "$AD_DOCS/AmazonDark-v7.488-returns-geometry-header-cta-source.zip" -d "$AD_STAGE" &&
...
git add -A &&
if ! git diff --cached --quiet; then git commit -m "v7.488: preserve return alert geometry and fix returns headers and CTAs"; fi &&
