...
AD_STAGE=$(mktemp -d /var/mobile/ad7494.XXXXXX) &&
unzip -q "$AD_DOCS/AmazonDark-v7.494-orders-exact-fix-source.zip" -d "$AD_STAGE" &&
...
git add -A &&
if ! git diff --cached --quiet; then git commit -m "v7.494: fix exact Orders divider, search seam, and quantity badge owners"; fi &&
