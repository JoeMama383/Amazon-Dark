...
AD_STAGE=$(mktemp -d /var/mobile/ad7484.XXXXXX) &&
unzip -q "$AD_DOCS/AmazonDark-v7.484-ci-build-repair-ui-followup-source.zip" -d "$AD_STAGE" &&
...
git add -A &&
if ! git diff --cached --quiet; then git commit -m "v7.484: repair CI validation and finish UI follow-up"; fi &&
