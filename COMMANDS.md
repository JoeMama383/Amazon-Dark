...
AD_STAGE=$(mktemp -d /var/mobile/ad7493.XXXXXX) &&
unzip -q "$AD_DOCS/AmazonDark-v7.493-orders-endtext-ci-repair-source.zip" -d "$AD_STAGE" &&
...
git add -A &&
if ! git diff --cached --quiet; then git commit -m "v7.493: exact-sync source, fix Orders end text, and retain Orders follow-up"; fi &&
