# AmazonDark v7.471 commands

## Push
```sh
cd /var/mobile/Amazon-Dark-phone &&
AD_DOCS=/private/var/mobile/Containers/Shared/AppGroup/D846D8DE-EE0F-4B82-9676-C68769E519CD/Documents &&
AD_STAGE=$(mktemp -d /var/mobile/ad7471.XXXXXX) &&
unzip -q "$AD_DOCS/AmazonDark-v7.471-pdp-four-adcard-repair-source.zip" -d "$AD_STAGE" &&
cp -a "$AD_STAGE/AmazonDark-v7.471-pdp-four-adcard-repair-source/." . &&
chmod 755 layout/DEBIAN/postinst scripts/ui-probe.sh scripts/skeleton-probe.sh scripts/validate.sh &&
AD_STRICT_VALIDATE=0 sh scripts/validate.sh &&
git add -A &&
if ! git diff --cached --quiet; then git commit -m "v7.471: persist PDP ad-card theme before hydration"; fi &&
...
```
Return to the exact target ad card, then background Amazon once.
```sh
