...
AD_STAGE=$(mktemp -d /var/mobile/ad7490.XXXXXX) &&
unzip -q "$AD_DOCS/AmazonDark-v7.490-orders-book-transition-source.zip" -d "$AD_STAGE" &&
...
git add -A &&
if ! git diff --cached --quiet; then git commit -m "v7.490: theme Your Orders and fix Book Details transition canvas"; fi &&
